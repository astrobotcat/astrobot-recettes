"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = parseImageDimensionError;exports.a = formatAssistantErrorText;exports.c = isCloudCodeAssistFormatError;exports.d = isFailoverAssistantError;exports.f = isFailoverErrorMessage;exports.g = isTransientHttpError;exports.h = isReasoningConstraintErrorMessage;exports.i = extractObservedOverflowTokenCount;exports.l = isCompactionFailureError;exports.m = isRateLimitAssistantError;exports.n = classifyFailoverSignal;exports.o = isAuthAssistantError;exports.p = isLikelyContextOverflowError;exports.r = classifyProviderRuntimeFailureKind;exports.s = isBillingAssistantError;exports.t = classifyFailoverReason;exports.u = isContextOverflowError;exports.v = parseImageSizeError;var _nodeRequireDvwmVoHm = require("./node-require-DvwmVoHm.js");
var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _subsystemCgmckbux = require("./subsystem-Cgmckbux.js");
var _assistantErrorFormatTQ4SCN8S = require("./assistant-error-format-tQ4SCN8S.js");
var _oauthRefreshFailureCdIusR4x = require("./oauth-refresh-failure-CdIusR4x.js");
...
const requireProviderRuntime = (0, _nodeRequireDvwmVoHm.t)("file:///home/linuxbrew/.linuxbrew/lib/node_modules/openclaw/dist/errors-CJULmF31.js");
...
const log = (0, _subsystemCgmckbux.t)("errors");
function isReasoningConstraintErrorMessage(raw) {
  if (!raw) return false;
  const lower = (0, _stringCoerceBUSzWgUA.i)(raw);
...
function isContextOverflowError(errorMessage) {
  ...
  const lower = (0, _stringCoerceBUSzWgUA.i)(errorMessage);
...
function isLikelyContextOverflowError(errorMessage) {
  ...
  if (hasRateLimitTpmHint(errorMessage)) return false;
  if (isReasoningConstraintErrorMessage(errorMessage)) return false;
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.v)(errorMessage)) return false;
  if (CONTEXT_WINDOW_TOO_SMALL_RE.test(errorMessage)) return false;
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.x)(errorMessage)) return false;
...
function inferSignalStatus(signal) {
  ...
  return (0, _assistantErrorFormatTQ4SCN8S.t)(stripErrorPrefix(signal.message?.trim() ?? ""))?.code;
}
function isHtmlErrorResponse(raw, status) {
  const trimmed = raw.trim();
  if (!trimmed) return false;
  const candidate = (0, _assistantErrorFormatTQ4SCN8S.t)(trimmed) ? trimmed : stripErrorPrefix(trimmed);
  const inferred = typeof status === "number" && Number.isFinite(status) ? status : (0, _assistantErrorFormatTQ4SCN8S.t)(candidate)?.code;
  if (typeof inferred !== "number" || inferred < 400) return false;
  const rest = (0, _assistantErrorFormatTQ4SCN8S.t)(candidate)?.rest ?? candidate;
...
function isOpenAICodexScopeContext(raw, provider) {
  return (0, _stringCoerceBUSzWgUA.i)(provider) === "openai-codex" || /\bopenai\s+codex\b/i.test(raw) || /\bcodex\b.*\bscopes?\b/i.test(raw);
}
function isAuthScopeErrorMessage(raw, status, provider) {
  if (!raw) return false;
  if (!isOpenAICodexScopeContext(raw, provider)) return false;
  const inferred = typeof status === "number" && Number.isFinite(status) ? status : (0, _assistantErrorFormatTQ4SCN8S.t)(raw.trim())?.code;
...
function isSandboxBlockedErrorMessage(raw) {
  return Boolean((0, _sanitizeUserFacingTextCQF1CTnZ.T)(raw)) || SANDBOX_BLOCKED_RE.test(raw);
}
function isSchemaErrorMessage(raw) {
  if (!raw || isReplayInvalidErrorMessage(raw) || isContextOverflowError(raw)) return false;
  return classifyFailoverReason(raw) === "format" || (0, _sanitizeUserFacingTextCQF1CTnZ.w)(raw);
}
function isTimeoutTransportErrorMessage(raw, status) {
  if (!raw) return false;
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.C)(raw) || INTERRUPTED_NETWORK_ERROR_RE.test(raw)) return true;
...
function normalize402Message(raw) {
  return (0, _stringCoerceBUSzWgUA.o)(raw)?.replace(LEADING_402_WRAPPER_RE, "").trim() ?? "";
}
function classify402Message(message) {
  ...
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.x)(normalized)) return "rate_limit";
...
function isTransientHttpError(raw) {
  const trimmed = raw.trim();
  if (!trimmed) return false;
  const status = (0, _assistantErrorFormatTQ4SCN8S.t)(trimmed);
...
function classifyFailoverClassificationFromHttpStatus(status, message, messageClassification) {
  ...
  if (status === 401 || status === 403) {
    if (message && (0, _sanitizeUserFacingTextCQF1CTnZ._)(message)) return toReasonClassification("auth_permanent");
...
function isAnthropicGenericUnknownError(raw, provider) {
  return isProvider(provider, "anthropic") && ((0, _stringCoerceBUSzWgUA.o)(raw)?.includes("an unknown error occurred") ?? false);
}
function isOpenRouterProviderReturnedError(raw, provider) {
  return isProvider(provider, "openrouter") && ((0, _stringCoerceBUSzWgUA.o)(raw)?.includes("provider returned error") ?? false);
}
...
function isExactUnknownNoDetailsError(raw) {
  return (0, _stringCoerceBUSzWgUA.o)(raw)?.trim() === "unknown error (no error details in response)";
}
function classifyFailoverClassificationFromMessage(raw, provider) {
  ...
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.b)(raw)) return toReasonClassification((0, _sanitizeUserFacingTextCQF1CTnZ.v)(raw) ? "billing" : "rate_limit");
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.x)(raw)) return toReasonClassification("rate_limit");
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.y)(raw)) return toReasonClassification("overloaded");
  if (isTransientHttpError(raw)) {
    if ((0, _assistantErrorFormatTQ4SCN8S.t)(raw.trim())?.code === 529) return toReasonClassification("overloaded");
    return toReasonClassification("timeout");
  }
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.v)(raw)) return toReasonClassification("billing");
  if ((0, _sanitizeUserFacingTextCQF1CTnZ._)(raw)) return toReasonClassification("auth_permanent");
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.g)(raw)) return toReasonClassification("auth");
  ...
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.S)(raw)) return toReasonClassification("timeout");
  ...
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.C)(raw)) return toReasonClassification("timeout");
...
function classifyProviderRuntimeFailureKind(signal) {
  ...
  if (message && (0, _oauthRefreshFailureCdIusR4x.n)(message)) return "auth_refresh";
...
function formatAssistantErrorText(msg, opts) {
  ...
  const providerRuntimeFailureKind = classifyProviderRuntimeFailureKind({
    status: (0, _assistantErrorFormatTQ4SCN8S.t)(raw)?.code,
    message: raw,
  ...
  if (unknownTool?.[1]) {
    const rewritten = (0, _runtimeStatusDhGewqgv.t)({
      cfg: opts?.cfg,
  ...
  const diskSpaceCopy = (0, _sanitizeUserFacingTextCQF1CTnZ.r)(raw);
  ...
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.s)(raw)) return "LLM request failed: provider returned an invalid streaming response. Please try again.";
  ...
  const transientCopy = (0, _sanitizeUserFacingTextCQF1CTnZ.i)(raw);
  if (transientCopy) return transientCopy;
  const transportCopy = (0, _sanitizeUserFacingTextCQF1CTnZ.a)(raw);
  if (transportCopy) return transportCopy;
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.C)(raw)) return "LLM request timed out.";
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.v)(raw)) return (0, _sanitizeUserFacingTextCQF1CTnZ.n)(opts?.provider, opts?.model ?? msg.model);
  ...
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.c)(raw) || (0, _sanitizeUserFacingTextCQF1CTnZ.l)(raw)) return (0, _assistantErrorFormatTQ4SCN8S.n)(raw);
  if (raw.length > 600) log.warn(`Long error truncated: ${raw.slice(0, 200)}`);
  return raw.length > 600 ? `${raw.slice(0, 600)}…` : raw;
}
function isRateLimitAssistantError(msg) {
  if (!msg || msg.stopReason !== "error") return false;
  return (0, _sanitizeUserFacingTextCQF1CTnZ.x)(msg.errorMessage ?? "");
}
...
function isBillingAssistantError(msg) {
  if (!msg || msg.stopReason !== "error") return false;
  return (0, _sanitizeUserFacingTextCQF1CTnZ.v)(msg.errorMessage ?? "");
}
...
function isJsonApiInternalServerError(raw) {
  if (!raw) return false;
  if (!(0, _stringCoerceBUSzWgUA.i)(raw).includes("\"type\":\"api_error\"")) return false;
  if ((0, _sanitizeUserFacingTextCQF1CTnZ.v)(raw) || (0, _sanitizeUserFacingTextCQF1CTnZ.g)(raw) || (0, _sanitizeUserFacingTextCQF1CTnZ._)(raw)) return false;
  return API_ERROR_TRANSIENT_SIGNALS_RE.test(raw);
}
function parseImageDimensionError(raw) {
  if (!raw) return null;
  if (!(0, _stringCoerceBUSzWgUA.i)(raw).includes("image dimensions exceed max allowed size")) return null;
...
function isCloudCodeAssistFormatError(raw) {
  return !isImageDimensionErrorMessage(raw) && (0, _sanitizeUserFacingTextCQF1CTnZ.w)(raw);
}
function isAuthAssistantError(msg) {
  if (!msg || msg.stopReason !== "error") return false;
  return (0, _sanitizeUserFacingTextCQF1CTnZ.g)(msg.errorMessage ?? "");
}
...
function classifyFailoverReason(raw, opts) {
  return failoverReasonFromClassification(classifyFailoverSignal({
    status: (0, _assistantErrorFormatTQ4SCN8S.t)(raw.trim())?.code,
    message: raw,
