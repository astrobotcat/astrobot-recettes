"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveContextTokensForModel;exports.i = lookupContextTokens;exports.n = applyConfiguredContextWindows;exports.o = resetContextWindowCacheForTest;exports.r = applyDiscoveredContextWindows;exports.t = void 0;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
const ANTHROPIC_CONTEXT_1M_TOKENS = exports.t = 1048576;
...
function loadModelsConfigRuntime() {
  CONTEXT_WINDOW_RUNTIME_STATE.modelsConfigRuntimePromise ??= Promise.resolve().then(() => jitiImport("./agents/models-config.runtime.js").then((m) => _interopRequireWildcard(m)));
  return CONTEXT_WINDOW_RUNTIME_STATE.modelsConfigRuntimePromise;
}
function isLikelyOpenClawCliProcess(argv = process.argv) {
  const entryBasename = (0, _stringCoerceBUSzWgUA.i)(_nodePath.default.basename(argv[1] ?? ""));
...
function getCommandPathFromArgv(argv) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    const arg = args[i];
    if (!arg || arg === "--") break;
    const consumed = (0, _loggerD8OnBgBc.I)(args, i);
    if (consumed > 0) {
      i += consumed - 1;
      continue;
    }
...
function primeConfiguredContextWindows() {
  ...
  try {
    ...
    const backoffMs = (0, _backoffDdY2WSF.t)(CONFIG_LOAD_RETRY_POLICY, CONTEXT_WINDOW_RUNTIME_STATE.configLoadFailures);
...
function ensureContextWindowCacheLoaded() {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    ...
    try {
      const { discoverAuthStorage, discoverModels } = await Promise.resolve().then(() => jitiImport("./agents/pi-model-discovery-runtime.js").then((m) => _interopRequireWildcard(m)));
...
function lookupContextTokens(modelId, options) {
  if (!modelId) return;
  if (options?.allowAsyncLoad === false) primeConfiguredContextWindows();else
...
function resolveConfiguredModelParams(cfg, provider, model) {
  ...
  if (!models) return;
  const key = (0, _stringCoerceBUSzWgUA.i)(`${provider}/${model}`);
  for (const [rawKey, entry] of Object.entries(models)) if ((0, _stringCoerceBUSzWgUA.i)(rawKey) === key) {
...
function resolveProviderModelRef(params) {
  ...
  if (slash <= 0) return;
  const provider = (0, _providerIdKaStHhRz.r)(modelRaw.slice(0, slash));
...
function resolveConfiguredProviderContextTokens(cfg, provider, model) {
  ...
  const exactResult = findContextTokens((id) => (0, _stringCoerceBUSzWgUA.i)(id) === (0, _stringCoerceBUSzWgUA.i)(provider));
  if (exactResult !== void 0) return exactResult;
  const normalizedProvider = (0, _providerIdKaStHhRz.r)(provider);
  return findContextTokens((id) => (0, _providerIdKaStHhRz.r)(id) === normalizedProvider);
}
...
function resolveContextTokensForModel(params) {
  ...
  if (params.provider && ref && !ref.model.includes("/")) {
    const qualifiedResult = lookupContextTokens(`${(0, _providerIdKaStHhRz.r)(ref.provider)}/${ref.model}`, { allowAsyncLoad: params.allowAsyncLoad });
