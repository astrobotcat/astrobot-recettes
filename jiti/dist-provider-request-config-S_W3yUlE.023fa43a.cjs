"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = mergeModelProviderRequestOverrides;exports.c = resolveProviderRequestPolicyConfig;exports.d = sanitizeRuntimeProviderRequestOverrides;exports.i = getModelProviderRequestTransport;exports.l = sanitizeConfiguredModelProviderRequest;exports.n = buildProviderRequestDispatcherPolicy;exports.o = normalizeBaseUrl;exports.r = buildProviderRequestTlsClientOptions;exports.s = resolveProviderRequestConfig;exports.t = attachModelProviderRequestTransport;exports.u = sanitizeConfiguredProviderRequest;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function sanitizeConfiguredProviderRequest(request) {
  ...
  if (rawAuth && typeof rawAuth === "object" && !Array.isArray(rawAuth)) {
    if (rawAuth.mode === "provider-default") auth = { mode: "provider-default" };else
    if (rawAuth.mode === "authorization-bearer") {
  ...
  const sanitizeTls = (tls, pathPrefix) => {
    ...
    if (rawTls.insecureSkipVerify === true) next.insecureSkipVerify = true;else
    if (rawTls.insecureSkipVerify === false) next.insecureSkipVerify = false;
  ...
  if (rawProxy && typeof rawProxy === "object" && !Array.isArray(rawProxy)) {
    ...
    if (rawProxy.mode === "explicit-proxy") {
...
function mergeProviderRequestHeaders(...headerSets) {
  ...
  for (const headers of headerSets) {
    ...
    for (const [key, value] of Object.entries(headers)) {
      const normalizedKey = (0, _stringCoerceBUSzWgUA.i)(key);
...
function applyResolvedAuthHeader(headers, auth) {
  ...
  const keysToDelete = new Set([(0, _stringCoerceBUSzWgUA.i)(auth.headerName)]);
  if (auth.mode === "header") keysToDelete.add("authorization");
  for (const key of Object.keys(next)) if (keysToDelete.has((0, _stringCoerceBUSzWgUA.i)(key))) delete next[key];
...
function resolveProviderRequestPolicyConfig(params) {
  ...
  const policy = (0, _providerAttributionBJhkfmfB.i)(policyInput);
  ...
  const protectedAttributionKeys = new Set(Object.keys(policy.attributionHeaders ?? {}).map((key) => (0, _stringCoerceBUSzWgUA.i)(key)));
  const unprotectedCallerHeaders = params.callerHeaders ? Object.fromEntries(Object.entries(params.callerHeaders).filter(([key]) => !protectedAttributionKeys.has((0, _stringCoerceBUSzWgUA.i)(key)))) : void 0;
