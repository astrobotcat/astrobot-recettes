"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = actionHasTarget;exports.n = exports.i = void 0;exports.o = actionRequiresTarget;exports.r = applyTargetToParams;exports.t = void 0;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function listActionTargetAliasSpecs(action, channel) {
  ...
  const normalizedChannel = (0, _stringCoerceBUSzWgUA.o)(channel);
  if (!normalizedChannel) return specs;
  const channelSpec = (0, _bootstrapRegistryBI6Zdzz.t)(normalizedChannel)?.actions?.messageActionTargetAliases?.[action];
...
function actionHasTarget(action, params, options) {
  if ((0, _stringCoerceBUSzWgUA.s)(params.to) ?? "") return true;
  if ((0, _stringCoerceBUSzWgUA.s)(params.channelId) ?? "") return true;
  const specs = listActionTargetAliasSpecs(action, options?.channel);
  if (specs.length === 0) return false;
  return specs.some((spec) => spec.aliases.some((alias) => {
    const value = params[alias];
    if (typeof value === "string") return Boolean((0, _stringCoerceBUSzWgUA.s)(value));
...
const hasNonEmptyString = exports.i = _stringCoerceBUSzWgUA.t;
...
function applyTargetToParams(params) {
  const target = (0, _stringCoerceBUSzWgUA.s)(params.args.target) ?? "";
