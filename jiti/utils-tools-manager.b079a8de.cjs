...
async function downloadTool(tool) {
  ...
  const extractDir = (0, _path.join)(TOOLS_DIR, `extract_tmp_${config.binaryName}_${process.pid}_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`);
  (0, _fs.mkdirSync)(extractDir, { recursive: true });
  try {
    if (assetName.endsWith(".tar.gz")) {
      const extractResult = (0, _child_process.spawnSync)("tar", ["xzf", archivePath, "-C", extractDir], { stdio: "pipe" });
    ...
    if (assetName.endsWith(".zip")) {
      await (0, _extractZip.default)(archivePath, { dir: extractDir });
    } else
    {
