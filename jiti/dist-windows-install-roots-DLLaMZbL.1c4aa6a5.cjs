"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = getWindowsProgramFilesRoots;exports.t = getWindowsInstallRoots;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function getWindowsRegExeCandidates(env) {
  ...
  {
    if (!root) continue;
    const key = (0, _stringCoerceBUSzWgUA.i)(root);
...
function getWindowsProgramFilesRoots(env = process.env) {
  ...
  {
    if (!value) continue;
    const key = (0, _stringCoerceBUSzWgUA.i)(value);
    if (seen.has(key)) continue;
    seen.add(key);
    result.push(value);
  }
