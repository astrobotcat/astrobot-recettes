"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = getActiveTaskCount;exports.c = markGatewayDraining;exports.d = waitForActiveTasks;exports.i = enqueueCommandInLane;exports.l = resetAllLanes;exports.n = void 0;exports.o = getQueueSize;exports.r = clearCommandLane;exports.s = getTotalQueueSize;exports.t = void 0;exports.u = setCommandLaneConcurrency;var _globalSingletonB80lDOJ = require("./global-singleton-B80lD-oJ.js");
...
function getQueueState() {
  const state = (0, _globalSingletonB80lDOJ.n)(COMMAND_QUEUE_STATE_KEY, () => ({
...
function normalizeLane(lane) {
  return lane.trim() || _lanesCU7Ll8dR.t.Main;
}
...
function drainLane(lane) {
  ...
  if (state.draining) {
    if (state.activeTaskIds.size === 0 && state.queue.length > 0) _diagnosticRuntimeDOaqJdZf.t.warn(`drainLane blocked: lane=${lane} draining=true active=0 queue=${state.queue.length}`);
    return;
  }
  state.draining = true;
  const pump = () => {
      ...
      while (state.activeTaskIds.size < state.maxConcurrent && state.queue.length > 0) {
        ...
        if (waitedMs >= entry.warnAfterMs) {
          try {
            ...
            _diagnosticRuntimeDOaqJdZf.t.error(`lane onWait callback failed: lane=${lane} error="${String(err)}"`);
          }
          _diagnosticRuntimeDOaqJdZf.t.warn(`lane wait exceeded: lane=${lane} waitedMs=${waitedMs} queueAhead=${state.queue.length}`);
        }
        (0, _diagnosticRuntimeDOaqJdZf.r)(lane, waitedMs, state.queue.length);
        ...
        (async () => {
          ...
          try {
            ...
            if (completeTask(state, taskId, taskGeneration)) {
              ...
              _diagnosticRuntimeDOaqJdZf.t.debug(`lane task done: lane=${lane} durationMs=${Date.now() - startTime} active=${state.activeTaskIds.size} queued=${state.queue.length}`);
              pump();
            }
            ...
            if (!isProbeLane && !isExpectedNonErrorLaneFailure(err)) _diagnosticRuntimeDOaqJdZf.t.error(`lane task error: lane=${lane} durationMs=${Date.now() - startTime} error="${String(err)}"`);else
            if (!isProbeLane) _diagnosticRuntimeDOaqJdZf.t.debug(`lane task interrupted: lane=${lane} durationMs=${Date.now() - startTime} reason="${String(err)}"`);
...
function enqueueCommandInLane(lane, task, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    (0, _diagnosticRuntimeDOaqJdZf.i)(cleaned, getLaneDepth(state));
    drainLane(cleaned);
  });
}
function getQueueSize(lane = _lanesCU7Ll8dR.t.Main) {
...
function clearCommandLane(lane = _lanesCU7Ll8dR.t.Main) {
  const cleaned = normalizeLane(lane);
