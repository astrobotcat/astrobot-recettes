 ...
 */let _createServer = null;let _httpImportPromise = null;if (typeof process !== "undefined" && (process.versions?.node || process.versions?.bun)) {_httpImportPromise = Promise.resolve().then(() => jitiImport("node:http").then((m) => _interopRequireWildcard(m))).then((m) => {_createServer = m.createServer;});
}
    ...
    const server = createServer((req, res) => {
      ...
      if (url.pathname === "/oauth-callback") {
        ...
        if (error) {
          ...
          res.end((0, _oauthPage.oauthErrorHtml)("Google authentication did not complete.", `Error: ${error}`));
          return;
        }
        if (code && state) {
          res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
          res.end((0, _oauthPage.oauthSuccessHtml)("Google authentication completed. You can close this window."));
