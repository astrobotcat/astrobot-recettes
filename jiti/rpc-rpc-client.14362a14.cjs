...
// RPC Client
// ============================================================================
class RpcClient {options;process = null;stopReadingStdout = null;
  eventListeners = [];
  ...
  async start() {
    ...
    this.stopReadingStdout = (0, _jsonl.attachJsonlLineReader)(this.process.stdout, (line) => {
  ...
  async send(command) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.process.stdin.write((0, _jsonl.serializeJsonLine)(fullCommand));
    });
  }
  getData(response) {
    if (!response.success) {
...
}exports.RpcClient = RpcClient; /* v9-5bbc750b50a6a9a0 */
