...
var _bash = require("./tools/bash.js");
...
class AgentSession {
  ...
  async _cycleScopedModel(direction) {
    ...
    let currentIndex = scopedModels.findIndex((sm) => (0, _piAi.modelsAreEqual)(sm.model, currentModel));
    if (currentIndex === -1)
    currentIndex = 0;
  ...
  _getThinkingLevelForModelSwitch(explicitLevel) {
    ...
    if (!this.supportsThinking()) {
      return this.settingsManager.getDefaultThinkingLevel() ?? _defaults.DEFAULT_THINKING_LEVEL;
    }
  ...
  async compact(customInstructions) {
    ...
    try {
      ...
      } else
      {
        ...
        const result = await (0, _index.compact)(preparation, this.model, apiKey, headers, customInstructions, this._compactionAbortController.signal);
  ...
  async _checkCompaction(assistantMessage, skipAbortedCheck = true) {
    ...
    const compactionEntry = (0, _sessionManager.getLatestCompactionEntry)(this.sessionManager.getBranch());
    ...
    if (sameModel && (0, _piAi.isContextOverflow)(assistantMessage, contextWindow)) {
    ...
    if (assistantMessage.stopReason === "error") {
      const messages = this.agent.state.messages;
      const estimate = (0, _index.estimateContextTokens)(messages);
    ...
    {
      contextTokens = (0, _index.calculateContextTokens)(assistantMessage.usage);
    }
  ...
  async _runAutoCompaction(reason, willRetry) {
    ...
    try {
      ...
      {
        // Generate compaction result
        const compactResult = await (0, _index.compact)(preparation, this.model, apiKey, headers, undefined, this._autoCompactionAbortController.signal);
  ...
  _refreshToolRegistry(options) {
    ...
    ...this._customTools.map((definition) => ({
      definition,
      sourceInfo: (0, _sourceInfo.createSyntheticSourceInfo)(`<sdk:${definition.name}>`, { source: "sdk" })
    }))];
    ...
    {
      definition,
      sourceInfo: (0, _sourceInfo.createSyntheticSourceInfo)(`<builtin:${name}>`, { source: "builtin" })
    }]
    ));
    ...
    definition.name,
    (0, _toolDefinitionWrapper.wrapToolDefinition)(definition)]
    ));
  ...
  _buildRuntime(options) {
    ...
    name,
    (0, _toolDefinitionWrapper.createToolDefinitionFromAgentTool)(tool)]
    )) :
    (0, _index4.createAllToolDefinitions)(this._cwd, {
  ...
  async executeBash(command, onChunk, options) {
    ...
    try {
      const result = await (0, _bashExecutor.executeBashWithOperations)(resolvedCommand, this.sessionManager.getCwd(), options?.operations ?? (0, _bash.createLocalBashOperations)(), {
        onChunk,
  ...
  getContextUsage() {
    ...
    const latestCompaction = (0, _sessionManager.getLatestCompactionEntry)(branchEntries);
    if (latestCompaction) {
      ...
      for (let i = branchEntries.length - 1; i > compactionIndex; i--) {
        ...
        if (entry.type === "message" && entry.message.role === "assistant") {
          ...
          if (assistant.stopReason !== "aborted" && assistant.stopReason !== "error") {
            const contextTokens = (0, _index.calculateContextTokens)(assistant.usage);
            if (contextTokens > 0) {
  ...
  async exportToHtml(outputPath) {
    ...
    const toolRenderer = (0, _toolRenderer.createToolHtmlRenderer)({
  ...
  exportToJsonl(outputPath) {
    const filePath = (0, _nodePath.resolve)(outputPath ?? `session-${new Date().toISOString().replace(/[:.]/g, "-")}.jsonl`);
    ...
    const header = {
      type: "session",
      version: _sessionManager.CURRENT_SESSION_VERSION,
    ...
    (0, _nodeFs.writeFileSync)(filePath, `${lines.join("\n")}\n`);
    return filePath;
  }
  // =========================================================================
  // Utilities
...
}exports.AgentSession = AgentSession; /* v9-b7a8540df5944126 */
