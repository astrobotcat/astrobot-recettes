"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = buildChannelSummary;var _themeD5sxSdHD = require("./theme-D5sxSdHD.js");
...
var _readOnlyAccountInspectBiLlFb9q = require("./read-only-account-inspect-BiLlFb9q.js");
var _accountSnapshotFieldsDXlFJrtW = require("./account-snapshot-fields-DXlFJrtW.js");
var _accountSummaryBQJhTf0A = require("./account-summary-BQJhTf0A.js");
...
const buildAccountDetails = (params) => {
  ...
  if ((0, _accountSnapshotFieldsDXlFJrtW.t)(params.entry.account)) details.push("secret unavailable in this command path");
  ...
  if (params.includeAllowFrom && snapshot.allowFrom?.length) {
    const formatted = (0, _accountSummaryBQJhTf0A.n)({
...
async function inspectChannelAccount(plugin, cfg, accountId) {
  return plugin.config.inspectAccount?.(cfg, accountId) ?? (await (0, _readOnlyAccountInspectBiLlFb9q.t)({
    channelId: plugin.id,
    cfg,
    accountId
  }));
...
async function buildChannelSummary(cfg, options) {
  const effective = cfg ?? (0, _io5pxHCi7V.a)();
  const lines = [];
  const resolved = {
    ...DEFAULT_OPTIONS,
    ...options
  ...
  for (const plugin of (0, _registryDelpa74L.r)()) {
    ...
    for (const accountId of resolvedAccountIds) {
      ...
      const useSourceUnavailableAccount = Boolean(sourceInspectedAccount && (0, _accountSnapshotFieldsDXlFJrtW.t)(sourceInspectedAccount) && (!(0, _accountSnapshotFieldsDXlFJrtW.n)(resolvedAccount) || sourceInspection?.configured === true && resolvedInspection?.configured === false));
      ...
      const enabled = selectedInspection?.enabled ?? (0, _accountSummaryBQJhTf0A.i)({
        plugin,
        account,
        cfg: effective
      });
      const configured = selectedInspection?.configured ?? (await (0, _accountSummaryBQJhTf0A.r)({
        plugin,
        account,
        cfg: effective,
        readAccountConfiguredField: true
      }));
      const snapshot = (0, _accountSummaryBQJhTf0A.t)({
        plugin,
        account,
        cfg: effective,
        accountId,
    ...
    const statusColor = status === "linked" || status === "configured" ? _themeD5sxSdHD.r.success : status === "not linked" ? _themeD5sxSdHD.r.error : _themeD5sxSdHD.r.muted;
    ...
    if (authAgeMs != null && authAgeMs >= 0) line += ` auth ${(0, _formatRelativeBErnCbgi.n)(authAgeMs)}`;
