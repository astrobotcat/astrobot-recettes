"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveProviderRequestPolicy;exports.n = resolveProviderEndpoint;exports.r = resolveProviderRequestCapabilities;exports.t = describeProviderRequestRoutingSummary;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function tryParseHostname(value) {
  try {
    return (0, _stringCoerceBUSzWgUA.o)(new URL(value).hostname);
  } catch {
    return;
  }
}
...
function normalizeComparableBaseUrl(value) {
  ...
  try {
    ...
    url.hash = "";
    url.search = "";
    return (0, _stringCoerceBUSzWgUA.o)(url.toString().replace(/\/+$/, ""));
  } catch {
    return;
  }
}
...
function resolveProviderAttributionIdentity(env = process.env) {
  return {
    ...
    version: (0, _versionBk5OWRN.s)(env)
  };
}
...
function resolveProviderRequestPolicy(input, env = process.env) {
  const provider = (0, _providerIdKaStHhRz.r)(input.provider ?? "");
  const policy = resolveProviderAttributionPolicy(provider, env);
  const endpointClass = resolveProviderEndpoint(input.baseUrl).endpointClass;
  const api = (0, _stringCoerceBUSzWgUA.o)(input.api);
  ...
  if (provider === "openai" && (api === "openai-completions" || api === "openai-responses" || input.capability === "audio" && api === "openai-audio-transcriptions") && usesOpenAIPublicAttributionHost) attributionProvider = "openai";else
  if (provider === "openai-codex" && (api === "openai-codex-responses" || api === "openai-responses") && usesOpenAICodexAttributionHost) attributionProvider = "openai-codex";else
...
function resolveProviderRequestCapabilities(input, env = process.env) {
  ...
  const api = (0, _stringCoerceBUSzWgUA.o)(input.api);
  const normalizedModelId = (0, _stringCoerceBUSzWgUA.o)(input.modelId);
  ...
  if (provider && MOONSHOT_COMPAT_PROVIDERS.has(provider)) compatibilityFamily = "moonshot";else
  if (provider === "ollama" && normalizedModelId?.startsWith("kimi-k") && normalizedModelId.includes(":cloud")) compatibilityFamily = "moonshot";
  return {
    ...policy,
    ...
    shouldStripResponsesPromptCache: input.compat?.supportsPromptCacheKey === true ? false : input.compat?.supportsPromptCacheKey === false ? api !== void 0 && OPENAI_RESPONSES_APIS.has(api) : api !== void 0 && OPENAI_RESPONSES_APIS.has(api) && policy.usesExplicitProxyLikeEndpoint,
...
function describeProviderRequestRoutingSummary(input, env = process.env) {
  const policy = resolveProviderRequestPolicy(input, env);
  const api = (0, _stringCoerceBUSzWgUA.o)(input.api) ?? "unknown";
