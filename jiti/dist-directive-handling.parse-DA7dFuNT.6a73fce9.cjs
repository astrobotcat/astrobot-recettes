"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = parseInlineDirectives;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function extractModelDirective(body, options) {
  ...
  const aliasMatch = modelMatch || aliases.length === 0 ? null : body.match(new RegExp(`(?:^|\\s)\\/(${aliases.map(_utilsD5DtWkEu.c).join("|")})(?=$|\\s|:)(?:\\s*:\\s*)?`, "i"));
  const match = modelMatch ?? aliasMatch;
  const raw = modelMatch ? modelMatch?.[1]?.trim() : aliasMatch?.[1]?.trim();
  let rawModel = raw;
  let rawProfile;
  if (raw) {
    const split = (0, _modelSelectionCli_3Gn8Wcd.h)(raw);
...
function parseExecDirectiveArgs(raw) {
  ...
  let i = (0, _directiveBJ5MwiY.i)(raw);
  let consumed = i;
  let execHost;
  let execSecurity;
  let execAsk;
  ...
  const takeToken = () => {
    const res = (0, _directiveBJ5MwiY.a)(raw, i);
    i = res.nextIndex;
    return res.token;
  };
  const splitToken = (token) => {
    ...
    const key = (0, _stringCoerceBUSzWgUA.o)(token.slice(0, idx));
  ...
  for (;;) {
    ...
    if (key === "host") {
      rawExecHost = value;
      execHost = (0, _execApprovalsDqhUuIS._)(value) ?? void 0;
    ...
    if (key === "node") {
      ...
      if (!trimmed) invalidNode = true;else
      execNode = trimmed;
      hasExecOptions = true;
      consumed = i;
      continue;
...
const matchLevelDirective = (body, names) => {
  const namePattern = names.map(_utilsD5DtWkEu.c).join("|");
...
function extractVerboseDirective(body) {
  ...
  const extracted = extractLevelDirective(body, ["verbose", "v"], _thinkingSharedCAbk7EZs.d);
  return {
    cleaned: extracted.cleaned,
...
function extractTraceDirective(body) {
  ...
  const extracted = extractLevelDirective(body, ["trace"], _thinkingSharedCAbk7EZs.l);
  return {
    cleaned: extracted.cleaned,
...
function extractFastDirective(body) {
  ...
  const extracted = extractLevelDirective(body, ["fast"], _thinkingSharedCAbk7EZs.o);
  return {
    cleaned: extracted.cleaned,
...
function extractElevatedDirective(body) {
  ...
  const extracted = extractLevelDirective(body, ["elevated", "elev"], _thinkingSharedCAbk7EZs.a);
  return {
    cleaned: extracted.cleaned,
...
function extractReasoningDirective(body) {
  ...
  const extracted = extractLevelDirective(body, ["reasoning", "reason"], _thinkingSharedCAbk7EZs.s);
  return {
    cleaned: extracted.cleaned,
...
function parseInlineDirectives(body, options) {
  ...
  const { cleaned: queueCleaned, queueMode, queueReset, rawMode, debounceMs, cap, dropPolicy, rawDebounce, rawCap, rawDrop, hasDirective: hasQueueDirective, hasOptions: hasQueueOptions } = (0, _directiveBJ5MwiY.t)(modelCleaned);
  return {
