"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = markdownToIRWithMeta;exports.i = markdownToIR;exports.n = renderMarkdownWithMarkers;exports.o = sliceMarkdownIR;exports.r = chunkMarkdownIR;exports.t = convertMarkdownTables;var _chunkC8HOq7ak = require("./chunk-C8HOq7ak.js");
  ...
  for (const token of tokens) switch (token.type) {
    ...
    case "table_close":
      if (state.table) {
        ...
        if (state.tableMode === "code") renderTableAsCode(state);else
        if (state.tableMode === "block") collectTableBlock(state);
      }
      state.table = null;
      break;
    case "thead_open":
...
function chunkMarkdownIR(ir, limit) {
  ...
  const chunks = (0, _chunkC8HOq7ak.a)(ir.text, limit);
  const results = [];
  let cursor = 0;
...
function renderMarkdownWithMarkers(ir, options) {
  ...
  for (const span of styled) {
    ...
    if (bucket) bucket.push(span);else
  ...
  if (options.buildLink) for (const link of ir.links) {
    ...
    if (openBucket) openBucket.push(rendered);else
