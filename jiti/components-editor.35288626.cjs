...
var _killRing = require("../kill-ring.js");
var _tui = require("../tui.js");
var _undoStack = require("../undo-stack.js");
var _utils = require("../utils.js");
var _selectList = require("./select-list.js");
const baseSegmenter = (0, _utils.getSegmenter)();
...
function wordWrapLine(line, maxWidth, preSegmented) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const isWs = !isPasteMarker(grapheme) && (0, _utils.isWhitespaceChar)(grapheme);
    ...
    if (gWidth > maxWidth) {
      ...
      currentWidth = (0, _utils.visibleWidth)(last.text);
      wrapOppIndex = -1;
      continue;
    }
    // Advance.
    ...
    if (isWs && next && (isPasteMarker(next.segment) || !(0, _utils.isWhitespaceChar)(next.segment))) {
...
class Editor {
  ...
  killRing = new _killRing.KillRing();
  lastAction = null;
  // Character jump mode
  jumpMode = null;
  ...
  // Undo support
  undoStack = new _undoStack.UndoStack();
  onSubmit;
  onChange;
  disableSubmit = false;
  ...
  invalidate() {
    ...
    if (this.scrollOffset > 0) {
      ...
      {
        result.push(this.borderColor((0, _utils.truncateToWidth)(indicator, width)));
      }
    } else
    {
  ...
  handleInput(data) {
    const kb = (0, _keybindings.getKeybindings)();
    ...
    if (kb.matches(data, "tui.editor.deleteCharBackward") || (0, _keys.matchesKey)(data, "shift+backspace")) {
      this.handleBackspace();
      return;
    }
    if (kb.matches(data, "tui.editor.deleteCharForward") || (0, _keys.matchesKey)(data, "shift+delete")) {
    ...
    const kittyPrintable = (0, _keys.decodeKittyPrintable)(data);
  ...
  shouldSubmitOnBackslashEnter(data, kb) {
    if (this.disableSubmit)
    return false;
    if (!(0, _keys.matchesKey)(data, "enter"))
    return false;
  ...
  moveWordForwards() {
    ...
    while (!next.done && !isPasteMarker(next.value.segment) && (0, _utils.isWhitespaceChar)(next.value.segment)) {
    ...
    if (!next.done) {
      ...
      if ((0, _utils.isPunctuationChar)(firstGrapheme)) {
        // Skip punctuation run
        while (!next.done && (0, _utils.isPunctuationChar)(next.value.segment) && !isPasteMarker(next.value.segment)) {
          newCol += next.value.segment.length;
          next = iterator.next();
        }
      } else
      {
        // Skip word run
        while (!next.done &&
        !(0, _utils.isWhitespaceChar)(next.value.segment) &&
        !(0, _utils.isPunctuationChar)(next.value.segment) &&
  ...
  createAutocompleteList(prefix, items) {
    ...
    return new _selectList.SelectList(items, this.autocompleteMaxVisible, this.theme.selectList, layout);
  }
...
}exports.Editor = Editor; /* v9-395f3fd9cf4d864a */
