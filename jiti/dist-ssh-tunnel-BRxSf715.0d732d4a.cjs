"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = startSshPortForward;exports.t = parseSshTarget;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
var _portsBQYvWPfn = require("./ports-BQYvWPfn.js");
...
async function canConnectLocal(port) {
  return await new Promise((resolve) => {
    const socket = _nodeNet.default.connect({
      host: "127.0.0.1",
      port
    });
    const done = (ok) => {
...
async function startSshPortForward(opts) {
  ...
  try {
    await (0, _portsBQYvWPfn.r)(localPort);
  } catch (err) {
    if ((0, _errorsD8p6rxH.s)(err) && err.code === "EADDRINUSE") localPort = await pickEphemeralPort();else
    throw err;
  }
  ...
  args.push("--", userHost);
  const stderr = [];
  const child = (0, _nodeChild_process.spawn)("/usr/bin/ssh", args, { stdio: [
    "ignore",
    "ignore",
    "pipe"]
  });
  ...
  try {
    ...
    throw new Error(`${(0, _errorsD8p6rxH.i)(err)}${suffix}`, { cause: err });
  }
  return {
    parsedTarget: parsed,
    localPort,
