"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = inspectPortUsage;exports.c = isDualStackLoopbackGatewayListeners;exports.i = handlePortError;exports.n = describePortOwner;exports.o = classifyPortListener;exports.r = ensurePortAvailable;exports.s = formatPortDiagnostics;exports.t = void 0;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
...
var _portsLsofBuPfzhlZ = require("./ports-lsof-BuPfzhlZ.js");
var _portsProbeBe3lexyD = require("./ports-probe-Be3lexyD.js");
//#region src/infra/ports-format.ts
function classifyPortListener(listener, port) {
  const raw = (0, _stringCoerceBUSzWgUA.i)(`${listener.commandLine ?? ""} ${listener.command ?? ""}`);
...
function parseListenerAddress(address) {
  ...
  if (bracketMatch) {
    ...
    return Number.isFinite(port) ? {
      host: (0, _stringCoerceBUSzWgUA.i)(bracketMatch[1]),
      port
    } : null;
  }
  const lastColon = normalized.lastIndexOf(":");
  if (lastColon <= 0 || lastColon >= normalized.length - 1) return null;
  const host = (0, _stringCoerceBUSzWgUA.i)(normalized.slice(0, lastColon));
...
function buildPortHints(listeners, port) {
  ...
  const hints = [];
  if (kinds.has("gateway")) hints.push(`Gateway already running locally. Stop it (${(0, _commandFormatDd3uP.t)("openclaw gateway stop")}) or use a different port.`);
...
async function runCommandSafe(argv, timeoutMs = 5e3) {
  try {
    const res = await (0, _execBAdwyfxI.r)(argv, { timeoutMs });
    return {
      stdout: res.stdout,
      stderr: res.stderr,
      code: res.code ?? 1
...
function parseLsofFieldOutput(output) {
  ...
  } else if (line.startsWith("c")) current.command = line.slice(1);else
...
function parseNetstatListeners(output, port) {
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (!line) continue;
    if (!(0, _stringCoerceBUSzWgUA.i)(line).includes("listen")) continue;
...
async function resolveWindowsImageName(pid) {
  ...
  for (const rawLine of res.stdout.split(/\r?\n/)) {
    const line = rawLine.trim();
    if (!(0, _stringCoerceBUSzWgUA.i)(line).startsWith("image name:")) continue;
...
async function resolveWindowsCommandLine(pid) {
  ...
  for (const rawLine of res.stdout.split(/\r?\n/)) {
    const line = rawLine.trim();
    if (!(0, _stringCoerceBUSzWgUA.i)(line).startsWith("commandline=")) continue;
...
async function tryListenOnHost(port, host) {
  try {
    ...
    if ((0, _errorsD8p6rxH.s)(err) && err.code === "EADDRINUSE") return "busy";
    if ((0, _errorsD8p6rxH.s)(err) && (err.code === "EADDRNOTAVAIL" || err.code === "EAFNOSUPPORT")) return "skip";
    return "unknown";
  }
}
...
async function ensurePortAvailable(port) {
  try {
    await (0, _portsProbeBe3lexyD.t)({ port });
  } catch (err) {
    if ((0, _errorsD8p6rxH.s)(err) && err.code === "EADDRINUSE") throw new PortInUseError(port);
    throw err;
  }
}
async function handlePortError(err, port, context, runtime = _runtimeDx7oeLYq.n) {
  if (err instanceof PortInUseError || (0, _errorsD8p6rxH.s)(err) && err.code === "EADDRINUSE") {
    const details = err instanceof PortInUseError ? err.details ?? (await describePortOwner(port)) : await describePortOwner(port);
    runtime.error((0, _globalsDe6QTwLG.t)(`${context} failed: port ${port} is already in use.`));
    if (details) {
      runtime.error((0, _globalsDe6QTwLG.n)("Port listener details:"));
      runtime.error(details);
      if (/openclaw|src\/index\.ts|dist\/index\.js/.test(details)) runtime.error((0, _globalsDe6QTwLG.s)("It looks like another OpenClaw instance is already running. Stop it or pick a different port."));
    }
    runtime.error((0, _globalsDe6QTwLG.n)("Resolve by stopping the process using the port or passing --port <free-port>."));
    runtime.exit(1);
  }
  runtime.error((0, _globalsDe6QTwLG.t)(`${context} failed: ${String(err)}`));
  if ((0, _globalsDe6QTwLG.a)()) {
    const stdout = err?.stdout;
    const stderr = err?.stderr;
    if (stdout?.trim()) (0, _loggerBA_TvTc.t)(`stdout: ${stdout.trim()}`);
    if (stderr?.trim()) (0, _loggerBA_TvTc.t)(`stderr: ${stderr.trim()}`);
  }
  runtime.exit(1);
  throw new Error("unreachable");
}
//#endregion /* v9-ea3392a56b148662 */
