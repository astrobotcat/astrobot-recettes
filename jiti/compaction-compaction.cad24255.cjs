 ...
 */
async function generateSummary(currentMessages, model, reserveTokens, apiKey, headers, signal, customInstructions, previousSummary) {
  ...
  const response = await (0, _piAi.completeSimple)(model, { systemPrompt: _utils.SUMMARIZATION_SYSTEM_PROMPT, messages: summarizationMessages }, completionOptions);
...
function prepareCompaction(pathEntries, settings) {
  ...
  const tokensBefore = estimateContextTokens((0, _sessionManager.buildSessionContext)(pathEntries).messages).tokens;
...
async function generateTurnPrefixSummary(messages, model, reserveTokens, apiKey, headers, signal) {
  ...
  const response = await (0, _piAi.completeSimple)(model, { systemPrompt: _utils.SUMMARIZATION_SYSTEM_PROMPT, messages: summarizationMessages }, { maxTokens, signal, apiKey, headers });
  if (response.stopReason === "error") {
