"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.S = shouldRefreshSnapshotForVersion;exports._ = verifyNodeToken;exports.a = recordRemoteNodeInfo;exports.b = registerSkillsChangeListener;exports.c = removeRemoteNodeInfo;exports.d = getPairedNode;exports.f = listNodePairing;exports.g = updatePairedNodeMetadata;exports.h = requestNodePairing;exports.i = primeRemoteSkillsCache;exports.l = setSkillsRemoteRegistry;exports.m = renamePairedNode;exports.n = resolveExecDefaults;exports.o = refreshRemoteBinsForConnectedNodes;exports.p = rejectNodePairing;exports.r = getRemoteSkillEligibility;exports.s = refreshRemoteNodeBins;exports.t = canExecRequestNode;exports.u = approveNodePairing;exports.v = bumpSkillsSnapshotVersion;exports.x = setSkillsChangeListenerErrorHandler;exports.y = getSkillsSnapshotVersion;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _nodePairingAuthzDTdV_SEj = require("./node-pairing-authz-DTdV_SEj.js");
var _skillsCwx5TftI = require("./skills-Cwx5TftI.js");
var _workspaceDirsBwQC27rz = require("./workspace-dirs-BwQC27rz.js");
var _pairingTokenBHGGVFL = require("./pairing-token-BHGGVFL2.js");
var _pairingPendingBUWAGw = require("./pairing-pending-BUWAGw-5.js");
...
const withLock = (0, _jsonFilesL0zR3LSb.t)();
function buildPendingNodePairingRequest(params) {
  return {
    requestId: params.requestId ?? (0, _nodeCrypto.randomUUID)(),
    ...
    caps: (0, _stringNormalizationXm3f27dv.t)(params.req.caps),
    commands: (0, _stringNormalizationXm3f27dv.t)(params.req.commands),
...
function refreshPendingNodePairingRequest(existing, incoming) {
  return {
    ...
    caps: (0, _stringNormalizationXm3f27dv.t)(incoming.caps) ?? existing.caps,
    commands: (0, _stringNormalizationXm3f27dv.t)(incoming.commands) ?? existing.commands,
...
function resolveNodeApprovalRequiredScopes(pending) {
  return (0, _nodePairingAuthzDTdV_SEj.t)(Array.isArray(pending.commands) ? pending.commands : []);
}
...
async function loadState(baseDir) {
  const { pendingPath, pairedPath } = (0, _pairingTokenBHGGVFL.a)(baseDir, "nodes");
  const [pending, paired] = await Promise.all([(0, _jsonFilesL0zR3LSb.n)(pendingPath), (0, _jsonFilesL0zR3LSb.n)(pairedPath)]);
  const state = {
    pendingById: pending ?? {},
    pairedByNodeId: paired ?? {}
  };
  (0, _pairingTokenBHGGVFL.r)(state.pendingById, Date.now(), PENDING_TTL_MS);
  return state;
}
async function persistState(state, baseDir) {
  const { pendingPath, pairedPath } = (0, _pairingTokenBHGGVFL.a)(baseDir, "nodes");
  await Promise.all([(0, _jsonFilesL0zR3LSb.r)(pendingPath, state.pendingById), (0, _jsonFilesL0zR3LSb.r)(pairedPath, state.pairedByNodeId)]);
}
function normalizeNodeId(nodeId) {
  return nodeId.trim();
}
function newToken() {
  return (0, _pairingTokenBHGGVFL.t)();
}
...
async function requestNodePairing(req, baseDir) {
  return await withLock(async () => {
    ...
    return await (0, _pairingTokenBHGGVFL.i)({
...
async function approveNodePairing(requestId, options, baseDir) {
  return await withLock(async () => {
    ...
    const missingScope = (0, _pairingTokenBHGGVFL.o)({
      role: OPERATOR_ROLE,
...
async function rejectNodePairing(requestId, baseDir) {
  return await withLock(async () => {
    return await (0, _pairingPendingBUWAGw.t)({
      requestId,
      idKey: "nodeId",
...
async function verifyNodeToken(nodeId, token, baseDir) {
  ...
  return (0, _pairingTokenBHGGVFL.n)(token, node.token) ? {
    ok: true,
    node
  } : { ok: false };
}
...
const log = (0, _subsystemCgmckbux.t)("gateway/skills-remote");
...
function isMacPlatform(platform, deviceFamily) {
  const platformNorm = (0, _stringCoerceBUSzWgUA.i)(platform);
  const familyNorm = (0, _stringCoerceBUSzWgUA.i)(deviceFamily);
...
function parseBinProbePayload(payloadJSON, payload) {
  ...
  try {
    ...
    if (Array.isArray(parsed.bins)) return parsed.bins.map((bin) => (0, _stringCoerceBUSzWgUA.s)(String(bin)) ?? "").filter(Boolean);
    if (typeof parsed.stdout === "string") return parsed.stdout.split(/\r?\n/).map((line) => (0, _stringCoerceBUSzWgUA.s)(line) ?? "").filter(Boolean);
  } catch {
    return [];
  }
  return [];
...
async function refreshRemoteNodeBins(params) {
  ...
  const workspaceDirs = (0, _workspaceDirsBwQC27rz.t)(params.cfg);
  const requiredBins = /* @__PURE__ */new Set();
  for (const workspaceDir of workspaceDirs) {
    const entries = (0, _skillsCwx5TftI.s)(workspaceDir, { config: params.cfg });
...
function resolveExecConfigState(params) {
  ...
  const resolvedAgentId = params.agentId ?? (0, _agentScopeKFH9bkHi.p)({
    sessionKey: params.sessionKey,
    config: cfg
  });
  const globalExec = cfg.tools?.exec;
  const agentExec = resolvedAgentId ? (0, _agentScopeKFH9bkHi._)(cfg, resolvedAgentId)?.tools?.exec : void 0;
  return {
    cfg,
...
function resolveExecSandboxAvailability(params) {
  return params.sandboxAvailable ?? (params.sessionKey ? (0, _runtimeStatusDhGewqgv.n)({
    cfg: params.cfg,
    sessionKey: params.sessionKey
  }).sandboxed : false);
}
function canExecRequestNode(params) {
  ...
  return (0, _bashToolsExecRuntimeDNTWYnIA.l)({
...
function resolveExecDefaults(params) {
  ...
  const resolved = (0, _bashToolsExecRuntimeDNTWYnIA.p)({
    configuredTarget: host,
    elevatedRequested: false,
    sandboxAvailable
  });
  const approvalDefaults = (0, _execApprovalsDqhUuIS.l)().defaults;
  ...
  return {
    ...
    canRequestNode: (0, _bashToolsExecRuntimeDNTWYnIA.l)({
      configuredTarget: host,
      requestedTarget: "node",
      sandboxAvailable
    })
