"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = createBoundaryAwareStreamFnForModel;exports.c = mergeTransportMetadata;exports.d = normalizeOpenAIReasoningEffort;exports.f = mapOpenAIReasoningEffortForModel;exports.h = resolveOpenAIStrictToolSetting;exports.i = buildTransportAwareSimpleStreamFn;exports.l = sanitizeTransportPayloadText;exports.m = resolveOpenAIStrictToolFlagForInventory;exports.n = registerProviderStreamForModel;exports.o = prepareTransportAwareSimpleModel;exports.p = normalizeOpenAIStrictToolParameters;exports.r = ensureCustomApiRegistered;exports.s = mergeTransportHeaders;exports.t = createAnthropicVertexStreamFnForModel;exports.u = buildGuardedModelFetch;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _piToolsSchemaD43D41q = require("./pi-tools.schema-D43D41q6.js");
var _runtimeCdS7ehWO = require("./runtime-CdS7ehWO.js");
var _openaiResponsesPayloadPolicyD48wiGIs = require("./openai-responses-payload-policy-D48wiGIs.js");
var _providerRequestConfigS_W3yUlE = require("./provider-request-config-S_W3yUlE.js");
var _systemPromptCacheBoundaryNUMoTji = require("./system-prompt-cache-boundary-NUMoTji1.js");
var _anthropicPayloadPolicyDKgsteMF = require("./anthropic-payload-policy-DKgsteMF.js");
var _copilotDynamicHeadersCYppRNbP = require("./copilot-dynamic-headers-CYppRNbP.js");
var _fetchGuardB3p4gGaY = require("./fetch-guard-B3p4gGaY.js");
var _geminiAuthCalhrTDk = require("./gemini-auth-CalhrTDk.js");
var _nodeCrypto = require("node:crypto");
var _piAi = require("@mariozechner/pi-ai");
var _openaiCompletions = require("@mariozechner/pi-ai/openai-completions");
var _openai = _interopRequireWildcard(require("openai"));
var _vertexSdk = require("@anthropic-ai/vertex-sdk");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
//#region src/agents/openai-tool-schema.ts
const optionalString = _stringCoerceBUSzWgUA.d;
function normalizeStrictOpenAIJsonSchema(schema) {
  return normalizeStrictOpenAIJsonSchemaRecursive((0, _piToolsSchemaD43D41q.t)(schema ?? {}));
}
...
function normalizeOpenAIStrictToolParameters(schema, strict) {
  if (!strict) return (0, _piToolsSchemaD43D41q.t)(schema ?? {});
...
function resolvesToNativeOpenAIStrictTools(model, transport) {
  const capabilities = (0, _providerAttributionBJhkfmfB.r)({
...
function resolveOpenAIReasoningEffortMap(model, fallbackMap = {}) {
  const provider = (0, _stringCoerceBUSzWgUA.i)(model.provider ?? "");
  const id = (0, _stringCoerceBUSzWgUA.i)(model.id ?? "");
...
function makeLogFn(fnLevel, logger, logLevel) {
  if (!logger || levelNumbers[fnLevel] > levelNumbers[logLevel]) return noop;else
...
var SSEDecoder = class {
  ...
  decode(line) {
    ...
    if (fieldname === "event") this.event = value;else
...
const addFormValue = async (form, key, value, stripFilenames) => {
  ...
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") form.append(key, String(value));else
  if (value instanceof Response) {
    let options = {};
    const contentType = value.headers.get("Content-Type");
    if (contentType) options = { type: contentType };
    form.append(key, makeFile([await value.blob()], getName(value, stripFilenames), options));
  } else if (isAsyncIterable(value)) form.append(key, makeFile([await new Response(ReadableStreamFrom(value)).blob()], getName(value, stripFilenames)));else
  if (isNamedBlob(value)) form.append(key, makeFile([value], getName(value, stripFilenames), { type: value.type }));else
  if (Array.isArray(value)) await Promise.all(value.map((entry) => addFormValue(form, key + "[]", entry, stripFilenames)));else
  if (typeof value === "object") await Promise.all(Object.entries(value).map(([name, prop]) => addFormValue(form, `${key}[${name}]`, prop, stripFilenames)));else
...
async function getBytes(value) {
  let parts = [];
  if (typeof value === "string" || ArrayBuffer.isView(value) || value instanceof ArrayBuffer) parts.push(value);else
  if (isBlobLike(value)) parts.push(value instanceof Blob ? value : await value.arrayBuffer());else
  if (isAsyncIterable(value)) for await (const chunk of value) parts.push(...(await getBytes(chunk)));else
  {
...
function* iterateHeaders(headers) {
  ...
  let iter;
  if (headers instanceof Headers) iter = headers.entries();else
  if (isReadonlyArray(headers)) iter = headers;else
  {
    shouldClear = true;
...
const tokenize = (input) => {
    ...
    let tail = [];
    tokens.map((token) => {
      if (token.type === "brace") if (token.value === "{") tail.push("}");else
      tail.splice(tail.lastIndexOf("}"), 1);
      if (token.type === "paren") if (token.value === "[") tail.push("]");else
      tail.splice(tail.lastIndexOf("]"), 1);
    });
...
var BetaToolRunner = class {
      ...
      while (true) {
        ...
        try {
          ...
          if (!(await __classPrivateFieldGet(this, _BetaToolRunner_instances, "m", _BetaToolRunner_checkAndCompact).call(this))) {
            ...
            if (toolMessage) __classPrivateFieldGet(this, _BetaToolRunner_state, "f").params.messages.push(toolMessage);else
            if (!__classPrivateFieldGet(this, _BetaToolRunner_mutated, "f")) break;
          }
        } finally {
  ...
  setMessagesParams(paramsOrMutator) {
    if (typeof paramsOrMutator === "function") __classPrivateFieldGet(this, _BetaToolRunner_state, "f").params = paramsOrMutator(__classPrivateFieldGet(this, _BetaToolRunner_state, "f").params);else
  ...
  */
  ...
  async retryRequest(options, retriesRemaining, requestLogID, responseHeaders) {
    ...
    if (retryAfterHeader && !timeoutMillis) {
      ...
      if (!Number.isNaN(timeoutSeconds)) timeoutMillis = timeoutSeconds * 1e3;else
  ...
  buildBody({ options: { body, headers: rawHeaders } }) {
    ...
    if (typeof body === "object" && (Symbol.asyncIterator in body || Symbol.iterator in body && "next" in body && typeof body.next === "function")) return {
      bodyHeaders: void 0,
      body: ReadableStreamFrom(body)
    };else
    if (typeof body === "object" && headers.values.get("content-type") === "application/x-www-form-urlencoded") return {
      bodyHeaders: { "content-type": "application/x-www-form-urlencoded" },
      body: this.stringifyQuery(body)
    };else
    return __classPrivateFieldGet(this, _BaseAnthropic_encoder, "f").call(this, {
      body,
      headers
    });
  }
...
function resolveModelRequestPolicy(model) {
  const debugProxy = (0, _runtimeCdS7ehWO.d)();
  ...
  const request = (0, _providerRequestConfigS_W3yUlE.a)((0, _providerRequestConfigS_W3yUlE.i)(model), { proxy: explicitDebugProxyUrl ? {
      mode: "explicit-proxy",
      url: explicitDebugProxyUrl
    } : void 0 });
  return (0, _providerRequestConfigS_W3yUlE.c)({
...
function buildGuardedModelFetch(model) {
  ...
  const dispatcherPolicy = (0, _providerRequestConfigS_W3yUlE.n)(requestConfig);
  return async (input, init) => {
    const request = input instanceof Request ? new Request(input, init) : void 0;
    const result = await (0, _fetchGuardB3p4gGaY.n)({
...
function createWritableTransportEventStream() {
  const eventStream = (0, _piAi.createAssistantMessageEventStream)();
  return {
    eventStream,
    stream: eventStream
  };
...
"WebFetch",
"WebSearch"].
map((tool) => [(0, _stringCoerceBUSzWgUA.i)(tool), tool]));
...
function toClaudeCodeName(name) {
  return CLAUDE_CODE_TOOL_LOOKUP.get((0, _stringCoerceBUSzWgUA.i)(name)) ?? name;
}
function fromClaudeCodeName(name, tools) {
  if (tools && tools.length > 0) {
    const lowerName = (0, _stringCoerceBUSzWgUA.i)(name);
    const matchedTool = tools.find((tool) => (0, _stringCoerceBUSzWgUA.i)(tool.name) === lowerName);
  ...
  if (model.provider === "github-copilot") {
    ...
    return {
      client: new Anthropic({
        ...
        }, model.headers, (0, _copilotDynamicHeadersCYppRNbP.t)({
          messages: context.messages,
          hasImages: (0, _copilotDynamicHeadersCYppRNbP.r)(context.messages)
        }), options?.headers),
        fetch
      }),
      isOAuthToken: false
...
function buildAnthropicParams(model, context, isOAuthToken, options) {
  ...
  const payloadPolicy = (0, _anthropicPayloadPolicyDKgsteMF.r)({
    provider: model.provider,
    api: model.api,
    baseUrl: model.baseUrl,
  ...
  if (context.systemPrompt) params.system = [{
    type: "text",
    text: sanitizeTransportPayloadText(context.systemPrompt)
  }];
  ...
  (0, _anthropicPayloadPolicyDKgsteMF.n)(params, payloadPolicy);
  return params;
}
...
function createAnthropicMessagesTransportStreamFn() {
  return (rawModel, context, rawOptions) => {
    ...
    (async () => {
      ...
      try {
        const apiKey = options?.apiKey ?? (0, _piAi.getEnvApiKey)(model.provider) ?? "";
        ...
        for await (const event of anthropicStream) {
          if (event.type === "message_start") {
            ...
            (0, _piAi.calculateCost)(model, output.usage);
            continue;
          }
          ...
          if (event.type === "content_block_delta") {
            ...
            if (block?.type === "toolCall" && delta?.type === "input_json_delta" && typeof delta.partial_json === "string") {
              ...
              block.arguments = (0, _piAi.parseStreamingJson)(block.partialJson);
              stream.push({
                type: "toolcall_delta",
                contentIndex: index,
          ...
          if (event.type === "content_block_stop") {
            ...
            if (block.type === "toolCall") {
              if (typeof block.partialJson === "string" && block.partialJson.length > 0) block.arguments = (0, _piAi.parseStreamingJson)(block.partialJson);
...
function isGemini3ProModel(modelId) {
  return /gemini-3(?:\.\d+)?-pro/.test((0, _stringCoerceBUSzWgUA.i)(modelId));
}
function isGemini3FlashModel(modelId) {
  return /gemini-3(?:\.\d+)?-flash/.test((0, _stringCoerceBUSzWgUA.i)(modelId));
}
function requiresToolCallId(modelId) {
  return modelId.startsWith("claude-") || modelId.startsWith("gpt-oss-");
}
function supportsMultimodalFunctionResponse(modelId) {
  const match = (0, _stringCoerceBUSzWgUA.i)(modelId).match(/^gemini(?:-live)?-(\d+)/);
...
function buildGoogleRequestUrl(model) {
  return `${(0, _io5pxHCi7V.J)(model.baseUrl)}/${resolveGoogleModelPath(model.id)}:streamGenerateContent?alt=sse`;
}
...
function resolveGoogleThinkingConfig(model, options) {
  ...
  if (options?.thinking) {
    ...
    if (options.thinking.level) config.thinkingLevel = options.thinking.level;else
    if (typeof options.thinking.budgetTokens === "number") config.thinkingBudget = options.thinking.budgetTokens;
    return config;
  }
...
function convertGoogleMessages(model, context) {
  ...
  for (const msg of transformedMessages) {
    ...
    if (msg.role === "toolResult") {
      ...
      if (last?.role === "user" && Array.isArray(last.parts) && last.parts.some((part) => "functionResponse" in part)) last.parts.push(functionResponse);else
      contents.push({
        role: "user",
...
function buildGoogleGenerativeAiParams(model, context, options) {
  ...
  if (context.systemPrompt) params.systemInstruction = { parts: [{ text: sanitizeTransportPayloadText((0, _systemPromptCacheBoundaryNUMoTji.i)(context.systemPrompt)) }] };
...
function buildGoogleHeaders(model, apiKey, optionHeaders) {
  return mergeTransportHeaders({ accept: "text/event-stream" }, apiKey ? (0, _geminiAuthCalhrTDk.t)(apiKey).headers : void 0, model.headers, optionHeaders) ?? { accept: "text/event-stream" };
}
...
function createGoogleGenerativeAiTransportStreamFn() {
  return (rawModel, context, rawOptions) => {
    ...
    (async () => {
      ...
      try {
        const apiKey = options?.apiKey ?? (0, _piAi.getEnvApiKey)(model.provider) ?? void 0;
        const fetch = buildGuardedModelFetch(model);
...
function convertResponsesMessages(model, context, allowedToolCallProviders, options) {
  ...
  if ((options?.includeSystemPrompt ?? true) && context.systemPrompt) messages.push({
    ...
    content: sanitizeTransportPayloadText((0, _systemPromptCacheBoundaryNUMoTji.i)(context.systemPrompt))
  });
  let msgIndex = 0;
  for (const msg of transformedMessages) {
...
async function processResponsesStream(openaiStream, output, stream, model, options) {
  ...
  for await (const rawEvent of openaiStream) {
    ...
    if (type === "response.created") output.responseId = stringifyUnknown(event.response?.id);else
    if (type === "response.output_item.added") {
      ...
      if (currentItem?.type === "function_call" && currentBlock?.type === "toolCall") {
        ...
        currentBlock.arguments = (0, _piAi.parseStreamingJson)(stringifyJsonLike(currentBlock.partialJson));
        stream.push({
          type: "toolcall_delta",
          contentIndex: blockIndex(),
      ...
      if (item.type === "reasoning" && currentBlock?.type === "thinking") {
        ...
        const args = currentBlock?.type === "toolCall" && currentBlock.partialJson ? (0, _piAi.parseStreamingJson)(stringifyJsonLike(currentBlock.partialJson, "{}")) : (0, _piAi.parseStreamingJson)(stringifyJsonLike(item.arguments, "{}"));
        stream.push({
          type: "toolcall_end",
          contentIndex: blockIndex(),
          toolCall: {
    ...
    } else if (type === "error") throw new Error(`Error Code ${stringifyUnknown(event.code, "unknown")}: ${stringifyUnknown(event.message, "Unknown error")}`);else
...
function buildOpenAIClientHeaders(model, context, optionHeaders, turnHeaders) {
  ...
  if (model.provider === "github-copilot") Object.assign(headers, (0, _copilotDynamicHeadersCYppRNbP.t)({
    messages: context.messages,
    hasImages: (0, _copilotDynamicHeadersCYppRNbP.r)(context.messages)
  }));
...
function resolveProviderTransportTurnState(model, params) {
  return (0, _providerRuntimeKhVgWetm.L)({
    provider: model.provider,
    context: {
      provider: model.provider,
      modelId: model.id,
    ...
    (async () => {
      ...
      try {
        ...
        const turnState = resolveProviderTransportTurnState(model, {
          sessionId: options?.sessionId,
          turnId: (0, _nodeCrypto.randomUUID)(),
          attempt: 1,
          transport: "stream"
        });
...
function buildOpenAIResponsesParams(model, context, options, metadata) {
  ...
  const payloadPolicy = (0, _openaiResponsesPayloadPolicyD48wiGIs.n)(model, { storeMode: "disable" });
  const params = {
    model: model.id,
    input: messages,
    stream: true,
  ...
  (0, _openaiResponsesPayloadPolicyD48wiGIs.t)(params, payloadPolicy);
  return params;
}
...
function createOpenAICompletionsTransportStreamFn() {
  return (model, context, options) => {
    ...
    (async () => {
      ...
      try {
        const client = createOpenAICompletionsClient(model, context, options?.apiKey || (0, _piAi.getEnvApiKey)(model.provider) || "", options?.headers);
...
async function processOpenAICompletionsStream(responseStream, output, model, stream) {
  ...
  const finishCurrentBlock = () => {
    ...
    if (currentBlock.type === "toolCall") {
      currentBlock.arguments = (0, _piAi.parseStreamingJson)(currentBlock.partialArgs);
      const completed = {
        ...currentBlock,
        arguments: (0, _piAi.parseStreamingJson)(currentBlock.partialArgs)
      };
  ...
  for await (const chunk of responseStream) {
    ...
    if (reasoningDelta) if (currentBlock?.type === "toolCall") {if (!pendingThinkingDelta) pendingThinkingDelta = { ...reasoningDelta };else
      pendingThinkingDelta.text += reasoningDelta.text;} else
...
function detectCompat(model) {
  ...
  const { capabilities, defaults: compatDefaults } = (0, _providerModelCompatDsxuyzi.s)(model);
...
function buildOpenAICompletionsParams(model, context, options) {
  const compat = getCompat(model);
  const messages = (0, _openaiCompletions.convertMessages)(model, context.systemPrompt ? {
    ...context,
    systemPrompt: (0, _systemPromptCacheBoundaryNUMoTji.i)(context.systemPrompt)
  } : context, compat);
  const params = {
    model: model.id,
    messages: compat.requiresStringContent ? (0, _openaiResponsesPayloadPolicyD48wiGIs.r)(messages) : messages,
    stream: true
  };
  if (compat.supportsUsageInStreaming) params.stream_options = { include_usage: true };
  if (compat.supportsStore) params.store = false;
  if (options?.maxTokens) if (compat.maxTokensField === "max_tokens") params.max_tokens = options.maxTokens;else
  ...
  if (compat.thinkingFormat === "openrouter" && model.reasoning && completionsReasoningEffort) params.reasoning = { effort: mapReasoningEffort(completionsReasoningEffort, compat.reasoningEffortMap) };else
  if (completionsReasoningEffort && model.reasoning && compat.supportsReasoningEffort) params.reasoning_effort = mapNativeOpenAIReasoningEffort(completionsReasoningEffort, compat.reasoningEffortMap);
  return params;
}
function parseTransportChunkUsage(rawUsage, model) {
  ...
  (0, _piAi.calculateCost)(model, usage);
  return usage;
}
...
function hasTransportOverrides(model) {
  const request = (0, _providerRequestConfigS_W3yUlE.i)(model);
...
function ensureCustomApiRegistered(api, streamFn) {
  if ((0, _piAi.getApiProvider)(api)) return false;
...
function registerProviderStreamForModel(params) {
  const streamFn = (0, _providerRuntimeKhVgWetm.N)({
...
function createAnthropicVertexOnPayload(params) {
  const policy = (0, _anthropicPayloadPolicyDKgsteMF.r)({
    provider: params.model.provider,
  ...
  function applyPolicy(payload) {
    if (payload && typeof payload === "object" && !Array.isArray(payload)) (0, _anthropicPayloadPolicyDKgsteMF.n)(payload, policy);
    return payload;
  }
...
function createAnthropicVertexStreamFn(projectId, region, baseURL) {
  const client = new _vertexSdk.AnthropicVertex({
    region,
    ...(baseURL ? { baseURL } : {}),
    ...(projectId ? { projectId } : {})
  });
  return (model, context, options) => {
    ...
    return (0, _piAi.streamAnthropic)(transportModel, context, opts);
  };
}
