"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = getReplyFromConfig;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
...
var _resetCkBotYFL = require("./reset-CkBotYFL.js");
var _sessionKeyDhT_3w6M = require("./session-key-DhT_3w6M.js");
...
require("./delivery-context-CToOfUqJ.js");
...
var _providerUtilsDNY7A_rU = require("./provider-utils-DNY7A_rU.js");
var _bootstrapCacheDunTQvFV = require("./bootstrap-cache-DunTQvFV.js");
var _runtimeStatusDhGewqgv = require("./runtime-status-DhGewqgv.js");
var _thinkingCtJu90cp = require("./thinking-CtJu90cp.js");
var _piBundleMcpToolsVusmAE = require("./pi-bundle-mcp-tools-vusm-AE2.js");
var _contentBlocksCoySVQBt = require("./content-blocks-CoySVQBt.js");
var _groupPolicyBtMLH9Qc = require("./group-policy-BtMLH9Qc.js");
...
var _sandboxInfoDFrJghWk = require("./sandbox-info-DFrJghWk.js");
...
var _inboundContext3c7B1YUQ = require("./inbound-context-3c7B1YUQ.js");
var _envelopeB1SOAhzt = require("./envelope-B1SOAhzt.js");
var _mentionsDXpYnKaK = require("./mentions-DXpYnKaK.js");
var _typingStartGuardClONjNhB = require("./typing-start-guard-ClONjNhB.js");
var _modelOverridesBdNgdLaq = require("./model-overrides-BdNgdLaq.js");
var _groupActivationDL8r3H_X = require("./group-activation-DL8r3H_X.js");
var _directiveHandlingDefaultsD4jJiOY = require("./directive-handling.defaults-D4j-jiOY.js");
var _fastModeBP5JceX = require("./fast-mode-BP5Jce-x.js");
var _blockStreamingYtBpcaVq = require("./block-streaming-YtBpcaVq.js");
var _commandAuthC4VasUjw = require("./command-auth-C4VasUjw.js");
var _commandsContextV7HiTv6d = require("./commands-context-v7HiTv6d.js");
var _directiveHandlingParseDA7dFuNT = require("./directive-handling.parse-DA7dFuNT.js");
var _directiveHandlingDirectiveOnlyOMYTDT = require("./directive-handling.directive-only-oMY-t-DT.js");
var _elevatedUnavailableDEH7uVT = require("./elevated-unavailable-DEH7uVT9.js");
var _sessionOverrideTDK5Oy = require("./session-override-TDK5Oy09.js");
var _storedModelOverrideDOM3YmmV = require("./stored-model-override-DOM3YmmV.js");
var _modelSelectionBUxj5f = require("./model-selection-BUxj5f-5.js");
var _skillCommandsBaseNbFzbjGB = require("./skill-commands-base-nbFzbjGB.js");
var _abortCutoffCLUBV9OI = require("./abort-cutoff-CLUBV9OI.js");
var _originRouting0ZGm8VdG = require("./origin-routing-0ZGm8VdG.js");
var _typingModeDcNmVk1f = require("./typing-mode-DcNmVk1f.js");
var _startupContextDVdfqRzB = require("./startup-context-DVdfqRzB.js");
var _sessionSystemEventsD8ngkWKO = require("./session-system-events-D8ngkWKO.js");
var _conversationBindingInputBOgGyfmJ = require("./conversation-binding-input-BOgGyfmJ.js");
var _sessionHooksZ9JJWO = require("./session-hooks-z9JJWO52.js");
...
function isNativeCommandSurface(surface) {
  const normalized = (0, _stringCoerceBUSzWgUA.o)(surface);
  ...
  if (!cachedNativeCommandSurfaces || cachedNativeCommandSurfacesVersion !== registryVersion || cachedNativeCommandSurfacesRegistry !== activeRegistry) {
    cachedNativeCommandSurfaces = new Set((0, _registryDelpa74L.r)().filter((plugin) => plugin.capabilities?.nativeCommands === true).map((plugin) => plugin.id));
...
function reserveSkillCommandNames(params) {
  for (const command of params.skillCommands) params.reservedCommands.add((0, _stringCoerceBUSzWgUA.i)(command.name));
}
function resolveConfiguredDirectiveAliases(params) {
  if (!params.commandTextHasSlash) return [];
  return Object.values(params.cfg.agents?.defaults?.models ?? {}).map((entry) => (0, _stringCoerceBUSzWgUA.s)(entry.alias)).filter((alias) => Boolean(alias)).filter((alias) => !params.reservedCommands.has((0, _stringCoerceBUSzWgUA.i)(alias)));
}
//#endregion
...
function loadCommandsStatus() {
  commandsStatusPromise ??= Promise.resolve().then(() => jitiImport("./commands-status.runtime-e5m21yqH.js").then((m) => _interopRequireWildcard(m)));
  return commandsStatusPromise;
}
function loadDirectiveLevels() {
  directiveLevelsPromise ??= Promise.resolve().then(() => jitiImport("./directive-handling.levels-DsfmfEXD.js").then((m) => _interopRequireWildcard(m)));
  return directiveLevelsPromise;
}
function loadDirectiveImpl() {
  directiveImplPromise ??= Promise.resolve().then(() => jitiImport("./directive-handling.impl-D6ShskBH.js").then((m) => _interopRequireWildcard(m)));
  return directiveImplPromise;
}
function loadDirectiveFastLane() {
  directiveFastLanePromise ??= Promise.resolve().then(() => jitiImport("./directive-handling.fast-lane-OncwBx2w.js").then((m) => _interopRequireWildcard(m)));
  return directiveFastLanePromise;
}
function loadDirectivePersist() {
  directivePersistPromise ??= Promise.resolve().then(() => jitiImport("./directive-handling.persist.runtime-XUaMaVxD.js").then((m) => _interopRequireWildcard(m)));
...
async function applyInlineDirectiveOverrides(params) {
  ...
  let directiveAck;
  if (modelState.resetModelOverride) (0, _systemEventsDq_M0n.i)(`Model override not allowed for this agent; reverted to ${initialModelLabel}.`, {
    sessionKey,
  ...
  if ((0, _directiveHandlingDirectiveOnlyOMYTDT.t)({
    directives,
...
function resolveFastSessionKey(params) {
  const { ctx } = params;
  const nativeCommandTarget = ctx.CommandSource === "native" ? (0, _stringCoerceBUSzWgUA.s)(ctx.CommandTargetSessionKey) : "";
  if (nativeCommandTarget) return nativeCommandTarget;
  return (0, _sessionKeyDhT_3w6M.n)(params.sessionScope, ctx, params.mainKey);
}
...
function resolveGetReplyConfig(params) {
  ...
  return (0, _schemaValidatorByqt0Q.i)(params.loadConfig(), configOverride);
}
...
function buildFastReplyCommandContext(params) {
  ...
  const surface = (0, _stringCoerceBUSzWgUA.o)(ctx.Surface ?? ctx.Provider) ?? "";
  const channel = (0, _stringCoerceBUSzWgUA.o)(ctx.Provider ?? surface) ?? "";
  const from = (0, _stringCoerceBUSzWgUA.s)(ctx.From);
  const to = (0, _stringCoerceBUSzWgUA.s)(ctx.To);
  return {
    surface,
    channel,
    channelId: (0, _registryCENZffQG.a)(channel) ?? (0, _registryCENZffQG.a)(surface) ?? void 0,
    ownerList: [],
    senderIsOwner: false,
    ...
    commandBodyNormalized: (0, _commandsRegistryNormalizeBy3bcvJy.r)(isGroup ? (0, _mentionsDXpYnKaK.o)(triggerBodyNormalized, ctx, cfg, agentId) : triggerBodyNormalized, { botUsername: ctx.BotUsername }),
    from,
    to
  };
}
...
function initFastReplySessionState(params) {
  ...
  const sessionStore = (0, _storeLoadDjLNEIy.t)(storePath, { skipCache: true });
  const existingEntry = sessionStore[sessionKey];
  const triggerBodyNormalized = (0, _mentionsDXpYnKaK.s)(ctx.BodyForCommands ?? ctx.CommandBody ?? ctx.RawBody ?? ctx.Body ?? "").trim();
  const normalizedChatType = (0, _chatTypeDFnPOWna.t)(ctx.ChatType);
  const isGroup = normalizedChatType != null && normalizedChatType !== "direct";
  const strippedForReset = isGroup ? (0, _mentionsDXpYnKaK.o)(triggerBodyNormalized, ctx, cfg, agentId) : triggerBodyNormalized;
  ...
  const sessionId = !resetTriggered && existingEntry ? existingEntry.sessionId : _nodeCrypto.default.randomUUID();
  const bodyStripped = resetTriggered ? strippedForReset.slice(resetMatch?.[0].length ?? 0).trimStart() : ctx.BodyForAgent ?? ctx.Body ?? "";
  const now = Date.now();
  const sessionFile = !resetTriggered && existingEntry?.sessionFile ? existingEntry.sessionFile : (0, _pathsCZMxg3hs.o)(sessionId, agentId);
  const sessionEntry = {
    ...
    ...((0, _stringCoerceBUSzWgUA.s)(ctx.Provider) ? { channel: (0, _stringCoerceBUSzWgUA.s)(ctx.Provider) } : {}),
    ...((0, _stringCoerceBUSzWgUA.s)(ctx.GroupSubject) ? { subject: (0, _stringCoerceBUSzWgUA.s)(ctx.GroupSubject) } : {}),
    ...((0, _stringCoerceBUSzWgUA.s)(ctx.GroupChannel) ? { groupChannel: (0, _stringCoerceBUSzWgUA.s)(ctx.GroupChannel) } : {})
  };
...
function loadGroupsRuntime() {
  groupsRuntimePromise ??= Promise.resolve().then(() => jitiImport("./groups.runtime-C07TR9i-.js").then((m) => _interopRequireWildcard(m)));
...
async function resolveGroupRequireMention(params) {
  ...
  const channel = await resolveRuntimeChannelId(groupResolution?.channel ?? (0, _stringCoerceBUSzWgUA.s)(ctx.Provider));
  if (!channel) return true;
  const rawGroupId = (ctx.From ?? "").trim();
  const groupId = groupResolution?.id ?? (0, _elevatedUnavailableDEH7uVT.n)(rawGroupId) ?? (rawGroupId || void 0);
  const groupChannel = (0, _stringCoerceBUSzWgUA.s)(ctx.GroupChannel) ?? (0, _stringCoerceBUSzWgUA.s)(ctx.GroupSubject);
  const groupSpace = (0, _stringCoerceBUSzWgUA.s)(ctx.GroupSpace);
  let requireMention;
  ...
  return (0, _groupPolicyBtMLH9Qc.n)({
    cfg,
    channel,
    groupId,
    accountId: ctx.AccountId
...
function resolveProviderLabel(rawProvider) {
  const providerKey = (0, _stringCoerceBUSzWgUA.o)(rawProvider) ?? "";
  if (!providerKey) return "chat";
  if ((0, _messageChannelCBqCPFa_.r)(providerKey)) return "WebChat";
  const label = {
    imessage: "iMessage",
    whatsapp: "WhatsApp"
  }[providerKey];
...
function buildGroupChatContext(params) {
  const subject = (0, _stringCoerceBUSzWgUA.s)(params.sessionCtx.GroupSubject);
  const members = (0, _stringCoerceBUSzWgUA.s)(params.sessionCtx.GroupMembers);
  const providerLabel = resolveProviderLabel(params.sessionCtx.Provider);
  const lines = [];
  if (subject) lines.push(`You are in the ${providerLabel} group chat "${subject}".`);else
...
function buildGroupIntro(params) {
  const activation = (0, _groupActivationDL8r3H_X.t)(params.sessionEntry?.groupActivation) ?? params.defaultActivation;
  return [
...
function parseExplicitElevatedAllowEntry(entry) {
  ...
  const fieldRaw = (0, _stringCoerceBUSzWgUA.i)(entry.slice(0, separatorIndex));
...
function slugAllowToken(value) {
  return (0, _stringNormalizationXm3f27dv.n)(value);
}
...
function resolveAllowFromFormatter(params) {
  const normalizedProvider = (0, _registryDelpa74L.i)(params.provider);
  const formatAllowFrom = normalizedProvider ? (0, _registryDelpa74L.t)(normalizedProvider)?.config?.formatAllowFrom : void 0;
  if (!formatAllowFrom) return (values) => (0, _stringNormalizationXm3f27dv.s)(values);
  return (values) => formatAllowFrom({
    cfg: params.cfg,
    accountId: params.accountId,
    allowFrom: values
  }).map((entry) => (0, _stringCoerceBUSzWgUA.s)(entry) ?? "").filter(Boolean);
}
function isApprovedElevatedSender(params) {
  ...
  const allowTokens = (0, _stringNormalizationXm3f27dv.s)(rawAllow);
  ...
  const senderId = (0, _stringCoerceBUSzWgUA.s)(params.ctx.SenderId);
  const senderFrom = (0, _stringCoerceBUSzWgUA.s)(params.ctx.From);
  const senderE164 = (0, _stringCoerceBUSzWgUA.s)(params.ctx.SenderE164);
...
function resolveElevatedPermissions(params) {
  ...
  const agentConfig = (0, _agentScopeKFH9bkHi._)(params.cfg, params.agentId)?.tools?.elevated;
  ...
  const normalizedProvider = (0, _registryDelpa74L.i)(params.provider);
  const fallbackAllowFrom = normalizedProvider ? (0, _registryDelpa74L.t)(normalizedProvider)?.elevated?.allowFromFallback?.({
    cfg: params.cfg,
...
function extractInlineSimpleCommand(body) {
  ...
  const alias = `/${(0, _stringCoerceBUSzWgUA.i)(match[1])}`;
...
function loadCommandsRegistry() {
  commandsRegistryPromise ??= Promise.resolve().then(() => jitiImport("./commands-registry.runtime-DVYDlpEA.js").then((m) => _interopRequireWildcard(m)));
  return commandsRegistryPromise;
}
function loadSkillCommands() {
  skillCommandsPromise ??= Promise.resolve().then(() => jitiImport("./skill-commands.runtime-3_wHYu8f.js").then((m) => _interopRequireWildcard(m)));
...
async function resolveReplyDirectives(params) {
  ...
  const agentEntry = (0, _agentScopeKFH9bkHi.h)(cfg).find((entry) => (0, _sessionKeyBh1lMwK.c)(entry.id) === (0, _sessionKeyBh1lMwK.c)(agentId));
  const targetSessionEntry = sessionStore[sessionKey] ?? sessionEntry;
  ...
  const command = (0, _commandsContextV7HiTv6d.t)({
    ctx,
    cfg,
    agentId,
    sessionKey,
  ...
  if (commandTextHasSlash) {
    ...
    for (const chatCommand of listChatCommands()) for (const alias of chatCommand.textAliases) reservedCommands.add((0, _stringCoerceBUSzWgUA.i)(alias.replace(/^\//, "")));
  }
  ...
  const configuredAliases = rawAliases.filter((alias) => !reservedCommands.has((0, _stringCoerceBUSzWgUA.i)(alias)));
  const allowStatusDirective = allowTextCommands && command.isAuthorizedSender;
  let parsedDirectives = (0, _directiveHandlingParseDA7dFuNT.t)(commandText, {
  ...
  if (parsedDirectives.hasThinkDirective || parsedDirectives.hasVerboseDirective || parsedDirectives.hasTraceDirective || parsedDirectives.hasFastDirective || parsedDirectives.hasReasoningDirective || parsedDirectives.hasElevatedDirective || parsedDirectives.hasExecDirective || parsedDirectives.hasModelDirective || parsedDirectives.hasQueueDirective) {
    const stripped = (0, _mentionsDXpYnKaK.s)(parsedDirectives.cleaned);
    const noMentions = isGroup ? (0, _mentionsDXpYnKaK.o)(stripped, ctx, cfg, agentId) : stripped;
    if (noMentions.trim().length > 0) {
      if ((0, _directiveHandlingParseDA7dFuNT.t)(noMentions, { modelAliases: configuredAliases }).cleaned.trim().length > 0) parsedDirectives = parsedDirectives.hasStatusDirective && allowTextCommands && command.isAuthorizedSender ? {
  ...
  let cleanedBody = (() => {
    ...
    if (!sessionCtx.CommandBody && !sessionCtx.RawBody) return (0, _directiveHandlingParseDA7dFuNT.t)(existingBody, {
      modelAliases: configuredAliases,
      allowStatusDirective
    }).cleaned;
    const markerIndex = existingBody.indexOf(_mentionsDXpYnKaK.t);
    if (markerIndex < 0) return (0, _directiveHandlingParseDA7dFuNT.t)(existingBody, {
      modelAliases: configuredAliases,
      allowStatusDirective
    }).cleaned;
    return `${existingBody.slice(0, markerIndex + _mentionsDXpYnKaK.t.length)}${(0, _directiveHandlingParseDA7dFuNT.t)(existingBody.slice(markerIndex + _mentionsDXpYnKaK.t.length), {
      modelAliases: configuredAliases,
      allowStatusDirective
    }).cleaned}`;
  })();
  ...
  const messageProviderKey = (0, _stringCoerceBUSzWgUA.s)(sessionCtx.Provider) ? (0, _stringCoerceBUSzWgUA.i)(sessionCtx.Provider) : (0, _stringCoerceBUSzWgUA.s)(ctx.Provider) ? (0, _stringCoerceBUSzWgUA.i)(ctx.Provider) : "";
  ...
  if (directives.hasElevatedDirective && (!elevatedEnabled || !elevatedAllowed)) {
    typing.cleanup();
    const runtimeSandboxed = (0, _runtimeStatusDhGewqgv.n)({
      cfg,
      sessionKey: ctx.SessionKey
    }).sandboxed;
    return {
      kind: "reply",
      reply: { text: (0, _elevatedUnavailableDEH7uVT.t)({
          runtimeSandboxed,
  ...
  const resolvedFastMode = directives.fastMode ?? (0, _fastModeBP5JceX.t)({
    cfg,
    provider,
    model,
    agentId,
  ...
  const blockReplyChunking = blockStreamingEnabled ? (0, _blockStreamingYtBpcaVq.n)(cfg, sessionCtx.Provider, sessionCtx.AccountId) : void 0;
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.OPENCLAW_TEST_FAST === "1"
  });
  const modelState = useFastReplyRuntime && !directives.hasModelDirective && !hasResolvedHeartbeatModelOverride && !(0, _stringCoerceBUSzWgUA.s)(targetSessionEntry?.modelOverride) && !(0, _stringCoerceBUSzWgUA.s)(targetSessionEntry?.providerOverride) ? (0, _modelSelectionBUxj5f.t)({
    agentCfg,
    provider,
    model
  }) : await (0, _modelSelectionBUxj5f.n)({
    cfg,
    agentId,
    agentCfg,
    sessionEntry: targetSessionEntry,
  ...
  let contextTokens = useFastReplyRuntime ? agentCfg?.contextTokens ?? 2e5 : (0, _modelSelectionBUxj5f.r)({
    cfg,
    agentCfg,
    provider,
    model
  ...
  const effectiveModelDirective = directives.hasModelDirective && ["status", "list"].includes((0, _stringCoerceBUSzWgUA.i)((0, _stringCoerceBUSzWgUA.s)(directives.rawModelDirective))) ? void 0 : directives.rawModelDirective;
...
function getBuiltinSlashCommands() {
  ...
  builtinSlashCommands = (0, _skillCommandsBaseNbFzbjGB.t)([
  "btw",
  "think",
  "verbose",
  "reasoning",
...
function resolveSlashCommandName(commandBodyNormalized) {
  ...
  const name = (0, _stringCoerceBUSzWgUA.o)(trimmed.match(/^\/([^\s:]+)(?::|\s|$)/)?.[1]) ?? "";
  return name ? name : null;
}
function expandBundleCommandPromptTemplate(template, args) {
  const normalizedArgs = (0, _stringCoerceBUSzWgUA.s)(args) || "";
...
function extractTextFromToolResult(result) {
  ...
  const trimmed = (0, _contentBlocksCoySVQBt.t)(content).join("").trim();
  return trimmed ? trimmed : null;
}
async function handleInlineActions(params) {
  ...
  const skillCommands = shouldLoadSkillCommands && params.skillCommands ? params.skillCommands : shouldLoadSkillCommands ? (await Promise.resolve().then(() => jitiImport("./skill-commands.runtime-3_wHYu8f.js").then((m) => _interopRequireWildcard(m)))).listSkillCommandsForWorkspace({
    workspaceDir,
    cfg,
    agentId,
    skillFilter
  }) : [];
  const skillInvocation = allowTextCommands && skillCommands.length > 0 ? (0, _skillCommandsBaseNbFzbjGB.n)({
    commandBodyNormalized: command.commandBodyNormalized,
    skillCommands
  }) : null;
  if (skillInvocation) {
    if (!command.isAuthorizedSender) {
      (0, _globalsDe6QTwLG.r)(`Ignoring /${skillInvocation.command.name} from unauthorized sender: ${command.senderId || "<unknown>"}`);
      typing.cleanup();
      return {
        kind: "reply",
        reply: void 0
    ...
    if (dispatch?.kind === "tool") {
      ...
      const channel = (0, _messageChannelCBqCPFa_.d)(ctx.Surface) ?? (0, _messageChannelCBqCPFa_.d)(ctx.Provider) ?? void 0;
      const { createOpenClawTools } = await Promise.resolve().then(() => jitiImport("./openclaw-tools.runtime-C3u1Zw03.js").then((m) => _interopRequireWildcard(m)));
      const tool = (0, _toolPolicyC3rJHw.n)(createOpenClawTools({
        agentSessionKey: sessionKey,
        agentChannel: channel,
        agentAccountId: ctx.AccountId,
        agentTo: ctx.OriginatingTo ?? ctx.To,
        agentThreadId: ctx.MessageThreadId ?? void 0,
        agentGroupId: (0, _elevatedUnavailableDEH7uVT.n)(ctx.From),
      ...
      const toolCallId = `cmd_${(0, _secureRandomDHoiZOqc.i)(8)}`;
      try {
        const toolArgs = {
          command: rawArgs,
  ...
  const isStopLikeInbound = (0, _abortPrimitivesBGJ_PnY.n)(command.rawBodyNormalized);
  const targetSessionEntry = sessionStore?.[sessionKey] ?? sessionEntry;
  if (!isStopLikeInbound && targetSessionEntry) {
    const cutoff = (0, _abortCutoffCLUBV9OI.r)(targetSessionEntry);
    const incoming = (0, _abortCutoffCLUBV9OI.i)(ctx);
    if (cutoff ? (0, _abortCutoffCLUBV9OI.o)({
    ...
    if (cutoff) await (await Promise.resolve().then(() => jitiImport("./abort-cutoff.runtime-Dgt-SEBN.js").then((m) => _interopRequireWildcard(m)))).clearAbortCutoffInSessionRuntime({
      sessionEntry: targetSessionEntry,
      sessionStore,
      sessionKey,
      storePath
  ...
  const handleInlineStatus = !(0, _directiveHandlingDirectiveOnlyOMYTDT.t)({
    directives,
    cleanedBody: directives.cleaned,
    ctx,
    cfg,
  ...
  if (handleInlineStatus) {
    const { buildStatusReply } = await Promise.resolve().then(() => jitiImport("./commands.runtime-BqzyrOiX.js").then((m) => _interopRequireWildcard(m)));
  ...
  const runCommands = async (commandInput) => {
    const { handleCommands } = await Promise.resolve().then(() => jitiImport("./commands.runtime-BqzyrOiX.js").then((m) => _interopRequireWildcard(m)));
  ...
  if ((command.channelId ? Boolean((0, _registryDelpa74L.t)(command.channelId)?.commands?.skipWhenConfigEmpty) : false) && isEmptyConfig && command.from && command.to && command.from !== command.to) {
    typing.cleanup();
    return {
      kind: "reply",
      reply: void 0
  ...
  if (!sessionEntry && command.abortKey) abortedLastRun = (0, _abortPrimitivesBGJ_PnY.t)(command.abortKey) ?? false;
  ...
  const remainingBodyAfterInlineStatus = (() => {
    ...
    return (0, _mentionsDXpYnKaK.o)(stripped, ctx, cfg, agentId).trim();
  })();
...
function loadSessionStoreRuntime$1() {
  sessionStoreRuntimePromise$1 ??= Promise.resolve().then(() => jitiImport("./store.runtime-Cz_llyv3.js").then((m) => _interopRequireWildcard(m)));
...
async function applySessionHints(params) {
  ...
  if (abortedHint) {
    ...
    } else if (params.abortKey) (0, _abortPrimitivesBGJ_PnY.i)(params.abortKey, false);
  }
...
async function resolvePreparedReplyQueueState(params) {
  ...
  if (params.queueMode === "interrupt") {
    ...
    (0, _globalsDe6QTwLG.r)(`Interrupting active run for ${params.sessionKey ?? params.sessionId} (aborted=${aborted})`);
  }
...
function truncateUntrustedJsonString(value) {
  ...
  return `${(0, _utilsD5DtWkEu.b)(value, Math.max(0, MAX_UNTRUSTED_JSON_STRING_CHARS - 14)).trimEnd()}…[truncated]`;
}
...
function formatConversationTimestamp(value, envelope) {
  ...
  return (0, _envelopeB1SOAhzt.n)(value, envelope);
}
...
function resolveInboundFormattingHints(ctx) {
  ...
  return (0, _registryLoadedC109837J.t)((0, _registryCENZffQG.a)(channelValue) ?? channelValue)?.agentPrompt?.inboundFormattingHints?.({ accountId: normalizePromptMetadataString(ctx.AccountId) ?? void 0 });
}
function buildInboundMetaSystemPrompt(ctx, options) {
  const chatType = (0, _chatTypeDFnPOWna.t)(ctx.ChatType);
...
function buildInboundUserContextPrefix(ctx, envelope) {
  ...
  const conversationInfo = {
    chat_id: shouldIncludeConversationInfo ? (0, _stringCoerceBUSzWgUA.s)(ctx.OriginatingTo) : void 0,
...
function isAudioPath(path) {
  ...
  const lower = (0, _stringCoerceBUSzWgUA.i)(path);
...
function buildInboundMediaNote(ctx) {
  ...
  const entries = paths.map((entry, index) => ({
    ...
    const isAudioByMime = types !== void 0 && (0, _stringCoerceBUSzWgUA.i)(entry.type).startsWith("audio/");
...
function appendUntrustedContext(base, untrusted) {
  ...
  const entries = untrusted.map((entry) => (0, _inboundTextMk2PQPNo.t)(entry)).filter((entry) => Boolean(entry));
...
function loadPiEmbeddedRuntime() {
  piEmbeddedRuntimePromise ??= Promise.resolve().then(() => jitiImport("./pi-embedded.runtime-d2CK3OpT.js").then((m) => _interopRequireWildcard(m)));
  return piEmbeddedRuntimePromise;
}
function loadAgentRunnerRuntime() {
  agentRunnerRuntimePromise ??= Promise.resolve().then(() => jitiImport("./agent-runner.runtime-CTlghBhJ.js").then((m) => _interopRequireWildcard(m)));
  return agentRunnerRuntimePromise;
}
function loadSessionUpdatesRuntime() {
  sessionUpdatesRuntimePromise ??= Promise.resolve().then(() => jitiImport("./session-updates.runtime-Dq0XJhdf.js").then((m) => _interopRequireWildcard(m)));
...
async function runPreparedReply(params) {
  ...
  const fullAccessState = (0, _sandboxInfoDFrJghWk.n)({ execElevated: {
      enabled: elevatedEnabled,
      allowed: elevatedAllowed,
  ...
  const { typingPolicy, suppressTyping } = (0, _typingPolicyCKG7EO6n.t)({
    requestedPolicy: opts?.typingPolicy,
    suppressTyping: opts?.suppressTyping === true,
    isHeartbeat,
    originatingChannel: ctx.OriginatingChannel
  });
  const typingMode = (0, _typingModeDcNmVk1f.n)({
  ...
  const groupIntro = shouldInjectGroupIntro ? buildGroupIntro({
    ...
    sessionEntry,
    defaultActivation,
    silentToken: _tokensCKM4Lddu.n
  }) : "";
  const groupSystemPrompt = (0, _stringCoerceBUSzWgUA.s)(sessionCtx.GroupSystemPrompt) ?? "";
  ...
  if (allowTextCommands && (!commandAuthorized || !command.isAuthorizedSender) && isWholeMessageCommand && ((0, _commandDetectionB01vikFY.t)(rawBodyTrimmed, cfg) || isResetOrNewCommand)) {
    typing.cleanup();
    return;
  }
  ...
  const startupContextPrelude = isBareSessionReset && (0, _startupContextDVdfqRzB.n)({
    cfg,
    action: startupAction
  }) ? await (0, _startupContextDVdfqRzB.t)({
    workspaceDir,
    cfg
  }) : null;
  const baseBodyFinal = isBareSessionReset ? (0, _startupContextDVdfqRzB.r)(cfg) : stripPromptThinkingDirectives(baseBody);
  const envelopeOptions = (0, _envelopeB1SOAhzt.a)(cfg);
  const inboundUserContext = buildInboundUserContextPrefix(isNewSession ? {
    ...sessionCtx,
    ...((0, _stringCoerceBUSzWgUA.s)(sessionCtx.ThreadHistoryBody) ? {
  ...
  if (!hasUserBody && !hasMediaAttachment) {
    ...
    (0, _globalsDe6QTwLG.r)("Inbound body empty after normalization; skipping agent run");
    typing.cleanup();
  ...
  const isMainSession = !(sessionEntry?.chatType === "group" || sessionEntry?.chatType === "channel") && sessionKey === (0, _sessionKeyBh1lMwK.l)(sessionCfg?.mainKey);
  if (!resolvedThinkLevel && prefixedBodyBase) {
    const parts = prefixedBodyBase.split(/\s+/);
    const maybeLevel = (0, _thinkingSharedCAbk7EZs.c)(parts[0]);
    if (maybeLevel && (maybeLevel !== "xhigh" || (0, _thinkingCtJu90cp.i)(provider, model))) {
      resolvedThinkLevel = maybeLevel;
      prefixedBodyBase = parts.slice(1).join(" ").trim();
    }
  }
  const prefixedBodyCore = prefixedBodyBase;
  const threadStarterBody = (0, _stringCoerceBUSzWgUA.s)(ctx.ThreadStarterBody);
  const threadHistoryBody = (0, _stringCoerceBUSzWgUA.s)(ctx.ThreadHistoryBody);
  ...
  const rebuildPromptBodies = async () => {
    if (!useFastReplyRuntime) {
      const eventsBlock = await (0, _sessionSystemEventsD8ngkWKO.t)({
        cfg,
        sessionKey,
        isMainSession,
        isNewSession
  ...
  if (resolvedThinkLevel === "xhigh" && !(0, _thinkingCtJu90cp.i)(provider, model)) {
    if (directives.hasThinkDirective && directives.thinkLevel !== void 0) {
      typing.cleanup();
      return { text: `Thinking level "xhigh" is only supported for ${(0, _thinkingSharedCAbk7EZs.n)()}. Use /think high or switch to one of those models.` };
    }
  ...
  const sessionFilePathOptions = (0, _pathsCZMxg3hs.a)({
    agentId,
    storePath
  });
  const resolvePreparedSessionState = () => {
    const latestSessionEntry = sessionStore && sessionKey ? (0, _storeDFXcceZJ.a)({
      store: sessionStore,
      sessionKey
    ...
    return {
      ...
      sessionFile: (0, _pathsCZMxg3hs.i)(latestSessionId, latestSessionEntry, sessionFilePathOptions)
    };
  };
  ...
  const laneSize = sessionLaneKey ? (0, _commandQueueBLDIg9zn.o)(sessionLaneKey) : 0;
  if (resolvedQueue.mode === "interrupt" && sessionLaneKey && laneSize > 0) {
    const cleared = (0, _commandQueueBLDIg9zn.r)(sessionLaneKey);
    ...
    (0, _globalsDe6QTwLG.r)(`Interrupting ${sessionLaneKey} (cleared ${cleared}, aborted=${aborted})`);
  }
  let authProfileId = useFastReplyRuntime ? void 0 : await (0, _sessionOverrideTDK5Oy.n)({
    cfg,
    provider,
    agentDir,
  ...
  const activeRunQueueAction = (0, _typingModeDcNmVk1f.r)({
    isActive,
  ...
  if (isActive && activeRunQueueAction === "run-now") {
    const queueState = await resolvePreparedReplyQueueState({
      ...
      refreshPreparedState: async () => {
        preparedSessionState = resolvePreparedSessionState();
        authProfileId = useFastReplyRuntime ? void 0 : await (0, _sessionOverrideTDK5Oy.n)({
          cfg,
          provider,
          agentDir,
          sessionEntry: preparedSessionState.sessionEntry,
  ...
  const followupRun = {
    ...
    run: {
      ...
      sessionKey,
      messageProvider: (0, _originRouting0ZGm8VdG.n)({
        originatingChannel: ctx.OriginatingChannel ?? sessionCtx.OriginatingChannel,
        provider: ctx.Provider ?? ctx.Surface ?? sessionCtx.Provider
      }),
      agentAccountId: sessionCtx.AccountId,
      groupId: (0, _storeDFXcceZJ.S)(sessionCtx)?.id ?? void 0,
      groupChannel: (0, _stringCoerceBUSzWgUA.s)(sessionCtx.GroupChannel) ?? (0, _stringCoerceBUSzWgUA.s)(sessionCtx.GroupSubject),
      groupSpace: (0, _stringCoerceBUSzWgUA.s)(sessionCtx.GroupSpace),
      senderId: (0, _stringCoerceBUSzWgUA.s)(sessionCtx.SenderId),
      senderName: (0, _stringCoerceBUSzWgUA.s)(sessionCtx.SenderName),
      senderUsername: (0, _stringCoerceBUSzWgUA.s)(sessionCtx.SenderUsername),
      senderE164: (0, _stringCoerceBUSzWgUA.s)(sessionCtx.SenderE164),
      ...
      fastMode: useFastReplyRuntime ? false : (0, _fastModeBP5JceX.t)({
        cfg,
        provider,
        model,
        agentId,
      ...
      ...(!useFastReplyRuntime && (0, _providerUtilsDNY7A_rU.t)(provider, {
        config: cfg,
        workspaceDir,
        modelId: model
...
function emitPreAgentMessageHooks(params) {
  ...
  const sessionKey = (0, _stringCoerceBUSzWgUA.s)(params.ctx.SessionKey);
  if (!sessionKey) return;
  const canonical = (0, _messageHookMappersV58z7z2I.n)(params.ctx);
  if (canonical.transcript) (0, _messageHookMappersV58z7z2I.f)((0, _internalHooks4i4Rq3Qq.m)((0, _internalHooks4i4Rq3Qq.n)("message", "transcribed", sessionKey, (0, _messageHookMappersV58z7z2I.o)(canonical, params.cfg))), "get-reply: message:transcribed internal hook failed");
  (0, _messageHookMappersV58z7z2I.f)((0, _internalHooks4i4Rq3Qq.m)((0, _internalHooks4i4Rq3Qq.n)("message", "preprocessed", sessionKey, (0, _messageHookMappersV58z7z2I.r)(canonical, params.cfg))), "get-reply: message:preprocessed internal hook failed");
}
//#endregion
//#region src/infra/session-maintenance-warning.ts
const warnedContexts = /* @__PURE__ */new Map();
const log$1 = (0, _subsystemCgmckbux.t)("session-maintenance-warning");
...
function resolveWarningDeliveryTarget(entry) {
  const context = (0, _deliveryContextSharedEClQPjt.t)(entry);
  const channel = context?.channel ? (0, _messageChannelCBqCPFa_.u)(context.channel) ?? context.channel : void 0;
  return {
    channel: channel && (0, _messageChannelCBqCPFa_.s)(channel) ? channel : void 0,
    to: context?.to,
...
async function deliverSessionMaintenanceWarning(params) {
  ...
  if (!target.channel || !target.to) {
    (0, _systemEventsDq_M0n.i)(text, { sessionKey: params.sessionKey });
    return;
  }
  const channel = (0, _messageChannelCBqCPFa_.u)(target.channel) ?? target.channel;
  if (!(0, _messageChannelCBqCPFa_.s)(channel)) {
    (0, _systemEventsDq_M0n.i)(text, { sessionKey: params.sessionKey });
    return;
  }
  try {
...
function resolveSessionKeyChannelHint(sessionKey) {
  ...
  const head = (0, _stringCoerceBUSzWgUA.o)(parsed.rest.split(":")[0]);
  if (!head || head === "main" || head === "cron" || head === "subagent" || head === "acp") return;
  return (0, _messageChannelCBqCPFa_.u)(head);
}
function isMainSessionKey(sessionKey) {
  const parsed = (0, _sessionKeyBh1lMwK.x)(sessionKey);
  if (!parsed) return (0, _stringCoerceBUSzWgUA.i)(sessionKey) === "main";
  return (0, _stringCoerceBUSzWgUA.i)(parsed.rest) === "main";
}
...
function hasStrictDirectSessionTail(parts, markerIndex) {
  if (!(0, _stringCoerceBUSzWgUA.s)(parts[markerIndex + 1])) return false;
  const tail = parts.slice(markerIndex + 2);
  if (tail.length === 0) return true;
  return tail.length === 2 && THREAD_SESSION_MARKERS.has(tail[0] ?? "") && Boolean((0, _stringCoerceBUSzWgUA.s)(tail[1]));
}
function isDirectSessionKey(sessionKey) {
  const raw = (0, _stringCoerceBUSzWgUA.i)(sessionKey);
  if (!raw) return false;
  const parts = ((0, _sessionKeyBh1lMwK.x)(raw)?.rest ?? raw).split(":").filter(Boolean);
  if (parts.length < 2) return false;
  if (DIRECT_SESSION_MARKERS.has(parts[0] ?? "")) return hasStrictDirectSessionTail(parts, 0);
  const channel = (0, _messageChannelCBqCPFa_.u)(parts[0]);
  if (!channel || !(0, _messageChannelCBqCPFa_.s)(channel)) return false;
  if (DIRECT_SESSION_MARKERS.has(parts[1] ?? "")) return hasStrictDirectSessionTail(parts, 1);
  return Boolean((0, _stringCoerceBUSzWgUA.s)(parts[1])) && DIRECT_SESSION_MARKERS.has(parts[2] ?? "") ? hasStrictDirectSessionTail(parts, 2) : false;
}
function isExternalRoutingChannel(channel) {
  return Boolean(channel && channel !== "webchat" && (0, _messageChannelCBqCPFa_.s)(channel));
}
function resolveLastChannelRaw(params) {
  const originatingChannel = (0, _messageChannelCBqCPFa_.u)(params.originatingChannelRaw);
  const persistedChannel = (0, _messageChannelCBqCPFa_.u)(params.persistedLastChannel);
  ...
  if (!isExternalRoutingChannel(originatingChannel)) {
    if (isExternalRoutingChannel(persistedChannel)) resolved = persistedChannel;else
    if (isExternalRoutingChannel(sessionKeyChannelHint)) resolved = sessionKeyChannelHint;
  }
  return resolved;
}
...
function maybeRetireLegacyMainDeliveryRoute(params) {
  ...
  const canonicalMainSessionKey = (0, _sessionKeyBh1lMwK.r)({
  ...
  const legacyRouteKey = (0, _deliveryContextSharedEClQPjt.n)((0, _deliveryContextSharedEClQPjt.t)(legacyMain));
  if (!legacyRouteKey) return;
  const activeDirectRouteKey = (0, _deliveryContextSharedEClQPjt.n)((0, _deliveryContextSharedEClQPjt.i)({
...
async function forkSessionFromParent(params) {
  return (await Promise.resolve().then(() => jitiImport("./session-fork.runtime-wc_Kzeog.js").then((m) => _interopRequireWildcard(m)))).forkSessionFromParentRuntime(params);
}
//#endregion
//#region src/auto-reply/reply/session.ts
const log = (0, _subsystemCgmckbux.t)("session-init");
...
function resolveSessionDefaultAccountId(params) {
  const explicit = (0, _stringCoerceBUSzWgUA.s)(params.accountIdRaw);
  if (explicit) return explicit;
  const persisted = (0, _stringCoerceBUSzWgUA.s)(params.persistedLastAccountId);
  if (persisted) return persisted;
  const channel = (0, _stringCoerceBUSzWgUA.o)(params.channelRaw);
  if (!channel) return;
  const configuredDefault = params.cfg.channels?.[channel]?.defaultAccount;
  return (0, _stringCoerceBUSzWgUA.s)(configuredDefault);
}
...
function isResetAuthorizedForContext(params) {
  const auth = (0, _commandAuthC4VasUjw.t)(params);
  if (!params.commandAuthorized && !auth.isAuthorizedSender) return false;
  const provider = params.ctx.Provider;
  if (!(provider ? (0, _messageChannelCBqCPFa_.r)(provider) : (0, _messageChannelCBqCPFa_.r)(params.ctx.Surface))) return true;
  const scopes = params.ctx.GatewayClientScopes;
  if (!Array.isArray(scopes) || scopes.length === 0) return true;
  return scopes.includes("operator.admin");
}
function resolveSessionConversationBindingContext(cfg, ctx) {
  const bindingContext = (0, _conversationBindingInputBOgGyfmJ.i)({
    cfg,
    ctx
  });
...
function resolveBoundConversationSessionKey(params) {
  ...
  const binding = (0, _sessionBindingServiceCP3mZirT.r)().resolveByConversation({
    channel: bindingContext.channel,
    accountId: bindingContext.accountId,
    conversationId: bindingContext.conversationId,
    ...(bindingContext.parentConversationId ? { parentConversationId: bindingContext.parentConversationId } : {})
  ...
  (0, _sessionBindingServiceCP3mZirT.r)().touch(binding.bindingId);
...
async function initSessionState(params) {
  ...
  const targetSessionKey = (ctx.CommandSource === "native" ? (0, _stringCoerceBUSzWgUA.s)(ctx.CommandTargetSessionKey) : void 0) ?? resolveBoundConversationSessionKey({
    cfg,
    ctx,
  ...
  const maintenanceConfig = (0, _storeDFXcceZJ.m)(sessionCfg?.maintenance);
  const mainKey = (0, _sessionKeyBh1lMwK.l)(sessionCfg?.mainKey);
  const agentId = (0, _agentScopeKFH9bkHi.p)({
    sessionKey: sessionCtxForState.SessionKey,
    config: cfg
  });
  const groupResolution = (0, _storeDFXcceZJ.S)(sessionCtxForState) ?? void 0;
  const resetTriggers = sessionCfg?.resetTriggers?.length ? sessionCfg.resetTriggers : _typesKCfeTvaK.t;
  const parentForkMaxTokens = resolveParentForkMaxTokens(cfg);
  const sessionScope = sessionCfg?.scope ?? "per-sender";
  const storePath = (0, _pathsCZMxg3hs.u)(sessionCfg?.store, { agentId });
  ...
  const triggerBodyNormalized = (0, _mentionsDXpYnKaK.s)(commandSource).trim();
  ...
  const strippedForReset = isGroup ? (0, _mentionsDXpYnKaK.o)(triggerBodyNormalized, ctx, cfg, agentId) : triggerBodyNormalized;
  const trimmedBodyLower = (0, _stringCoerceBUSzWgUA.i)(trimmedBody);
  const strippedForResetLower = (0, _stringCoerceBUSzWgUA.i)(strippedForReset);
  let matchedResetTriggerLower;
  for (const trigger of resetTriggers) {
    if (!trigger) continue;
    if (!resetAuthorized) break;
    const triggerLower = (0, _stringCoerceBUSzWgUA.i)(trigger);
  ...
  sessionKey = (0, _mainSessionDtefsIzj.t)({
    cfg,
    agentId,
    sessionKey: (0, _sessionKeyDhT_3w6M.n)(sessionScope, sessionCtxForState, mainKey)
  });
  ...
  const now = Date.now();
  const isThread = (0, _resetCkBotYFL.i)({
    sessionKey,
  ...
  const resetPolicy = (0, _resetCkBotYFL.c)({
    sessionCfg,
    resetType: (0, _resetCkBotYFL.r)({
      sessionKey,
      isGroup,
      isThread
    }),
    resetOverride: (0, _resetCkBotYFL.n)({
      sessionCfg,
  ...
  const entryFreshness = entry ? isSystemEvent ? { fresh: true } : (0, _resetCkBotYFL.o)({
  ...
  const isInterSession = (0, _inputProvenanceBIw3ISWS.i)(ctx.InputProvenance);
  ...
  const deliveryFields = isSystemEvent ? (0, _deliveryContextSharedEClQPjt.a)({
  ...
  }) : (0, _deliveryContextSharedEClQPjt.a)({ deliveryContext: {
  ...
  const metaPatch = (0, _storeDFXcceZJ.v)({
  ...
  const parentSessionKey = (0, _stringCoerceBUSzWgUA.s)(ctx.ParentSessionKey);
  ...
  const threadIdFromSessionKey = (0, _threadInfoDA0w7q1W.t)(sessionCtxForState.SessionKey ?? sessionKey).threadId;
  const fallbackSessionFile = !sessionEntry.sessionFile ? (0, _pathsCZMxg3hs.o)(sessionEntry.sessionId, agentId, ctx.MessageThreadId ?? threadIdFromSessionKey) : void 0;
  sessionEntry = (await (0, _sessionFileB_37cdL.t)({
  ...
  const sessionCtx = {
    ...sessionCtxForState,
    BodyStripped: (0, _inboundTextMk2PQPNo.t)(bodyStripped ?? sessionCtxForState.BodyForAgent ?? sessionCtxForState.Body ?? sessionCtxForState.CommandBody ?? sessionCtxForState.RawBody ?? sessionCtxForState.BodyForCommands ?? ""),
    SessionId: sessionId,
  ...
  if (hookRunner && isNewSession) {
    ...
    if (previousSessionEntry?.sessionId && previousSessionEntry.sessionId !== effectiveSessionId) {
      if (hookRunner.hasHooks("session_end")) {
        const payload = (0, _sessionHooksZ9JJWO.t)({
          sessionId: previousSessionEntry.sessionId,
          sessionKey,
          cfg,
    ...
    if (hookRunner.hasHooks("session_start")) {
      const payload = (0, _sessionHooksZ9JJWO.n)({
...
function createTypingController(params) {
  const { onReplyStart, onCleanup, typingIntervalSeconds = 6, typingTtlMs = 2 * 6e4, silentToken = _tokensCKM4Lddu.n, log } = params;
  ...
  const startGuard = (0, _typingStartGuardClONjNhB.t)({
    isSealed: () => sealed,
  ...
  const typingLoop = (0, _typingStartGuardClONjNhB.n)({
  ...
  const startTypingOnText = async (text) => {
    ...
    if (!trimmed) return;
    if (silentToken && ((0, _tokensCKM4Lddu.a)(trimmed, silentToken) || (0, _tokensCKM4Lddu.i)(trimmed, silentToken))) return;
    refreshTypingTtl();
...
function loadSessionResetModelRuntime() {
  sessionResetModelRuntimePromise ??= Promise.resolve().then(() => jitiImport("./session-reset-model.runtime-ChtZkJ0T.js").then((m) => _interopRequireWildcard(m)));
  return sessionResetModelRuntimePromise;
}
function loadStageSandboxMediaRuntime() {
  stageSandboxMediaRuntimePromise ??= Promise.resolve().then(() => jitiImport("./stage-sandbox-media.runtime-DURIkzFY.js").then((m) => _interopRequireWildcard(m)));
...
function loadHookRunnerGlobal() {
  hookRunnerGlobalPromise ??= Promise.resolve().then(() => jitiImport("./hook-runner-global-BdOpIhxP.js").then((m) => _interopRequireWildcard(m)));
  return hookRunnerGlobalPromise;
}
function loadOriginRouting() {
  originRoutingPromise ??= Promise.resolve().then(() => jitiImport("./origin-routing-CxGU7z77.js").then((m) => _interopRequireWildcard(m)));
  return originRoutingPromise;
}
function mergeSkillFilters(channelFilter, agentFilter) {
  const normalize = (list) => {
    if (!Array.isArray(list)) return;
    return (0, _stringNormalizationXm3f27dv.s)(list);
  };
...
function hasInboundMedia(ctx) {
  return Boolean(ctx.StickerMediaIncluded || ctx.Sticker || (0, _stringCoerceBUSzWgUA.s)(ctx.MediaPath) || (0, _stringCoerceBUSzWgUA.s)(ctx.MediaUrl) || ctx.MediaPaths?.some((value) => (0, _stringCoerceBUSzWgUA.s)(value)) || ctx.MediaUrls?.some((value) => (0, _stringCoerceBUSzWgUA.s)(value)) || ctx.MediaTypes?.length);
}
...
async function applyMediaUnderstandingIfNeeded(params) {
  ...
  const { applyMediaUnderstanding } = await Promise.resolve().then(() => jitiImport("./apply.runtime-DEfGTIg9.js").then((m) => _interopRequireWildcard(m)));
  await applyMediaUnderstanding(params);
  return true;
}
async function applyLinkUnderstandingIfNeeded(params) {
  if (!hasLinkCandidate(params.ctx)) return false;
  const { applyLinkUnderstanding } = await Promise.resolve().then(() => jitiImport("./apply.runtime-C9pFx-Xw.js").then((m) => _interopRequireWildcard(m)));
...
async function getReplyFromConfig(ctx, opts, configOverride) {
  ...
  const agentSessionKey = (ctx.CommandSource === "native" ? (0, _stringCoerceBUSzWgUA.s)(ctx.CommandTargetSessionKey) : void 0) || ctx.SessionKey;
  const agentId = (0, _agentScopeKFH9bkHi.p)({
    sessionKey: agentSessionKey,
    config: cfg
  });
  const mergedSkillFilter = mergeSkillFilters(opts?.skillFilter, (0, _agentScopeKFH9bkHi.l)(cfg, agentId));
  ...
  const sessionCfg = cfg.session;
  const { defaultProvider, defaultModel, aliasIndex } = (0, _directiveHandlingDefaultsD4jJiOY.t)({
    cfg,
    agentId
  });
  ...
  if (opts?.isHeartbeat) {
    const heartbeatRaw = (0, _stringCoerceBUSzWgUA.s)(opts.heartbeatModelOverride) ?? (0, _stringCoerceBUSzWgUA.s)(agentCfg?.heartbeat?.model) ?? "";
    const heartbeatRef = heartbeatRaw ? (0, _modelSelectionCTdyYoio.m)({
      raw: heartbeatRaw,
      defaultProvider,
      aliasIndex
    }) : null;
  ...
  const workspaceDirRaw = (0, _agentScopeKFH9bkHi.b)(cfg, agentId) ?? _workspaceHhTlRYqM.n;
  const workspaceDir = (useFastTestBootstrap ? (await _promises.default.mkdir(workspaceDirRaw, { recursive: true }), { dir: workspaceDirRaw }) : await (0, _workspaceHhTlRYqM.d)({
    dir: workspaceDirRaw,
    ensureBootstrapFiles: !agentCfg?.skipBootstrap && !isFastTestEnv
  })).dir;
  const agentDir = (0, _agentScopeKFH9bkHi.y)(cfg, agentId);
  const timeoutMs = (0, _timeoutCxW3WOrq.t)({
    cfg,
  ...
  const typing = createTypingController({
    ...
    silentToken: _tokensCKM4Lddu.n,
    log: _runtimeDx7oeLYq.n.log
  });
  opts?.onTypingController?.(typing);
  const finalized = (0, _inboundContext3c7B1YUQ.t)(ctx);
  if (!isFastTestEnv) {
  ...
  if (resetTriggered && (0, _stringCoerceBUSzWgUA.s)(bodyStripped)) {
  ...
  const channelModelOverride = (0, _modelOverridesBdNgdLaq.t)({
    cfg,
  ...
  const hasSessionModelOverride = Boolean((0, _stringCoerceBUSzWgUA.s)(sessionEntry.modelOverride) || (0, _stringCoerceBUSzWgUA.s)(sessionEntry.providerOverride));
  const storedModelOverride = (0, _storedModelOverrideDOM3YmmV.t)({
    sessionEntry,
    sessionStore,
    sessionKey,
  ...
  if (shouldUseReplyFastDirectiveExecution({
    ...
    resolvedVerboseLevel: (0, _thinkingSharedCAbk7EZs.d)(agentCfg?.verboseDefault),
    ...
    modelState: (0, _modelSelectionBUxj5f.t)({
      agentCfg,
      provider,
      model
    }),
  ...
  const maybeEmitMissingResetHooks = async () => {
    ...
    const { emitResetCommandHooks } = await Promise.resolve().then(() => jitiImport("./commands-core.runtime-C_jbTWFV.js").then((m) => _interopRequireWildcard(m)));
