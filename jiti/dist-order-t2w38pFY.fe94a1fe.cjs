"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = clearExpiredCooldowns;exports.c = markAuthProfileCooldown;exports.d = resolveProfileUnusableUntilForDisplay;exports.f = resolveProfilesUnavailableReason;exports.i = clearAuthProfileCooldown;exports.l = markAuthProfileFailure;exports.m = resolveTokenExpiryState;exports.n = resolveAuthProfileOrder;exports.o = getSoonestCooldownExpiry;exports.p = evaluateStoredCredentialEligibility;exports.r = calculateAuthProfileCooldownMs;exports.s = isProfileInCooldown;exports.t = resolveAuthProfileEligibility;exports.u = markAuthProfileUsed;var _subsystemCgmckbux = require("./subsystem-Cgmckbux.js");
...
var _redactIdentifierBi5PTLUH = require("./redact-identifier-Bi5PTLUH.js");
var _consoleSanitizeOEKi2aRr = require("./console-sanitize-OEKi2aRr.js");
...
function hasConfiguredSecretString(value) {
  return (0, _typesSecretsCeL3gSMO.l)(value) !== void 0;
}
function evaluateStoredCredentialEligibility(params) {
  ...
  if ((0, _typesSecretsCeL3gSMO.l)(credential.access) === void 0 && (0, _typesSecretsCeL3gSMO.l)(credential.refresh) === void 0) return {
    eligible: false,
    reasonCode: "missing_credential"
  };
  return {
...
const observationLog = (0, _subsystemCgmckbux.t)("agent/embedded");
function logAuthProfileFailureStateChange(params) {
  ...
  const safeProfileId = (0, _redactIdentifierBi5PTLUH.t)(params.profileId, { len: 12 });
  const safeRunId = (0, _consoleSanitizeOEKi2aRr.t)(params.runId) ?? "-";
  const safeProvider = (0, _consoleSanitizeOEKi2aRr.t)(params.provider) ?? "-";
...
const authProfileUsageDeps = {
  saveAuthProfileStore: _storeC1I9Mkh.c,
  updateAuthProfileStoreWithLock: _storeC1I9Mkh.l
};
...
function shouldProbeWhamForFailure(provider, reason) {
  return (0, _providerIdKaStHhRz.r)(provider ?? "") === "openai-codex" && (reason === "rate_limit" || reason === "unknown");
}
...
function resolveAuthCooldownConfig(params) {
  ...
  const billingBackoffHours = resolvePositiveNumber((() => {
    ...
    if (!map) return;
    for (const [key, value] of Object.entries(map)) if ((0, _providerIdKaStHhRz.r)(key) === params.providerId) return value;
...
function computeNextProfileUsageStats(params) {
  ...
  if (disabledFailureReason) {
    ...
    if (typeof params.existing.cooldownUntil === "number" && params.existing.cooldownUntil > params.now) {
      ...
      if (params.existing.cooldownModel && params.modelId && params.existing.cooldownModel !== params.modelId) updatedStats.cooldownModel = void 0;else
      if (params.reason === "rate_limit" && !params.modelId && params.existing.cooldownModel) updatedStats.cooldownModel = void 0;else
      if (params.reason !== "rate_limit") updatedStats.cooldownModel = void 0;else
...
async function markAuthProfileFailure(params) {
  ...
  const updated = await authProfileUsageDeps.updateAuthProfileStoreWithLock({
    ...
    updater: (freshStore) => {
      ...
      const cfgResolved = resolveAuthCooldownConfig({
        cfg,
        providerId: (0, _providerIdKaStHhRz.r)(profile.provider)
      });
  ...
  const cfgResolved = resolveAuthCooldownConfig({
    cfg,
    providerId: (0, _providerIdKaStHhRz.r)(store.profiles[profileId]?.provider ?? "")
  });
...
function resolveAuthProfileEligibility(params) {
  const providerAuthKey = (0, _providerEnvVarsSjBhOn.o)(params.provider, { config: params.cfg });
  const cred = params.store.profiles[params.profileId];
  if (!cred) return {
    eligible: false,
    reasonCode: "profile_missing"
  };
  if ((0, _providerEnvVarsSjBhOn.o)(cred.provider, { config: params.cfg }) !== providerAuthKey) return {
    eligible: false,
    reasonCode: "provider_mismatch"
  };
  const profileConfig = params.cfg?.auth?.profiles?.[params.profileId];
  if (profileConfig) {
    if ((0, _providerEnvVarsSjBhOn.o)(profileConfig.provider, { config: params.cfg }) !== providerAuthKey) return {
      eligible: false,
      reasonCode: "provider_mismatch"
    };
...
function resolveAuthProfileOrder(params) {
  ...
  const providerAuthKey = (0, _providerEnvVarsSjBhOn.o)(provider, { config: cfg });
  const now = Date.now();
  clearExpiredCooldowns(store, now);
  const storedOrder = (0, _providerIdKaStHhRz.n)(store.order, providerKey);
  const configuredOrder = (0, _providerIdKaStHhRz.n)(cfg?.auth?.order, providerKey);
  const explicitOrder = storedOrder ?? configuredOrder;
  const explicitProfiles = cfg?.auth?.profiles ? Object.entries(cfg.auth.profiles).filter(([, profile]) => (0, _providerEnvVarsSjBhOn.o)(profile.provider, { config: cfg }) === providerAuthKey).map(([profileId]) => profileId) : [];
  const baseOrder = explicitOrder ?? (explicitProfiles.length > 0 ? explicitProfiles : (0, _profilesCVErLX2C.n)(store, provider));
  ...
  if (filtered.length === 0 && explicitProfiles.length > 0 && allBaseProfilesMissing) filtered = (0, _profilesCVErLX2C.n)(store, provider).filter(isValidProfile);
  const deduped = (0, _profilesCVErLX2C.t)(filtered);
