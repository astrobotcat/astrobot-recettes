"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.discoverBedrockModels = discoverBedrockModels;exports.mergeImplicitBedrockProvider = mergeImplicitBedrockProvider;exports.resetBedrockDiscoveryCacheForTest = resetBedrockDiscoveryCacheForTest;exports.resolveBedrockConfigApiKey = resolveBedrockConfigApiKey;exports.resolveImplicitBedrockProvider = resolveImplicitBedrockProvider;var _clientBedrock = require("@aws-sdk/client-bedrock");
var _core = require("openclaw/plugin-sdk/core");
var _errorRuntime = require("openclaw/plugin-sdk/error-runtime");
var _providerAuthRuntime = require("openclaw/plugin-sdk/provider-auth-runtime");
var _textRuntime = require("openclaw/plugin-sdk/text-runtime");
//#region extensions/amazon-bedrock/discovery.ts
const log = (0, _core.createSubsystemLogger)("bedrock-discovery");
...
function normalizeProviderFilter(filter) {
  ...
  const normalized = new Set(filter.map((entry) => (0, _textRuntime.normalizeOptionalLowercaseString)(entry)).filter((entry) => Boolean(entry)));
...
function includesTextModalities(modalities) {
  return (modalities ?? []).some((entry) => (0, _textRuntime.normalizeOptionalLowercaseString)(entry) === "text");
}
...
function mapInputModalities(summary) {
  ...
  for (const modality of inputs) {
    const lower = (0, _textRuntime.normalizeOptionalLowercaseString)(modality);
...
function inferReasoningSupport(summary) {
  const haystack = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(`${summary.modelId ?? ""} ${summary.modelName ?? ""}`);
...
function matchesProviderFilter(summary, filter) {
  ...
  const normalized = (0, _textRuntime.normalizeOptionalLowercaseString)(summary.providerName ?? (typeof summary.modelId === "string" ? summary.modelId.split(".")[0] : void 0));
...
async function fetchInferenceProfileSummaries(client) {
  try {
    ...
    let nextToken;
    do {
      const response = await client.send(new _clientBedrock.ListInferenceProfilesCommand({ nextToken }));
      for (const summary of response.inferenceProfileSummaries ?? []) profiles.push(summary);
      nextToken = response.nextToken;
    } while (nextToken);
    return profiles;
  } catch (error) {
    log.debug?.("Skipping inference profile discovery", { error: (0, _errorRuntime.formatErrorMessage)(error) });
    return [];
  }
}
/**
  ...
  for (const profile of profiles) {
    ...
    if (providerFilter.length > 0) {
      if (!(profile.models ?? []).some((m) => {
        ...
        return provider ? providerFilter.includes((0, _textRuntime.normalizeOptionalLowercaseString)(provider) ?? "") : false;
      })) continue;
    }
    const baseModelId = resolveBaseModelId(profile);
    const baseModel = baseModelId ? foundationModels.get((0, _textRuntime.normalizeLowercaseStringOrEmpty)(baseModelId)) : void 0;
    discovered.push({
...
function resolveBedrockConfigApiKey(env = process.env) {
  return (0, _providerAuthRuntime.resolveAwsSdkEnvVarName)(env);
}
async function discoverBedrockModels(params) {
  ...
  const client = (params.clientFactory ?? ((region) => new _clientBedrock.BedrockClient({ region })))(params.region);
  const discoveryPromise = (async () => {
    const [foundationResponse, profileSummaries] = await Promise.all([client.send(new _clientBedrock.ListFoundationModelsCommand({})), fetchInferenceProfileSummaries(client)]);
    const discovered = [];
    ...
    for (const summary of foundationResponse.modelSummaries ?? []) {
      ...
      discovered.push(def);
      const normalizedId = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(def.id);
    ...
    for (const profile of inferenceProfiles) {
      const normalizedId = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(profile.id);
  ...
  try {
    ...
    if (!hasLoggedBedrockError) {
      ...
      log.warn("Failed to discover Bedrock models", { error: (0, _errorRuntime.formatErrorMessage)(error) });
    }
    return [];
  }
}
async function resolveImplicitBedrockProvider(params) {
  ...
  const hasAwsCreds = (0, _providerAuthRuntime.resolveAwsSdkEnvVarName)(env) !== void 0;
