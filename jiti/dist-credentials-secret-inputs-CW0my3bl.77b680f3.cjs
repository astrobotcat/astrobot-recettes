"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = assignResolvedGatewaySecretInput;exports.r = readGatewaySecretInputValue;exports.t = resolveGatewayCredentialsWithSecretInputs;var _typesSecretsCeL3gSMO = require("./types.secrets-CeL3gSMO.js");
...
async function resolveSecretInputString(params) {
  const normalize = params.normalize ?? _typesSecretsCeL3gSMO.l;
  ...
  let resolved;
  try {
    resolved = await (0, _resolveBKszWuwe.i)(ref, {
...
async function resolveGatewaySecretInputString(params) {
  const value = await resolveSecretInputString({
    ...
    value: params.value,
    env: params.env,
    normalize: _credentialPlannerCxtmVh4K.a,
    onResolveRefError: () => {
      throw new _credentialsDHNlrJW.t(params.path);
    }
  });
...
function gatewaySecretInputPathCanWin(params) {
  ...
  try {
    const resolved = (0, _credentialsDHNlrJW.r)(resolveGatewayCredentialsFromConfigOptions({
      cfg: probeConfig,
      env: params.env,
...
async function resolveGatewayCredentialsFromConfigWithSecretInputs(params) {
  ...
  for (;;) try {
    return (0, _credentialsDHNlrJW.r)(resolveGatewayCredentialsFromConfigOptions({
      cfg: resolvedConfig,
      env: params.env,
      options: params.options
    }));
  } catch (error) {
    if (!(error instanceof _credentialsDHNlrJW.t)) throw error;
