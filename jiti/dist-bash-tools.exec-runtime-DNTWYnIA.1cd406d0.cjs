"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = listFinishedSessions;exports.C = resolveWorkdir;exports.D = drainSession;exports.E = deleteSession;exports.F = tail;exports.I = applyPathPrepend;exports.L = normalizePathPrepend;exports.M = markBackgrounded;exports.N = markExited;exports.O = getFinishedSession;exports.P = setJobTtlMs;exports.S = resolveSandboxWorkdir;exports.T = truncateMiddle;exports._ = clampWithDefault;exports.a = void 0;exports.b = pad;exports.c = createApprovalSlug;exports.d = renderExecTargetLabel;exports.f = resolveApprovalRunningNoticeMs;exports.g = buildSandboxEnv;exports.h = buildDockerExecArgs;exports.i = void 0;exports.j = listRunningSessions;exports.k = getSession;exports.l = isRequestedExecTargetAllowed;exports.m = runExecProcess;exports.n = void 0;exports.o = applyShellPath;exports.p = resolveExecTarget;exports.r = void 0;exports.s = buildApprovalPendingMessage;exports.t = void 0;exports.u = normalizeNotifyOutput;exports.v = coerceEnv;exports.w = sliceLogLines;exports.x = readEnvInt;exports.y = deriveSessionName;var _utilsD5DtWkEu = require("./utils-D5DtWkEu.js");
require("./host-env-security-C2piJKe2.js");
...
require("./bash-tools.schemas-Hw2HS_ba.js");
...
var _shellUtils7dy62Zl_ = require("./shell-utils-7dy62Zl_.js");
var _supervisorDxNFYOdH = require("./supervisor-DxNFYOdH.js");
...
function randomChoice(values, fallback) {
  return values[(0, _secureRandomDHoiZOqc.r)(values.length)] ?? fallback;
}
const SLUG_FALLBACK_ALPHABET = "abcdefghijklmnopqrstuvwxyz0123456789";
function createFallbackSuffix(length) {
  let suffix = "";
  for (let i = 0; i < length; i += 1) suffix += SLUG_FALLBACK_ALPHABET[(0, _secureRandomDHoiZOqc.r)(36)] ?? "x";
  return suffix;
}
...
function appendOutput(session, stream, chunk) {
  ...
  if (stream === "stdout") session.pendingStdoutChars = pendingChars;else
...
function moveToFinished(session, status) {
  ...
  if (session.stdin) {
    if (typeof session.stdin.destroy === "function") session.stdin.destroy();else
    if (typeof session.stdin.end === "function") session.stdin.end();
    try {
...
async function resolveSandboxWorkdir(params) {
  ...
  try {
    const resolved = await (0, _sandboxPathsC5p25GeS.n)({
      filePath: candidateWorkdir,
      cwd: process.cwd(),
      root: params.sandbox.workspaceDir
    });
    if (!(await _promises.default.stat(resolved.resolved)).isDirectory()) throw new Error("workdir is not a directory");
    const relative = resolved.relative ? resolved.relative.split(_nodePath.default.sep).join(_nodePath.default.posix.sep) : "";
    const containerWorkdir = relative ? _nodePath.default.posix.join(params.sandbox.containerWorkdir, relative) : params.sandbox.containerWorkdir;
    return {
...
function mapContainerWorkdirToHost(params) {
  ...
  if (workdir === containerRoot) return _nodePath.default.resolve(params.sandbox.workspaceDir);
  if (!workdir.startsWith(`${containerRoot}/`)) return;
  const rel = workdir.slice(containerRoot.length + 1).split("/").filter(Boolean);
  return _nodePath.default.resolve(params.sandbox.workspaceDir, ...rel);
}
function normalizeContainerPath(input) {
  const normalized = input.trim().replace(/\\/g, "/");
  if (!normalized) return ".";
  return _nodePath.default.posix.normalize(normalized);
}
function resolveWorkdir(workdir, warnings) {
  const fallback = safeCwd() ?? (0, _nodeOs.homedir)();
  try {
    if ((0, _nodeFs.statSync)(workdir).isDirectory()) return workdir;
  } catch {}
...
function truncateMiddle(str, max) {
  ...
  return `${(0, _utilsD5DtWkEu.y)(str, 0, half)}...${(0, _utilsD5DtWkEu.y)(str, -half)}`;
}
...
const DEFAULT_MAX_OUTPUT = exports.r = clampWithDefault(readEnvInt("PI_BASH_MAX_OUTPUT_CHARS"), 2e5, 1e3, 2e5);
const DEFAULT_PENDING_MAX_OUTPUT = exports.a = clampWithDefault(readEnvInt("OPENCLAW_BASH_PENDING_MAX_OUTPUT_CHARS"), 3e4, 1e3, 2e5);
const DEFAULT_PATH = exports.i = process.env.PATH ?? "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin";
const DEFAULT_NOTIFY_SNIPPET_CHARS = 180;
const DEFAULT_APPROVAL_TIMEOUT_MS = exports.n = _execApprovalsDqhUuIS.r;
const DEFAULT_APPROVAL_REQUEST_TIMEOUT_MS = exports.t = DEFAULT_APPROVAL_TIMEOUT_MS + 1e4;
...
function applyShellPath(env, shellPath) {
  if (!shellPath) return;
  const entries = shellPath.split(_nodePath.default.delimiter).map((part) => part.trim()).filter(Boolean);
...
function maybeNotifyOnExit(session, status) {
  ...
  (0, _systemEventsDq_M0n.i)(output ? `Exec ${status} (${session.id.slice(0, 8)}, ${exitLabel}) :: ${output}` : `Exec ${status} (${session.id.slice(0, 8)}, ${exitLabel})`, {
    sessionKey,
    deliveryContext: session.notifyDeliveryContext,
    trusted: false
  });
  (0, _heartbeatWakeA3GAt85y.n)((0, _sessionKeyBh1lMwK.p)(sessionKey, { reason: "exec-event" }));
}
...
function buildApprovalPendingMessage(params) {
  ...
  const lines = [];
  const allowedDecisions = params.allowedDecisions ?? (0, _execApprovalsDqhUuIS.w)();
...
async function runExecProcess(opts) {
  ...
  const supervisor = (0, _supervisorDxNFYOdH.t)();
  ...
  const session = {
    ...
    notifyDeliveryContext: (0, _deliveryContextSharedEClQPjt.i)(opts.notifyDeliveryContext),
  ...
  const handleStdout = (data) => {
    ...
    const str = (0, _shellUtils7dy62Zl_.r)(raw);
  ...
  const handleStderr = (data) => {
    const str = (0, _shellUtils7dy62Zl_.r)(data);
    for (const chunk of chunkString(str)) {
  ...
  const spawnSpec = await (async () => {
    ...
    const { shell, args: shellArgs } = (0, _shellUtils7dy62Zl_.n)();
    const childArgv = [
    shell,
    ...shellArgs,
    execCommand];
  ...
  try {
    ...
    if (spawnSpec.mode === "pty") {
      ...
      (0, _loggerBA_TvTc.a)(`exec: PTY spawn failed (${String(err)}); retrying without PTY for "${opts.command}".`);
