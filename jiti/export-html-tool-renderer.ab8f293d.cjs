 ...
 */function createToolHtmlRenderer(deps) {const { getToolDefinition, theme, cwd, width = 100 } = deps;const renderedCallComponents = new Map();const renderedResultComponents = new Map();
  ...
  return {
    ...
    renderResult(toolCallId, toolName, result, details, isError) {
      try {
        ...
        const collapsed = (0, _ansiToHtml.ansiLinesToHtml)(collapsedComponent.render(width));
        // Render expanded
        ...
        const expanded = (0, _ansiToHtml.ansiLinesToHtml)(expandedComponent.render(width));
        return {
