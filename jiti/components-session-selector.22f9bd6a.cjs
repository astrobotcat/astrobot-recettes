...
class SessionSelectorHeader {
  ...
  render(width) {
    ...
    } else
    {
      ...
      const hint1 = (0, _keybindingHints.keyHint)("tui.input.tab", "scope") + sep + _theme.theme.fg("muted", 're:<pattern> regex · "phrase" exact');
      const hint2Parts = [
...
class SessionSelectorComponent extends _piTui.Container {
  ...
  enterRenameMode(sessionPath, currentName) {
    ...
    panel.addChild(new _piTui.Spacer(1));
    panel.addChild(new _piTui.Text(_theme.theme.fg("muted", `${(0, _keybindingHints.keyText)("tui.select.confirm")} to save · ${(0, _keybindingHints.keyText)("tui.select.cancel")} to cancel`), 1, 0));
...
}exports.SessionSelectorComponent = SessionSelectorComponent; /* v9-e7acc44556bb49c0 */
