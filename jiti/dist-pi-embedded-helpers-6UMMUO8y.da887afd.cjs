"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = resolveBootstrapPromptTruncationWarningMode;exports.a = isMessagingToolDuplicateNormalized;exports.c = extractToolCallsFromAssistant;exports.d = downgradeOpenAIFunctionCallReasoningPairs;exports.f = downgradeOpenAIReasoningBlocks;exports.g = resolveBootstrapMaxChars;exports.h = ensureSessionHeader;exports.i = isMessagingToolDuplicate;exports.l = extractToolResultId;exports.m = buildBootstrapContextFiles;exports.n = validateGeminiTurns;exports.o = normalizeTextForComparison;exports.p = isGoogleModelApi;exports.r = pickFallbackThinkingLevel;exports.s = sanitizeSessionMessagesImages;exports.t = validateAnthropicTurns;exports.u = sanitizeToolCallIdsForCloudCodeAssist;exports.v = resolveBootstrapTotalMaxChars;exports.y = sanitizeGoogleTurnOrdering;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _utilsD5DtWkEu = require("./utils-D5DtWkEu.js");
var _thinkingSharedCAbk7EZs = require("./thinking.shared-CAbk7EZs.js");
require("./sanitize-user-facing-text-CQF1CTnZ.js");
var _errorsCJULmF = require("./errors-CJULmF31.js");
...
function clampToBudget(content, budget) {
  ...
  if (budget <= 3) return (0, _utilsD5DtWkEu.b)(content, budget);
  return `${(0, _utilsD5DtWkEu.b)(content, budget - 1)}…`;
}
async function ensureSessionHeader(params) {
  ...
  await _promises.default.mkdir(_nodePath.default.dirname(file), {
    recursive: true,
    mode: 448
  });
  const entry = {
...
function buildBootstrapContextFiles(files, opts) {
  ...
  for (const file of files) {
    ...
    const pathValue = (0, _stringCoerceBUSzWgUA.s)(file.path) ?? "";
    if (!pathValue) {
...
function normalizeAllowedToolNames(allowedToolNames) {
  ...
  for (const name of allowedToolNames) {
    ...
    if (!trimmed) continue;
    normalized.add((0, _stringCoerceBUSzWgUA.i)(trimmed));
  }
...
function toolCallNeedsReplayMutation(block) {
  ...
  if ((0, _stringCoerceBUSzWgUA.i)(trimmedName) !== "sessions_spawn") return false;
...
function hasReplaySafeToolCallName(block, allowedToolNames) {
  ...
  return allowedToolNames.has((0, _stringCoerceBUSzWgUA.i)(trimmed));
}
...
function shortHash(text, length = 8) {
  return (0, _nodeCrypto.createHash)("sha256").update(text).digest("hex").slice(0, length);
}
...
function createOccurrenceAwareResolver(mode, options) {
  ...
  const resolveAssistantId = (id) => {
    ...
    if (pending) pending.push(next);else
  ...
  const preserveAssistantId = (id) => {
    used.add(id);
    const pending = pendingByRawId.get(id);
    if (pending) pending.push(id);else
...
async function sanitizeSessionMessagesImages(messages, label, options) {
  ...
  for (const msg of sanitizedIds) {
    ...
    if (role === "toolResult") {
      const toolMsg = msg;
      const nextContent = await (0, _toolImagesD6i9LZcG.t)(Array.isArray(toolMsg.content) ? toolMsg.content : [], label, imageSanitization);
      out.push({
        ...toolMsg,
        content: nextContent
      });
    ...
    if (role === "user") {
      ...
      if (Array.isArray(content)) {
        const nextContent = await (0, _toolImagesD6i9LZcG.t)(content, label, imageSanitization);
        out.push({
          ...userMsg,
          content: nextContent
        });
    ...
    if (role === "assistant") {
      ...
      if (Array.isArray(content)) {
        ...
        if (!allowNonImageSanitization) {
          const nextContent = await (0, _toolImagesD6i9LZcG.t)(strippedContent, label, imageSanitization);
          out.push({
            ...assistantMsg,
            content: nextContent
          });
        ...
        const finalContent = await (0, _toolImagesD6i9LZcG.t)(options?.preserveSignatures && strippedContent.some((block) => isThinkingOrRedactedBlock(block)) ? strippedContent : strippedContent.filter((block) => {
          if (!block || typeof block !== "object") return true;
          const rec = block;
...
function normalizeTextForComparison(text) {
  return (0, _stringCoerceBUSzWgUA.i)(text).replace(/\p{Emoji_Presentation}|\p{Extended_Pictographic}/gu, "").replace(/\s+/g, " ").trim();
}
...
function pickFallbackThinkingLevel(params) {
  ...
  if (!raw) return;
  if ((0, _errorsCJULmF.h)(raw) && !params.attempted.has("minimal")) return "minimal";
  ...
  for (const entry of supported) {
    const normalized = (0, _thinkingSharedCAbk7EZs.c)(entry);
...
function extractToolResultMatchIds(record) {
  ...
  record.call_id])
  {
    const id = (0, _stringCoerceBUSzWgUA.s)(value);
    if (id) ids.add(id);
  }
  return ids;
}
function extractToolResultMatchName(record) {
  return (0, _stringCoerceBUSzWgUA.s)(record.toolName) ?? (0, _stringCoerceBUSzWgUA.s)(record.name) ?? null;
}
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (hasThinking) {
      if (originalContent.every((block) => {
        ...
        const blockId = (0, _stringCoerceBUSzWgUA.s)(block.id);
        const blockName = (0, _stringCoerceBUSzWgUA.s)(block.name);
