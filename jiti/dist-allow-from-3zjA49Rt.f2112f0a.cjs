"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = mapAllowlistResolutionInputs;exports.i = isNormalizedSenderAllowed;exports.n = formatNormalizedAllowFromEntries;exports.o = mapBasicAllowlistResolutionEntries;exports.r = isAllowedParsedChatSender;exports.t = formatAllowFromLowercase;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function formatAllowFromLowercase(params) {
  return params.allowFrom.map((entry) => String(entry).trim()).filter(Boolean).map((entry) => params.stripPrefixRe ? entry.replace(params.stripPrefixRe, "") : entry).map((entry) => (0, _stringCoerceBUSzWgUA.o)(entry)).filter((entry) => Boolean(entry));
}
...
function isNormalizedSenderAllowed(params) {
  ...
  const sender = (0, _stringCoerceBUSzWgUA.o)(String(params.senderId));
