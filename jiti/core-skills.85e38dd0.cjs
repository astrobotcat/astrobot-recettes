...
var _sourceInfo = require("./source-info.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
function addIgnoreRules(ig, dir, rootDir) {
  const relativeDir = (0, _path.relative)(rootDir, dir);
  const prefix = relativeDir ? `${toPosixPath(relativeDir)}/` : "";
  for (const filename of IGNORE_FILE_NAMES) {
    const ignorePath = (0, _path.join)(dir, filename);
    if (!(0, _fs.existsSync)(ignorePath))
    continue;
    try {
      const content = (0, _fs.readFileSync)(ignorePath, "utf-8");
...
function createSkillSourceInfo(filePath, baseDir, source) {
  switch (source) {
    case "user":
      return (0, _sourceInfo.createSyntheticSourceInfo)(filePath, {
        source: "local",
        scope: "user",
        baseDir
      });
    ...
    default:
      return (0, _sourceInfo.createSyntheticSourceInfo)(filePath, { source, baseDir });
  }
}
/**
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        try {
          isFile = (0, _fs.statSync)(fullPath).isFile();
        }
        catch {
          continue;
        }
      }
      const relPath = toPosixPath((0, _path.relative)(root, fullPath));
