"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = registerSessionBindingAdapter;exports.i = isSessionBindingError;exports.n = void 0;exports.o = unregisterSessionBindingAdapter;exports.r = getSessionBindingService;exports.t = void 0;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
require("./json-store-iQhnwImo.js");
...
function buildConversationKey(ref) {
  const normalized = (0, _conversationIdBkdkxdwP.r)(ref);
  return [
  normalized.channel,
...
function resolveBindingsFilePath(env = process.env) {
  return _nodePath.default.join((0, _pathsDvv9VRAc._)(env), "bindings", "current-conversations.json");
}
...
function loadBindingsIntoMemory() {
  ...
  const parsed = (0, _jsonFileDAv7R9GL.t)(resolveBindingsFilePath());
  ...
  for (const record of bindings ?? []) {
    ...
    const conversation = (0, _conversationIdBkdkxdwP.r)(record.conversation);
...
function persistBindingsToDisk() {
  (0, _jsonFileDAv7R9GL.n)(resolveBindingsFilePath(), toPersistedFile());
}
...
function resolveChannelSupportsCurrentConversationBinding(channel) {
  const normalized = (0, _registryCENZffQG.a)(channel) ?? (0, _stringCoerceBUSzWgUA.o)((0, _conversationIdBkdkxdwP.t)(channel));
  if (!normalized) return false;
  const matchesPluginId = (plugin) => plugin.id === normalized || (plugin.meta?.aliases ?? []).some((alias) => (0, _stringCoerceBUSzWgUA.o)(alias) === normalized);
  if (((0, _runtimeChannelStateCKbFw7bt.t)()?.channels ?? []).find((entry) => matchesPluginId(entry.plugin))?.plugin?.conversationBindings?.supportsCurrentConversationBinding === true) return true;
  return false;
}
...
async function bindGenericCurrentConversation(input) {
  const conversation = (0, _conversationIdBkdkxdwP.r)(input.conversation);
...
const ADAPTERS_BY_CHANNEL_ACCOUNT = (0, _globalSingletonB80lDOJ.t)(Symbol.for("openclaw.sessionBinding.adapters"));
...
function createDefaultSessionBindingService() {
  return {
    bind: async (input) => {
      const normalizedConversation = (0, _conversationIdBkdkxdwP.r)(input.conversation);
