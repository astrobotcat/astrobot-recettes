...
function parseAgentSessionKey(sessionKey) {
  const raw = (0, _stringCoerceBUSzWgUA.o)(sessionKey);
  if (!raw) return null;
  const parts = raw.split(":").filter(Boolean);
  if (parts.length < 3) return null;
  if (parts[0] !== "agent") return null;
  const agentId = (0, _stringCoerceBUSzWgUA.s)(parts[1]);
...
function isCronSessionKey(sessionKey) {
  const parsed = parseAgentSessionKey(sessionKey);
  if (!parsed) return false;
  return (0, _stringCoerceBUSzWgUA.o)(parsed.rest)?.startsWith("cron:") === true;
}
function isSubagentSessionKey(sessionKey) {
  const raw = (0, _stringCoerceBUSzWgUA.s)(sessionKey);
  if (!raw) return false;
  if ((0, _stringCoerceBUSzWgUA.o)(raw)?.startsWith("subagent:")) return true;
  return (0, _stringCoerceBUSzWgUA.o)(parseAgentSessionKey(raw)?.rest)?.startsWith("subagent:") === true;
}
...
function isAcpSessionKey(sessionKey) {
  const raw = (0, _stringCoerceBUSzWgUA.s)(sessionKey);
  if (!raw) return false;
  if ((0, _stringCoerceBUSzWgUA.i)(raw).startsWith("acp:")) return true;
  return (0, _stringCoerceBUSzWgUA.o)(parseAgentSessionKey(raw)?.rest)?.startsWith("acp:") === true;
}
function parseThreadSessionSuffix(sessionKey) {
  ...
  const markerIndex = (0, _stringCoerceBUSzWgUA.i)(raw).lastIndexOf(":thread:");
  return {
    baseSessionKey: markerIndex === -1 ? raw : raw.slice(0, markerIndex),
    threadId: (0, _stringCoerceBUSzWgUA.s)(markerIndex === -1 ? void 0 : raw.slice(markerIndex + 8))
  };
}
function parseRawSessionConversationRef(sessionKey) {
  ...
  const bodyStartIndex = rawParts.length >= 3 && (0, _stringCoerceBUSzWgUA.o)(rawParts[0]) === "agent" ? 2 : 0;
  const parts = rawParts.slice(bodyStartIndex);
  if (parts.length < 3) return null;
  const channel = (0, _stringCoerceBUSzWgUA.o)(parts[0]);
  const kind = (0, _stringCoerceBUSzWgUA.o)(parts[1]);
  if (!channel || kind !== "group" && kind !== "channel") return null;
  const rawId = (0, _stringCoerceBUSzWgUA.s)(parts.slice(2).join(":"));
  const prefix = (0, _stringCoerceBUSzWgUA.s)(rawParts.slice(0, bodyStartIndex + 2).join(":"));
...
function normalizeMainKey(value) {
  return (0, _stringCoerceBUSzWgUA.i)(value) || "main";
}
...
function classifySessionKeyShape(sessionKey) {
  ...
  return (0, _stringCoerceBUSzWgUA.i)(raw).startsWith("agent:") ? "malformed_agent" : "legacy_or_alias";
}
...
function buildAgentPeerSessionKey(params) {
  ...
  if (peerKind === "direct") {
    ...
    peerId = (0, _stringCoerceBUSzWgUA.i)(peerId);
    if (dmScope === "per-account-channel-peer" && peerId) {
      const channel = (0, _stringCoerceBUSzWgUA.i)(params.channel) || "unknown";
  ...
  const channel = (0, _stringCoerceBUSzWgUA.i)(params.channel) || "unknown";
  const peerId = (0, _stringCoerceBUSzWgUA.i)(params.peerId) || "unknown";
...
function resolveThreadSessionKeys(params) {
  ...
  const normalizedThread = params.normalizeThreadId?.(threadId) ?? (0, _stringCoerceBUSzWgUA.i)(threadId);
  return {
    sessionKey: params.useSuffix ?? true ? `${params.baseSessionKey}:thread:${normalizedThread}` : params.baseSessionKey,
    parentSessionKey: params.parentSessionKey
  };
}
//#endregion /* v9-b291281f7d5f2782 */
