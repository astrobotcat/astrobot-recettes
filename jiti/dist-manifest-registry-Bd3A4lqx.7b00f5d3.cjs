"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = clearPluginManifestRegistryCache;exports.c = resolveEffectivePluginActivationState;exports.d = isBundledChannelEnabledByChannelConfig$1;exports.f = normalizePluginsConfigWithResolver$1;exports.h = resolveMemorySlotDecisionShared;exports.i = resolveManifestContractPluginIdsByCompatibilityRuntimePath;exports.l = resolveMemorySlotDecision;exports.m = resolveEnableStateShared;exports.n = resolveManifestContractOwnerPluginId;exports.o = hasExplicitPluginConfig;exports.p = resolveEnableStateResult;exports.r = resolveManifestContractPluginIds;exports.s = normalizePluginsConfigWithResolver;exports.t = loadPluginManifestRegistry;exports.u = hasExplicitPluginConfig$1;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _minHostVersionBZQ2SlKG = require("./min-host-version-BZQ2SlKG.js");
...
function normalizeSlotValue(value) {
  ...
  if (!trimmed) return;
  if ((0, _stringCoerceBUSzWgUA.o)(trimmed) === "none") return null;
  return trimmed;
}
function normalizePluginEntries(entries, normalizePluginId) {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    const subagent = subagentRaw && typeof subagentRaw === "object" && !Array.isArray(subagentRaw) ? {
      ...
      allowedModels: Array.isArray(subagentRaw.allowedModels) ? subagentRaw.allowedModels.map((model) => (0, _stringCoerceBUSzWgUA.s)(model)).filter((model) => Boolean(model)) : void 0
    } : void 0;
...
function isBundledChannelEnabledByChannelConfig$1(cfg, pluginId) {
  if (!cfg) return false;
  const channelId = (0, _idsCYPyP4SY.r)(pluginId);
...
function resolveManifestContractOwnerPluginId(params) {
  const normalizedValue = (0, _stringCoerceBUSzWgUA.o)(params.value);
  if (!normalizedValue) return;
  return loadPluginManifestRegistry({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env
  }).plugins.find((plugin) => (!params.origin || plugin.origin === params.origin) && listContractValues(plugin, params.contract).some((candidate) => (0, _stringCoerceBUSzWgUA.o)(candidate) === normalizedValue))?.id;
}
...
function buildCacheKey(params) {
  ...
  return `${roots.workspace ?? ""}::${roots.global}::${roots.stock ?? ""}::${(0, _versionBk5OWRN.o)(params.env)}::${JSON.stringify(loadPaths)}`;
}
...
function normalizePreferredPluginIds(raw) {
  return (0, _stringNormalizationXm3f27dv.a)(raw);
}
function mergePackageChannelMetaIntoChannelConfigs(params) {
  ...
  const label = existing.label ?? (0, _stringCoerceBUSzWgUA.s)(params.packageChannel?.label) ?? "";
  const description = existing.description ?? (0, _stringCoerceBUSzWgUA.s)(params.packageChannel?.blurb) ?? "";
...
function buildRecord(params) {
  ...
  return {
    ...
    name: (0, _stringCoerceBUSzWgUA.s)(params.manifest.name) ?? params.candidate.packageName,
    description: (0, _stringCoerceBUSzWgUA.s)(params.manifest.description) ?? params.candidate.packageDescription,
    version: (0, _stringCoerceBUSzWgUA.s)(params.manifest.version) ?? params.candidate.packageVersion,
    ...
    providerDiscoverySource: params.manifest.providerDiscoveryEntry ? _nodePath.default.resolve(params.candidate.rootDir, params.manifest.providerDiscoveryEntry) : void 0,
...
function buildBundleRecord(params) {
  return {
    id: params.manifest.id,
    name: (0, _stringCoerceBUSzWgUA.s)(params.manifest.name) ?? params.candidate.idHint,
    description: (0, _stringCoerceBUSzWgUA.s)(params.manifest.description),
    version: (0, _stringCoerceBUSzWgUA.s)(params.manifest.version),
    format: "bundle",
    bundleFormat: params.candidate.bundleFormat,
...
function matchesInstalledPluginRecord(params) {
  ...
  const candidateSource = (0, _utilsD5DtWkEu.m)(params.candidate.source, params.env);
  const trackedPaths = [record.installPath, record.sourcePath].filter((entry) => typeof entry === "string" && entry.trim().length > 0).map((entry) => (0, _utilsD5DtWkEu.m)(entry, params.env));
  if (trackedPaths.length === 0) return false;
  return trackedPaths.some((trackedPath) => {
    return candidateSource === trackedPath || (0, _pathSafetyCMqhKBu.n)(trackedPath, candidateSource);
  });
}
...
function loadPluginManifestRegistry(params = {}) {
  ...
  const currentHostVersion = (0, _versionBk5OWRN.o)(env);
  for (const candidate of candidates) {
    ...
    } : isBundleRecord && candidate.bundleFormat ? (0, _pathSafetyCMqhKBu.l)({
      rootDir: candidate.rootDir,
      bundleFormat: candidate.bundleFormat,
      rejectHardlinks
    }) : (0, _manifestDKZWfJEu.i)(candidate.rootDir, rejectHardlinks);
    if (!manifestRes.ok) {
      diagnostics.push({
        level: "error",
    ...
    const minHostVersionCheck = (0, _minHostVersionBZQ2SlKG.t)({
    ...
    if (existing) {
      ...
      if ((() => {
        ...
        const existingReal = (0, _pathSafetyCMqhKBu.r)(existing.candidate.rootDir, realpathCache);
        const candidateReal = (0, _pathSafetyCMqhKBu.r)(candidate.rootDir, realpathCache);
