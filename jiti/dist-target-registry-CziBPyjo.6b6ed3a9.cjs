...
function listChannelSecretTargetRegistryEntries() {
  const entries = [];
  for (const record of (0, _manifestRegistryBd3A4lqx.t)({}).plugins) {
    if (record.origin !== "bundled") continue;
    if (record.channels.length === 0) continue;
    try {
      const contractApi = (0, _channelContractApiVtp76BhJ.t)(record.id);
...
function parsePathPattern(pathPattern) {
  return (0, _sharedDLDubI1E.i)(pathPattern).map((segment) => {
...
function expandPathTokens(root, tokens) {
  ...
  const walk = (node, tokenIndex, segments, captures) => {
    ...
    if (token.kind === "literal") {
      if (!(0, _utilsD5DtWkEu.l)(node)) return;
      if (isLeaf) {
        out.push({
...
function getCompiledBundledChannelOpenClawTargets(channelId) {
  ...
  const compiledEntries = (0, _channelContractApiVtp76BhJ.t)(normalizedChannelId)?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "openclaw.json").map(compileTargetRegistryEntry) ?? null;
...
function discoverSecretTargetsFromEntries(source, discoveryEntries) {
  ...
  for (const entry of discoveryEntries) {
    ...
    for (const match of expanded) {
      ...
      seen.add(key);
      const refValue = resolved.refPathSegments ? (0, _pathUtilsCiRQUmZp.n)(source, resolved.refPathSegments) : void 0;
      out.push({
        entry,
