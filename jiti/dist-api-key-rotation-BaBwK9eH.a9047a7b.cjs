"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = executeWithApiKeyRotation;exports.t = collectProviderApiKeysForExecution;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
...
function collectProviderApiKeys(provider, options = {}) {
  ...
  const forcedSingle = config.liveSingle ? (0, _stringCoerceBUSzWgUA.s)(env[config.liveSingle]) : void 0;
  if (forcedSingle) return [forcedSingle];
  const fromList = parseKeyList(config.listVar ? env[config.listVar] : void 0);
  const primary = config.primaryVar ? (0, _stringCoerceBUSzWgUA.s)(env[config.primaryVar]) : void 0;
  const fromPrefixed = config.prefixedVar ? collectEnvPrefixedKeys(config.prefixedVar, env) : [];
  const fallback = config.fallbackVars.map((envVar) => (0, _stringCoerceBUSzWgUA.s)(env[envVar])).filter(Boolean);
  const manifestFallback = (options.providerEnvVars ?? (0, _providerEnvVarsSjBhOn.t)(normalizedProvider)).map((envVar) => (0, _stringCoerceBUSzWgUA.s)(env[envVar])).filter(Boolean);
