 ...
 */class FileAuthStorageBackend {authPath;constructor(authPath = (0, _path.join)((0, _config.getAgentDir)(), "auth.json")) {this.authPath = authPath;}ensureParentDir() {
  ...
  withLock(fn) {
    ...
    try {
      ...
      if (next !== undefined) {
        (0, _fs.writeFileSync)(this.authPath, next, "utf-8");
        (0, _fs.chmodSync)(this.authPath, 0o600);
      }
      return result;
    } finally
...
class AuthStorage {
  ...
  async getApiKey(providerId, options) {
    ...
    if (cred?.type === "api_key") {
      return (0, _resolveConfigValue.resolveConfigValue)(cred.key);
    }
    if (cred?.type === "oauth") {
      const provider = (0, _oauth.getOAuthProvider)(providerId);
      if (!provider) {
...
}exports.AuthStorage = AuthStorage; /* v9-3bff1d47c38eeefc */
