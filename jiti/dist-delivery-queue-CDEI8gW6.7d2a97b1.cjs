"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = recoverPendingDeliveries;exports.c = ensureQueueDir;exports.d = loadPendingDelivery;exports.f = moveToFailed;exports.i = isPermanentDeliveryError;exports.l = failDelivery;exports.n = drainPendingDeliveries;exports.o = ackDelivery;exports.r = isEntryEligibleForRecoveryRetry;exports.s = enqueueDelivery;exports.t = computeBackoffMs;exports.u = loadPendingDeliveries;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
...
function resolveQueueDir(stateDir) {
  const base = stateDir ?? (0, _pathsDvv9VRAc._)();
...
async function readQueueEntry(filePath) {
  return JSON.parse(await _nodeFs.default.promises.readFile(filePath, "utf-8"));
}
...
async function enqueueDelivery(params, stateDir) {
  ...
  const id = (0, _secureRandomDHoiZOqc.a)();
  await writeQueueEntry(_nodePath.default.join(queueDir, `${id}.json`), {
    id,
    enqueuedAt: Date.now(),
...
async function loadPendingDelivery(id, stateDir) {
  ...
  try {
    if (!(await _nodeFs.default.promises.stat(jsonPath)).isFile()) return null;
...
async function loadPendingDeliveries(stateDir) {
  ...
  let files;
  try {
    files = await _nodeFs.default.promises.readdir(queueDir);
  } catch (err) {
    if (getErrnoCode$1(err) === "ENOENT") return [];
    throw err;
  }
  for (const file of files) if (file.endsWith(".delivered")) await unlinkBestEffort(_nodePath.default.join(queueDir, file));
  const entries = [];
  for (const file of files) {
    if (!file.endsWith(".json")) continue;
    const filePath = _nodePath.default.join(queueDir, file);
    try {
      if (!(await _nodeFs.default.promises.stat(filePath)).isFile()) continue;
...
async function moveToFailed(id, stateDir) {
  ...
  await _nodeFs.default.promises.mkdir(failedDir, {
    recursive: true,
    mode: 448
  });
  await _nodeFs.default.promises.rename(_nodePath.default.join(queueDir, `${id}.json`), _nodePath.default.join(failedDir, `${id}.json`));
}
...
async function drainQueuedEntry(opts) {
  ...
  try {
    ...
    const errMsg = (0, _errorsD8p6rxH.i)(err);
