"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveModelDirectiveSelection;exports.n = createModelSelectionState;exports.r = resolveContextTokens;exports.t = createFastTestModelSelectionState;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _contextD6_2eKB = require("./context-D6_2eK-b.js");
var _modelOverridesBEFWLg = require("./model-overrides-BEFWLg94.js");
var _sessionOverrideTDK5Oy = require("./session-override-TDK5Oy09.js");
var _storedModelOverrideDOM3YmmV = require("./stored-model-override-DOM3YmmV.js");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
function loadModelCatalogRuntime() {
  modelCatalogRuntimePromise ??= Promise.resolve().then(() => jitiImport("./agents/model-catalog.runtime.js").then((m) => _interopRequireWildcard(m)));
...
function scoreFuzzyMatch(params) {
  ...
  const fragment = (0, _stringCoerceBUSzWgUA.i)(params.fragment);
  const providerLower = (0, _stringCoerceBUSzWgUA.i)(provider);
  const modelLower = (0, _stringCoerceBUSzWgUA.i)(model);
  const haystack = `${providerLower}/${modelLower}`;
  const key = (0, _modelSelectionCli_3Gn8Wcd.f)(provider, model);
  ...
  for (const alias of aliases) score += scoreFragment((0, _stringCoerceBUSzWgUA.i)(alias), {
    exact: 140,
    starts: 90,
    includes: 60
  });
  ...
  if (fragmentVariants.length === 0 && variantCount > 0) score -= variantCount * 30;else
  if (fragmentVariants.length > 0) {
    if (variantMatchCount > 0) score += variantMatchCount * 40;
    if (variantMatchCount === 0) score -= 20;
  }
  const isDefault = provider === (0, _providerIdKaStHhRz.r)(params.defaultProvider) && model === params.defaultModel;
...
async function createModelSelectionState(params) {
  ...
  const configuredModelCatalog = (0, _modelSelectionCTdyYoio.r)({ cfg });
  ...
  const agentEntry = params.agentId ? (0, _agentScopeKFH9bkHi._)(cfg, params.agentId) : void 0;
  const directStoredOverride = (0, _modelSelectionCTdyYoio.g)({
    defaultProvider,
  ...
  if (needsModelCatalog) {
    ...
    const allowed = (0, _modelSelectionCTdyYoio.t)({
      cfg,
  ...
  if (sessionEntry && sessionStore && sessionKey && directStoredOverride) {
    const normalizedOverride = (0, _modelSelectionCli_3Gn8Wcd.p)(directStoredOverride.provider, directStoredOverride.model);
    const key = (0, _modelSelectionCli_3Gn8Wcd.f)(normalizedOverride.provider, normalizedOverride.model);
    if (allowedModelKeys.size > 0 && !allowedModelKeys.has(key)) {
      const { updated } = (0, _modelOverridesBEFWLg.t)({
        entry: sessionEntry,
        selection: {
  ...
  const storedOverride = (0, _storedModelOverrideDOM3YmmV.t)({
    sessionEntry,
    sessionStore,
    sessionKey,
    parentSessionKey,
  ...
  if (storedOverride?.model && !skipStoredOverride) {
    const normalizedStoredOverride = (0, _modelSelectionCli_3Gn8Wcd.p)(storedOverride.provider || defaultProvider, storedOverride.model);
    const key = (0, _modelSelectionCli_3Gn8Wcd.f)(normalizedStoredOverride.provider, normalizedStoredOverride.model);
    if (allowedModelKeys.size === 0 || allowedModelKeys.has(key)) {
  ...
  if (sessionEntry && sessionStore && sessionKey && sessionEntry.authProfileOverride) {
    const { ensureAuthProfileStore } = await Promise.resolve().then(() => jitiImport("./agents/auth-profiles.runtime.js").then((m) => _interopRequireWildcard(m)));
    ...
    const providerKey = (0, _providerIdKaStHhRz.r)(provider);
    if (!profile || (0, _providerIdKaStHhRz.r)(profile.provider) !== providerKey) await (0, _sessionOverrideTDK5Oy.t)({
      sessionEntry,
      sessionStore,
      sessionKey,
      storePath
  ...
  const resolveDefaultThinkingLevel = async () => {
    ...
    const resolved = (0, _modelSelectionCTdyYoio.x)({
      cfg,
      provider,
      model,
  ...
  const resolveDefaultReasoningLevel = async () => {
    ...
    return (0, _modelSelectionCTdyYoio.v)({
      provider,
      model,
...
function resolveModelDirectiveSelection(params) {
  ...
  const rawLower = (0, _stringCoerceBUSzWgUA.i)(rawTrimmed);
  const pickAliasForKey = (provider, model) => aliasIndex.byKey.get((0, _modelSelectionCli_3Gn8Wcd.f)(provider, model))?.[0];
  ...
  const resolveFuzzy = (params) => {
    const fragment = (0, _stringCoerceBUSzWgUA.i)(params.fragment);
    if (!fragment) return {};
    const providerFilter = params.provider ? (0, _providerIdKaStHhRz.r)(params.provider) : void 0;
    const candidates = [];
    for (const key of allowedModelKeys) {
      const slash = key.indexOf("/");
      if (slash <= 0) continue;
      const provider = (0, _providerIdKaStHhRz.r)(key.slice(0, slash));
    ...
    if (!params.provider) {
      ...
      for (const match of aliasMatches) {
        const key = (0, _modelSelectionCli_3Gn8Wcd.f)(match.provider, match.model);
  ...
  const resolvedKey = (0, _modelSelectionCli_3Gn8Wcd.f)(resolved.ref.provider, resolved.ref.model);
  ...
  if (rawLower.includes("/")) {
    ...
    const fuzzy = resolveFuzzy({
      provider: (0, _providerIdKaStHhRz.r)(rawTrimmed.slice(0, slash).trim()),
...
function resolveContextTokens(params) {
  return params.agentCfg?.contextTokens ?? (0, _contextD6_2eKB.a)({
    cfg: params.cfg,
    provider: params.provider,
    model: params.model,
