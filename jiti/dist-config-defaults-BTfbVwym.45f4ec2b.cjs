"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = normalizeAnthropicProviderConfig;exports.t = applyAnthropicConfigDefaults;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _providerIdKaStHhRz = require("./provider-id-KaStHhRz.js");
require("./provider-model-shared-DyDnBaDe.js");
...
function parseProviderModelRef(raw, defaultProvider) {
  ...
  return {
    provider: (0, _providerIdKaStHhRz.r)(provider),
    model
  };
}
function isAnthropicCacheRetentionTarget(parsed) {
  return Boolean(parsed && (parsed.provider === "anthropic" || parsed.provider === "amazon-bedrock" && (0, _stringCoerceBUSzWgUA.i)(parsed.model).includes("anthropic.claude")));
}
function usesClaudeCliModelSelection(config) {
  ...
  return Object.keys(config.agents?.defaults?.models ?? {}).some((key) => {
    return parseProviderModelRef(key, "anthropic")?.provider === _cliSharedDOMKlVw.t;
  });
}
...
function applyAnthropicConfigDefaults(params) {
  ...
  if (authMode === "oauth" && usesClaudeCliModelSelection(params.config)) {
    const nextModels = defaults.models ? { ...defaults.models } : {};
    let modelsMutated = false;
    for (const ref of _cliSharedDOMKlVw.r) {
