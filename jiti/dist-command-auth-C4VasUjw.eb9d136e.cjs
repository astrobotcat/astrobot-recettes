"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = resolveCommandAuthorization;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function resolveProviderFromContext(ctx, cfg) {
  ...
  ctx.Provider].
  map((value) => (0, _messageChannelCBqCPFa_.u)(value)).filter((value) => Boolean(value));
  const explicitMessageChannel = explicitMessageChannels.find((value) => value !== _messageChannelCoreBIZsQ6dr.r);
  if (!explicitMessageChannel && explicitMessageChannels.includes("webchat")) return {
    providerId: void 0,
    hadResolutionError: false
  };
  const direct = (0, _registryCENZffQG.a)(explicitMessageChannel ?? void 0) ?? explicitMessageChannel ?? (0, _registryCENZffQG.a)(ctx.Provider) ?? (0, _registryCENZffQG.a)(ctx.Surface) ?? (0, _registryCENZffQG.a)(ctx.OriginatingChannel);
  if (direct) return {
    providerId: direct,
    hadResolutionError: false
  };
  ...
  for (const candidate of candidates) {
    const normalizedCandidateChannel = (0, _messageChannelCBqCPFa_.u)(candidate);
    if (normalizedCandidateChannel === "webchat") return {
      providerId: void 0,
      hadResolutionError: false
    };
    const normalized = (0, _registryCENZffQG.a)(normalizedCandidateChannel ?? void 0) ?? normalizedCandidateChannel ?? (0, _registryCENZffQG.a)(candidate);
...
function probeInferredProviders(ctx, cfg) {
  ...
  return {
    candidates: (0, _registryLoadedC109837J.n)().map((plugin) => {
...
function formatAllowFromList(params) {
  ...
  return (0, _stringNormalizationXm3f27dv.s)(allowFrom);
}
...
function describeAllowFromResolutionError(err) {
  if (err instanceof Error) return ((0, _stringCoerceBUSzWgUA.s)(err.name) ?? "") || "Error";
...
function resolveOwnerAllowFromList(params) {
  ...
  for (const entry of raw) {
    const trimmed = (0, _stringCoerceBUSzWgUA.s)(String(entry ?? "")) ?? "";
    if (!trimmed) continue;
    const separatorIndex = trimmed.indexOf(":");
    if (separatorIndex > 0) {
      const channel = (0, _registryCENZffQG.a)(trimmed.slice(0, separatorIndex));
      if (channel) {
...
function shouldUseFromAsSenderFallback(params) {
  ...
  const chatType = (0, _stringCoerceBUSzWgUA.i)(params.chatType);
...
function resolveFallbackAllowFrom(params) {
  const providerId = (0, _stringCoerceBUSzWgUA.s)(params.providerId);
...
function resolveFallbackAccountConfig(accounts, accountId) {
  const normalizedAccountId = (0, _stringCoerceBUSzWgUA.o)(accountId);
  if (!accounts || !normalizedAccountId) return;
  const direct = accounts[normalizedAccountId];
  if (direct) return direct;
  const matchKey = Object.keys(accounts).find((key) => (0, _stringCoerceBUSzWgUA.o)(key) === normalizedAccountId);
...
function resolveCommandAuthorization(params) {
  ...
  const plugin = providerId ? (0, _registryLoadedC109837J.t)(providerId) ?? void 0 : void 0;
  ...
  const senderIsOwnerByScope = (0, _messageChannelCBqCPFa_.r)(ctx.Provider) && Array.isArray(ctx.GatewayClientScopes) && ctx.GatewayClientScopes.includes("operator.admin");
