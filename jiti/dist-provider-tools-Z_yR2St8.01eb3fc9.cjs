"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = buildProviderToolCompatFamilyHooks;exports.c = inspectGeminiToolSchemas;exports.d = normalizeOpenAIToolSchemas;exports.f = resolveXaiModelCompatPatch;exports.g = cleanSchemaForGemini;exports.h = void 0;exports.i = applyXaiModelCompat;exports.l = inspectOpenAIToolSchemas;exports.m = stripXaiUnsupportedKeywords;exports.n = void 0;exports.o = findOpenAIStrictSchemaViolations;exports.p = stripUnsupportedSchemaKeywords;exports.r = void 0;exports.s = findUnsupportedSchemaKeywords;exports.t = void 0;exports.u = normalizeGeminiToolSchemas;var _providerModelCompatDsxuyzi = require("./provider-model-compat-Dsxuyzi4.js");
//#region src/agents/schema/clean-for-gemini.ts
const GEMINI_UNSUPPORTED_SCHEMA_KEYWORDS = exports.h = new Set([
"patternProperties",
...
function tryFlattenLiteralAnyOf(variants) {
  ...
  for (const variant of variants) {
    ...
    const v = variant;
    let literalValue;
    if ("const" in v) literalValue = v.const;else
    if (Array.isArray(v.enum) && v.enum.length === 1) literalValue = v.enum[0];else
    return null;
    const variantType = typeof v.type === "string" ? v.type : null;
    if (!variantType) return null;
    if (commonType === null) commonType = variantType;else
    if (commonType !== variantType) return null;
...
function sanitizeRequiredFields(schema) {
  ...
  if (required.length > 0) schema.required = required;else
...
function cleanSchemaForGeminiWithDefs(schema, defs, refStack) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (key === "properties") {if (value && typeof value === "object" && !Array.isArray(value)) cleaned[key] = Object.fromEntries(Object.entries(value).map(([k, v]) => [k, cleanSchemaForGeminiWithDefs(v, nextDefs, refStack)]));else
      cleaned[key] = {};} else
    if (key === "items" && value) {if (Array.isArray(value)) cleaned[key] = value.map((entry) => cleanSchemaForGeminiWithDefs(entry, nextDefs, refStack));else
      if (typeof value === "object") cleaned[key] = cleanSchemaForGeminiWithDefs(value, nextDefs, refStack);else
      cleaned[key] = value;} else
    if (key === "anyOf" && Array.isArray(value)) cleaned[key] = cleanedAnyOf ?? value.map((variant) => cleanSchemaForGeminiWithDefs(variant, nextDefs, refStack));else
    if (key === "oneOf" && Array.isArray(value)) cleaned[key] = cleanedOneOf ?? value.map((variant) => cleanSchemaForGeminiWithDefs(variant, nextDefs, refStack));else
    if (key === "allOf" && Array.isArray(value)) cleaned[key] = value.map((variant) => cleanSchemaForGeminiWithDefs(variant, nextDefs, refStack));else
    cleaned[key] = value;
  }
...
const XAI_TOOL_SCHEMA_PROFILE = exports.n = "xai";
const HTML_ENTITY_TOOL_CALL_ARGUMENTS_ENCODING = exports.t = "html-entities";
const XAI_UNSUPPORTED_SCHEMA_KEYWORDS = exports.r = new Set([
"minLength",
"maxLength",
"minItems",
"maxItems",
...
function applyXaiModelCompat(model) {
  return (0, _providerModelCompatDsxuyzi.t)(model, resolveXaiModelCompatPatch());
}
...
function findOpenAIStrictSchemaViolations(schema, path, options) {
  ...
  if (record.type === "object") {
    ...
    if (!required) violations.push(`${path}.required`);else
    if (properties) {
