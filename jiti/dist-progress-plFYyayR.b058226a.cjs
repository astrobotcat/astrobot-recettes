"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = withProgress;exports.r = withProgressTotals;exports.t = createCliProgress;var _progressLineXDw3PVgH = require("./progress-line-xDw3PVgH.js");
...
function createCliProgress(options) {
  ...
  activeProgress += 1;
  if (isTty) (0, _progressLineXDw3PVgH.n)(stream);
  const controller = canOsc ? (0, _oscProgress.createOscProgressController)({
    env: process.env,
    isTty: stream.isTTY,
  ...
  const renderLine = allowLine ? () => {
    ...
    stream.write(`${_themeD5sxSdHD.r.accent(label)}${suffix}`);
  } : null;
  ...
  const applyState = () => {
    if (!started) return;
    if (controller) if (indeterminate) controller.setIndeterminate(label);else
    controller.setPercent(label, percent);
    if (spin) spin.message(_themeD5sxSdHD.r.accent(label));
  ...
  const start = () => {
    if (started) return;
    started = true;
    if (spin) spin.start(_themeD5sxSdHD.r.accent(label));
    applyState();
  };
  if (delayMs === 0) start();else
  ...
  const done = () => {
    ...
    if (spin) spin.stop();
    (0, _progressLineXDw3PVgH.t)();
    if (isTty) (0, _progressLineXDw3PVgH.r)(stream);
    activeProgress = Math.max(0, activeProgress - 1);
  };
  return {
    setLabel,
