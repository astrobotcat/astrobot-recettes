"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.C = isTimeoutErrorMessage;exports.D = parseExecApprovalResultText;exports.E = isExecDeniedResultText;exports.S = isServerErrorMessage;exports.T = formatExecDeniedUserMessage;exports._ = isAuthPermanentErrorMessage;exports.a = formatTransportErrorCopy;exports.b = isPeriodicUsageLimitErrorMessage;exports.c = isLikelyHttpErrorText;exports.d = stableStringify;exports.f = void 0;exports.g = isAuthErrorMessage;exports.h = hasInternalRuntimeContext;exports.i = formatRateLimitOrOverloadedErrorCopy;exports.l = isRawApiErrorPayload;exports.m = escapeInternalRuntimeContextDelimiters;exports.n = formatBillingErrorMessage;exports.o = getApiErrorPayloadFingerprint;exports.p = void 0;exports.r = formatDiskSpaceErrorCopy;exports.s = isInvalidStreamingEventOrderError;exports.t = void 0;exports.u = sanitizeUserFacingText;exports.v = isBillingErrorMessage;exports.w = matchesFormatErrorPattern;exports.x = isRateLimitErrorMessage;exports.y = isOverloadedErrorMessage;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function formatExecDeniedUserMessage(resultText) {
  ...
  const metadata = (0, _stringCoerceBUSzWgUA.i)(parsed.metadata);
...
const INTERNAL_RUNTIME_CONTEXT_BEGIN = exports.f = "<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>";
const INTERNAL_RUNTIME_CONTEXT_END = exports.p = "<<<END_OPENCLAW_INTERNAL_CONTEXT>>>";
...
const BILLING_ERROR_USER_MESSAGE = exports.t = formatBillingErrorMessage();
...
function extractProviderRateLimitMessage(raw) {
  ...
  const candidate = ((0, _assistantErrorFormatTQ4SCN8S.i)(raw) ?? (0, _assistantErrorFormatTQ4SCN8S.i)(withoutPrefix))?.message ?? ((0, _assistantErrorFormatTQ4SCN8S.t)(withoutPrefix)?.rest || withoutPrefix);
  if (!candidate || !RATE_LIMIT_SPECIFIC_HINT_RE.test(candidate)) return;
  if ((0, _assistantErrorFormatTQ4SCN8S.r)(withoutPrefix)) return;
...
function formatTransportErrorCopy(raw) {
  if (!raw) return;
  if ((0, _assistantErrorFormatTQ4SCN8S.r)(raw)) return;
...
function getApiErrorPayloadFingerprint(raw) {
  if (!raw) return null;
  const payload = (0, _assistantErrorFormatTQ4SCN8S.a)(raw);
...
function isLikelyProviderErrorType(type) {
  const normalized = (0, _stringCoerceBUSzWgUA.o)(type);
...
function shouldRewriteRawPayloadWithoutErrorContext(raw) {
  ...
  const info = (0, _assistantErrorFormatTQ4SCN8S.i)(raw);
...
function isLikelyHttpErrorText(raw) {
  if ((0, _assistantErrorFormatTQ4SCN8S.r)(raw)) return true;
  const status = (0, _assistantErrorFormatTQ4SCN8S.t)(raw);
  if (!status) return false;
  if (status.code < 400) return false;
  const message = (0, _stringCoerceBUSzWgUA.i)(status.rest);
...
function sanitizeUserFacingText(text, opts) {
  ...
  if (!trimmed) return "";
  if (!errorContext && shouldRewriteRawPayloadWithoutErrorContext(trimmed)) return (0, _assistantErrorFormatTQ4SCN8S.n)(trimmed);
  if (errorContext) {
    ...
    if (isRawApiErrorPayload(trimmed) || isLikelyHttpErrorText(trimmed)) return (0, _assistantErrorFormatTQ4SCN8S.n)(trimmed);
    if (ERROR_PREFIX_RE.test(trimmed)) {
      ...
      return (0, _assistantErrorFormatTQ4SCN8S.n)(trimmed);
    }
  }
  return collapseConsecutiveDuplicateBlocks(stripped.replace(/^(?:[ \t]*\r?\n)+/, ""));
}
//#endregion /* v9-f5a9c3876946685e */
