"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = applyAnthropicPayloadPolicyToParams;exports.r = resolveAnthropicPayloadPolicy;exports.t = applyAnthropicEphemeralCacheControlMarkers;var _providerAttributionBJhkfmfB = require("./provider-attribution-BJhkfmfB.js");
...
function applyAnthropicCacheControlToSystem(system, cacheControl) {
  ...
  for (const block of system) {
    ...
    const split = (0, _systemPromptCacheBoundaryNUMoTji.r)(record.text);
    if (!split) {
...
function stripAnthropicSystemPromptBoundary(system) {
  ...
  for (const block of system) {
    ...
    const record = block;
    if (record.type === "text" && typeof record.text === "string") record.text = (0, _systemPromptCacheBoundaryNUMoTji.i)(record.text);
  }
}
...
function resolveAnthropicPayloadPolicy(input) {
  return {
    allowsServiceTier: (0, _providerAttributionBJhkfmfB.r)({
