"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveEffectiveSharedGatewayAuth;exports.i = isLocalDirectRequest;exports.n = authorizeHttpGatewayConnect;exports.o = resolveGatewayAuth;exports.r = authorizeWsControlUiGatewayConnect;exports.s = checkBrowserOrigin;exports.t = assertGatewayAuthConfigured;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _typesSecretsCeL3gSMO = require("./types.secrets-CeL3gSMO.js");
var _credentialsDHNlrJW = require("./credentials-DH-nlrJW.js");
var _tailscaleBNpgHmIw = require("./tailscale-BNpgHmIw.js");
var _secretEqualDqrgJW5g = require("./secret-equal-DqrgJW5g.js");
var _netLBInRHnX = require("./net-lBInRHnX.js");
var _authRateLimitDrZ1TbNS = require("./auth-rate-limit-DrZ1TbNS.js");
...
function parseOrigin(originRaw) {
  ...
  try {
    ...
    return {
      origin: (0, _stringCoerceBUSzWgUA.i)(url.origin),
      host: (0, _stringCoerceBUSzWgUA.i)(url.host),
      hostname: (0, _stringCoerceBUSzWgUA.i)(url.hostname)
    };
  } catch {
    return null;
  }
...
function checkBrowserOrigin(params) {
  ...
  const allowlist = new Set((params.allowedOrigins ?? []).map((value) => (0, _stringCoerceBUSzWgUA.o)(value)).filter(Boolean));
  if (allowlist.has("*") || allowlist.has(parsedOrigin.origin)) return {
    ok: true,
    matchedBy: "allowlist"
  };
  const requestHost = (0, _netLBInRHnX.d)(params.requestHost);
  ...
  if (params.isLocalClient && (0, _netLBInRHnX.a)(parsedOrigin.hostname)) return {
    ok: true,
...
function buildSerializationKey(ip, scope) {
  return `${normalizeScope(scope)}:${(0, _authRateLimitDrZ1TbNS.o)(ip)}`;
}
...
function hasExplicitSharedSecretAuth(connectAuth) {
  return Boolean((0, _stringCoerceBUSzWgUA.s)(connectAuth?.token) || (0, _stringCoerceBUSzWgUA.s)(connectAuth?.password));
}
function normalizeLogin(login) {
  return (0, _stringCoerceBUSzWgUA.i)(login);
}
...
function isLocalDirectRequest(req, _trustedProxies, _allowRealIpFallback = false) {
  if (!req) return false;
  if (!Boolean(req.headers?.forwarded || req.headers?.["x-forwarded-for"] || req.headers?.["x-forwarded-proto"] || req.headers?.["x-real-ip"] || req.headers?.["x-forwarded-host"])) return (0, _netLBInRHnX.i)(req.socket?.remoteAddress);
  return false;
}
function getTailscaleUser(req) {
  if (!req) return null;
  const login = (0, _stringCoerceBUSzWgUA.s)(req.headers["tailscale-user-login"]);
  if (!login) return null;
  const nameRaw = req.headers["tailscale-user-name"];
  const profilePic = req.headers["tailscale-user-profile-pic"];
  return {
    login,
    name: (0, _stringCoerceBUSzWgUA.s)(nameRaw) ?? login,
    profilePic: (0, _stringCoerceBUSzWgUA.s)(profilePic)
  };
}
...
function isTailscaleProxyRequest(req) {
  if (!req) return false;
  return (0, _netLBInRHnX.i)(req.socket?.remoteAddress) && hasTailscaleProxyHeaders(req);
}
...
function resolveGatewayAuth(params) {
  ...
  const tokenRef = (0, _typesSecretsCeL3gSMO.d)({ value: authConfig.token }).ref;
  const passwordRef = (0, _typesSecretsCeL3gSMO.d)({ value: authConfig.password }).ref;
  const resolvedCredentials = (0, _credentialsDHNlrJW.i)({
...
function authorizeTrustedProxy(params) {
  ...
  if (!remoteAddr || !(0, _netLBInRHnX.l)(remoteAddr, trustedProxies)) return { reason: "trusted_proxy_untrusted_source" };
  if ((0, _netLBInRHnX.i)(remoteAddr)) return { reason: "trusted_proxy_loopback_source" };
  const requiredHeaders = trustedProxyConfig.requiredHeaders ?? [];
  for (const header of requiredHeaders) {
    const value = headerValue(req.headers[(0, _stringCoerceBUSzWgUA.i)(header)]);
    if (!value || value.trim() === "") return { reason: `trusted_proxy_missing_header_${header}` };
  }
  const userHeaderValue = headerValue(req.headers[(0, _stringCoerceBUSzWgUA.i)(trustedProxyConfig.userHeader)]);
...
function authorizeTokenAuth(params) {
  ...
  if (!(0, _secretEqualDqrgJW5g.t)(params.connectToken, params.authToken)) {
...
async function authorizeGatewayConnect(params) {
  ...
  const ip = params.clientIp ?? (0, _netLBInRHnX._)(req, trustedProxies, params.allowRealIpFallback === true) ?? req?.socket?.remoteAddress;
...
async function authorizeGatewayConnectCore(params) {
  ...
  const tailscaleWhois = params.tailscaleWhois ?? _tailscaleBNpgHmIw.s;
  const authSurface = params.authSurface ?? "http";
  ...
  if (auth.mode === "password") {
    ...
    if (!(0, _secretEqualDqrgJW5g.t)(password, auth.password)) {
