"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveToolsBySender;exports.n = resolveChannelGroupRequireMention;exports.r = resolveChannelGroupToolsPolicy;exports.t = resolveChannelGroupPolicy;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _accountIdJ7GeQlaZ = require("./account-id-j7GeQlaZ.js");
var _typesToolsCdFillif = require("./types.tools-CdFillif.js");
...
function resolveChannelGroupConfig(groups, groupId, caseInsensitive = false) {
  ...
  const target = (0, _stringCoerceBUSzWgUA.i)(groupId);
  const matchedKey = Object.keys(groups).find((key) => key !== "*" && (0, _stringCoerceBUSzWgUA.i)(key) === target);
...
function normalizeSenderKey(value, options = {}) {
  ...
  if (!trimmed) return "";
  return (0, _stringCoerceBUSzWgUA.i)(options.stripLeadingAt && trimmed.startsWith("@") ? trimmed.slice(1) : trimmed);
}
...
function parseSenderPolicyKey(rawKey) {
  ...
  const typed = (0, _typesToolsCdFillif.n)(trimmed);
  if (typed) {
...
function resolveChannelGroups(cfg, channel, accountId) {
  ...
  return (0, _accountLookupZCs8AOJr.t)(channelConfig.accounts, normalizedAccountId)?.groups ?? channelConfig.groups;
}
function resolveChannelGroupPolicyMode(cfg, channel, accountId) {
  const normalizedAccountId = (0, _accountIdJ7GeQlaZ.n)(accountId);
  const channelConfig = cfg.channels?.[channel];
  if (!channelConfig) return;
  return (0, _accountLookupZCs8AOJr.t)(channelConfig.accounts, normalizedAccountId)?.groupPolicy ?? channelConfig.groupPolicy;
}
