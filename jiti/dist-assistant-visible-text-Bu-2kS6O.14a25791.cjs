"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = stripDowngradedToolCallText;exports.c = stripReasoningTagsFromText;exports.d = isInsideCode;exports.i = stripAssistantInternalScaffolding;exports.l = stripModelSpecialTokens;exports.n = sanitizeAssistantVisibleTextWithOptions;exports.o = stripMinimaxToolCallXml;exports.r = sanitizeAssistantVisibleTextWithProfile;exports.s = stripToolCallXmlTags;exports.t = sanitizeAssistantVisibleText;exports.u = findCodeRegions;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function stripModelSpecialTokens(text) {
  ...
  for (const match of text.matchAll(MODEL_SPECIAL_TOKEN_RE)) {
    ...
    if (isInsideCode(start, codeRegions) || overlapsCodeRegion(start, end, codeRegions)) out += matched;else
    if (shouldInsertSeparator(text[start - 1], text[end])) out += " ";
    cursor = end;
  }
...
function parseToolCallTagAt(text, start) {
  ...
  const tagName = (0, _stringCoerceBUSzWgUA.i)(text.slice(nameStart, cursor));
...
function stripToolCallXmlTags(text) {
  ...
  if (!inToolCallBlock) result += text.slice(lastIndex);else
  if (toolCallBlockTagName === "function") result += text.slice(toolCallBlockStart);
  return result;
}
/**
...
function stripDowngradedToolCallText(text) {
  ...
  const stripToolCalls = (input) => {
    ...
    for (const match of input.matchAll(toolCallRe)) {
      ...
      while (index < input.length && (input[index] === " " || input[index] === "	")) index += 1;
      if ((0, _stringCoerceBUSzWgUA.i)(input.slice(index, index + 9)) === "arguments") {
        index += 9;
