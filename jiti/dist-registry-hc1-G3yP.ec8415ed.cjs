"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = registerContextEngineForOwner;exports.r = resolveContextEngine;exports.t = registerContextEngine;var _ansiBs_ZZlnS = require("./ansi-Bs_ZZlnS.js");
...
const contextEngineRegistryState = (0, _globalSingletonB80lDOJ.n)(CONTEXT_ENGINE_REGISTRY_STATE, () => ({ engines: /* @__PURE__ */new Map() }));
...
function describeResolvedContextEngineContractError(engineId, engine) {
  ...
  if (!info || typeof info !== "object") issues.push("missing info");else
  {
    ...
    if (!infoId) issues.push("missing info.id");else
    if (infoId !== engineId) issues.push(`info.id must match registered id "${engineId}"`);
...
async function resolveContextEngine(config) {
  ...
  if (!entry) {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiBs_ZZlnS.t)(engineId)}" is not registered; falling back to default engine "${defaultEngineId}".`);
  ...
  try {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiBs_ZZlnS.t)(engineId)}" factory threw during resolution: ${(0, _ansiBs_ZZlnS.t)(factoryError instanceof Error ? factoryError.message : String(factoryError))}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId);
  }
  let contractError;
  try {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiBs_ZZlnS.t)(engineId)}" contract validation threw: ${(0, _ansiBs_ZZlnS.t)(validationError instanceof Error ? validationError.message : String(validationError))}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId);
  }
  if (contractError) {
    if (isDefaultEngine) throw new Error(contractError);
    console.error(`[context-engine] ${(0, _ansiBs_ZZlnS.t)(contractError)}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId);
  }
