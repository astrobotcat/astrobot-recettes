"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = summarizeAgentEventForWsLog;exports.n = logWs;exports.r = shouldLogWs;exports.t = formatForLog;var _redactD4nea1HF = require("./redact-D4nea1HF.js");
...
var _wsLoggingCnIeNr3I = require("./ws-logging-CnIeNr3I.js");
var _chalk = _interopRequireDefault(require("chalk"));function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
//#region src/gateway/ws-log.ts
const LOG_VALUE_LIMIT = 240;
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const WS_LOG_REDACT_OPTIONS = {
  mode: "tools",
  patterns: (0, _redactD4nea1HF.t)()
};
...
const wsLog = (0, _subsystemCgmckbux.t)("gateway/ws");
...
function collectWsRestMeta(meta) {
  ...
  for (const [key, value] of Object.entries(meta)) {
    ...
    restMeta.push(`${_chalk.default.dim(key)}=${formatForLog(value)}`);
  }
  return restMeta;
}
function buildWsHeadline(params) {
  if ((params.kind === "req" || params.kind === "res") && params.method) return _chalk.default.bold(params.method);
  if (params.kind === "event" && params.event) return _chalk.default.bold(params.event);
}
function buildWsStatusToken(kind, ok) {
  if (kind !== "res" || ok === void 0) return;
  return ok ? _chalk.default.greenBright("✓") : _chalk.default.redBright("✗");
}
...
function shouldLogWs() {
  return (0, _subsystemCgmckbux.f)("gateway/ws");
}
...
function formatForLog(value) {
  try {
    ...
    if (!str) return "";
    const redacted = (0, _redactD4nea1HF.r)(str, WS_LOG_REDACT_OPTIONS);
...
function summarizeAgentEventForWsLog(payload) {
  ...
  const rec = payload;
  const runId = (0, _stringCoerceBUSzWgUA.d)(rec.runId);
  const stream = (0, _stringCoerceBUSzWgUA.d)(rec.stream);
  const seq = typeof rec.seq === "number" ? rec.seq : void 0;
  const sessionKey = (0, _stringCoerceBUSzWgUA.d)(rec.sessionKey);
  ...
  if (stream === "assistant") {
    const text = (0, _stringCoerceBUSzWgUA.d)(data.text);
    if (text?.trim()) extra.text = compactPreview(text);
    const mediaCount = (0, _replyPayloadDb_8BQiX.p)({ mediaUrls: Array.isArray(data.mediaUrls) ? data.mediaUrls : void 0 }).mediaCount;
    if (mediaCount > 0) extra.media = mediaCount;
    return extra;
  }
  if (stream === "tool") {
    const phase = (0, _stringCoerceBUSzWgUA.d)(data.phase);
    const name = (0, _stringCoerceBUSzWgUA.d)(data.name);
    if (phase || name) extra.tool = `${phase ?? "?"}:${name ?? "?"}`;
    const toolCallId = (0, _stringCoerceBUSzWgUA.d)(data.toolCallId);
    if (toolCallId) extra.call = shortId(toolCallId);
    const meta = (0, _stringCoerceBUSzWgUA.d)(data.meta);
...
function logWs(direction, kind, meta) {
  if (!(0, _subsystemCgmckbux.f)("gateway/ws")) return;
  const style = (0, _wsLoggingCnIeNr3I.t)();
  ...
  const prefix = `${(direction === "in" ? _chalk.default.greenBright : _chalk.default.cyanBright)(dirArrow)} ${_chalk.default.bold(kind)}`;
  ...
  const durationToken = typeof durationMs === "number" ? _chalk.default.dim(`${durationMs}ms`) : void 0;
  const restMeta = collectWsRestMeta(meta);
  const trailing = [];
  if (connId) trailing.push(`${_chalk.default.dim("conn")}=${_chalk.default.gray(shortId(connId))}`);
  if (id) trailing.push(`${_chalk.default.dim("id")}=${_chalk.default.gray(shortId(id))}`);
  logWsInfoLine({
    prefix,
    statusToken,
    headline,
...
function logWsOptimized(direction, kind, meta) {
  ...
  if (kind === "parse-error") {
    ...
    wsLog.warn([
    `${_chalk.default.redBright("✗")} ${_chalk.default.bold("parse-error")}`,
    errorMsg ? `${_chalk.default.dim("error")}=${errorMsg}` : void 0,
    `${_chalk.default.dim("conn")}=${_chalk.default.gray(shortId(connId ?? "?"))}`].
  ...
  const restMeta = collectWsRestMeta(meta);
  logWsInfoLine({
    prefix: `${_chalk.default.yellowBright("⇄")} ${_chalk.default.bold("res")}`,
    statusToken,
    headline: method ? _chalk.default.bold(method) : void 0,
    durationToken,
    restMeta,
    trailing: [connId ? `${_chalk.default.dim("conn")}=${_chalk.default.gray(shortId(connId))}` : "", id ? `${_chalk.default.dim("id")}=${_chalk.default.gray(shortId(id))}` : ""].filter(Boolean)
  });
}
function logWsCompact(direction, kind, meta) {
  ...
  const prefix = `${(kind === "req" || kind === "res" ? _chalk.default.yellowBright : direction === "in" ? _chalk.default.greenBright : _chalk.default.cyanBright)(compactArrow)} ${_chalk.default.bold(kind)}`;
  const statusToken = buildWsStatusToken(kind, ok);
  ...
  const durationToken = typeof startedAt === "number" ? _chalk.default.dim(`${now - startedAt}ms`) : void 0;
  const headline = buildWsHeadline({
    kind,
    method,
  ...
  if (connId && connId !== wsLastCompactConnId) {
    trailing.push(`${_chalk.default.dim("conn")}=${_chalk.default.gray(shortId(connId))}`);
