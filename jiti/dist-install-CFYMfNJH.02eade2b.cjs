"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolvePluginInstallDir;exports.i = installPluginFromPath;exports.n = installPluginFromArchive;exports.r = installPluginFromNpmSpec;exports.t = void 0;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _utilsD5DtWkEu = require("./utils-D5DtWkEu.js");
var _manifestDKZWfJEu = require("./manifest-DKZWfJEu.js");
var _installSafePathT8fz8O = require("./install-safe-path-T8fz8O29.js");
...
async function loadPluginInstallRuntime() {
  pluginInstallRuntimePromise ??= Promise.resolve().then(() => jitiImport("./install.runtime-B13jZink.js").then((m) => _interopRequireWildcard(m)));
...
const PLUGIN_INSTALL_ERROR_CODE = exports.t = {
...
function safeFileName(input) {
  return (0, _installSafePathT8fz8O.i)(input);
}
function encodePluginInstallDirName(pluginId) {
  const trimmed = pluginId.trim();
  if (!trimmed.includes("/")) return (0, _installSafePathT8fz8O.i)(trimmed);
  return `@${(0, _installSafePathT8fz8O.a)(trimmed)}`;
}
...
function matchesExpectedPluginId(params) {
  ...
  return !params.manifestPluginId && params.pluginId === params.npmPluginId && params.expectedPluginId === (0, _installSafePathT8fz8O.o)(params.npmPluginId);
}
function ensureOpenClawExtensions(params) {
  const resolved = (0, _manifestDKZWfJEu.a)(params.manifest);
...
function buildFileInstallResult(pluginId, targetFile) {
  return {
    ...
    manifestName: void 0,
    version: void 0,
    extensions: [_nodePath.default.basename(targetFile)]
  };
}
...
function resolvePluginInstallDir(pluginId, extensionsDir) {
  const extensionsBase = extensionsDir ? (0, _utilsD5DtWkEu.m)(extensionsDir) : _nodePath.default.join(_utilsD5DtWkEu.t, "extensions");
  const pluginIdError = validatePluginId(pluginId);
  if (pluginIdError) throw new Error(pluginIdError);
  const targetDirResult = (0, _installSafePathT8fz8O.r)({
...
async function resolvePluginInstallTarget(params) {
  const extensionsDir = params.extensionsDir ? (0, _utilsD5DtWkEu.m)(params.extensionsDir) : _nodePath.default.join(_utilsD5DtWkEu.t, "extensions");
...
async function detectNativePackageInstallSource(packageDir) {
  const runtime = await loadPluginInstallRuntime();
  const manifestPath = _nodePath.default.join(packageDir, "package.json");
...
async function installPluginFromPackageDir(params) {
  const runtime = await loadPluginInstallRuntime();
  const { logger, timeoutMs, mode, dryRun } = runtime.resolveTimedInstallModeOptions(params, defaultLogger);
  const manifestPath = _nodePath.default.join(params.packageDir, "package.json");
  ...
  const pkgName = (0, _stringCoerceBUSzWgUA.s)(manifest.name) ?? "";
  ...
  if (manifestPluginId && !(0, _installSafePathT8fz8O.n)(npmPluginId, manifestPluginId)) logger.info?.(`Plugin manifest id "${manifestPluginId}" differs from npm package name "${npmPluginId}"; using manifest id as the config key.`);
  ...
  return await installPluginDirectoryIntoExtensions({
    ...
    afterCopy: async (installedDir) => {
      for (const entry of extensions) {
        const resolvedEntry = _nodePath.default.resolve(installedDir, entry);
...
async function installPluginFromDir(params) {
  const runtime = await loadPluginInstallRuntime();
  const dirPath = (0, _utilsD5DtWkEu.m)(params.dirPath);
  const installPolicyRequest = params.installPolicyRequest ?? {
    kind: "plugin-dir",
  ...
  if (!(await _promises.default.stat(dirPath)).isDirectory()) return {
    ok: false,
...
async function installPluginFromFile(params) {
  ...
  const filePath = (0, _utilsD5DtWkEu.m)(params.filePath);
  const installPolicyRequest = params.installPolicyRequest ?? {
    kind: "plugin-file",
  ...
  const extensionsDir = params.extensionsDir ? (0, _utilsD5DtWkEu.m)(params.extensionsDir) : _nodePath.default.join(_utilsD5DtWkEu.t, "extensions");
  await _promises.default.mkdir(extensionsDir, { recursive: true });
  const pluginId = _nodePath.default.basename(filePath, _nodePath.default.extname(filePath)) || "plugin";
  const pluginIdError = validatePluginId(pluginId);
  if (pluginIdError) return {
    ok: false,
    error: pluginIdError
  };
  const targetFile = _nodePath.default.join(extensionsDir, `${safeFileName(pluginId)}${_nodePath.default.extname(filePath)}`);
  ...
  try {
    await runtime.writeFileFromPathWithinRoot({
      ...
      relativePath: _nodePath.default.basename(preparedTarget.targetPath),
      sourcePath: filePath
    });
  } catch (err) {
    return {
