"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = shouldApplyStartupContext;exports.r = buildBareSessionResetPrompt;exports.t = buildSessionStartupContextPrelude;var _boundaryFileReadDXLy_w6L = require("./boundary-file-read-DXLy_w6L.js");
var _dateTimeCH5CpFpu = require("./date-time-CH5CpFpu.js");
var _currentTimeDumgMhEz = require("./current-time-DumgMhEz.js");
...
function buildBareSessionResetPrompt(cfg, nowMs) {
  return (0, _currentTimeDumgMhEz.t)(BARE_SESSION_RESET_PROMPT_BASE, cfg ?? {}, nowMs ?? Date.now());
}
//#endregion
...
async function readFromFd(params) {
  ...
  const bytesRead = await new Promise((resolve, reject) => {
    _nodeFs.default.read(params.fd, buf, 0, params.maxFileBytes, 0, (error, read) => {
      if (error) {
        reject(error);
        return;
      }
...
async function closeFd(fd) {
  await new Promise((resolve, reject) => {
    _nodeFs.default.close(fd, (error) => {
      if (error) {
        reject(error);
        return;
      }
...
async function readStartupMemoryFile(params) {
  const opened = await (0, _boundaryFileReadDXLy_w6L.r)({
    absolutePath: _nodePath.default.join(params.workspaceDir, params.relativePath),
...
async function buildSessionStartupContextPrelude(params) {
  ...
  const timezone = (0, _dateTimeCH5CpFpu.i)(params.cfg?.agents?.defaults?.userTimezone);
