"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = withResolvedTelegramForumFlag;exports.B = resolveTelegramPrimaryMedia;exports.C = resolveTelegramDirectPeerId;exports.D = resolveTelegramReplyId;exports.E = resolveTelegramGroupAllowFromContext;exports.F = getTelegramTextParts;exports.H = exports.G = void 0;exports.I = hasBotMention;exports.L = isBinaryContent;exports.M = buildSenderName;exports.N = expandTextLinks;exports.O = resolveTelegramStreamMode;exports.P = extractTelegramLocation;exports.R = normalizeForwardedContext;exports.S = extractTelegramForumFlag;exports.T = resolveTelegramForumThreadId;exports.U = void 0;Object.defineProperty(exports, "V", { enumerable: true, get: function () {return _allowFrom.firstDefined;} });exports.W = void 0;exports._ = buildTelegramParentPeer;exports.a = splitTelegramHtmlChunks;exports.b = buildTypingThreadParams;exports.c = isSafeToRetrySendError;exports.d = isTelegramRateLimitError;exports.f = isTelegramServerError;exports.g = buildTelegramGroupPeerId;exports.h = buildTelegramGroupFrom;exports.i = renderTelegramHtmlText;exports.j = buildSenderLabel;exports.k = resolveTelegramThreadSpec;exports.l = isTelegramClientRejection;exports.m = buildGroupLabel;exports.n = markdownToTelegramHtml;exports.o = wrapFileReferencesInHtml;exports.p = tagTelegramNetworkError;exports.r = markdownToTelegramHtmlChunks;exports.s = isRecoverableTelegramNetworkError;exports.t = markdownToTelegramChunks;exports.u = isTelegramPollingNetworkError;exports.v = buildTelegramRoutingTarget;exports.w = resolveTelegramForumFlag;exports.x = describeReplyTarget;exports.y = buildTelegramThreadParams;exports.z = resolveTelegramMediaPlaceholder;var _outboundParamsDPqRaEDE = require("./outbound-params-DPqRaEDE.js");
var _previewStreamingDHVltX4_ = require("./preview-streaming-DHVltX4_.js");
...
var _allowFrom = require("openclaw/plugin-sdk/allow-from");
...
const normalizeAllowFrom = (list) => {
  const entries = (list ?? []).map((value) => (0, _textRuntime.normalizeOptionalString)(String(value)) ?? "").filter(Boolean);
...
};exports.U = normalizeAllowFrom;
const normalizeDmAllowFromWithStore = (params) => normalizeAllowFrom((0, _allowFrom.mergeDmAllowFromSources)(params));exports.W = normalizeDmAllowFromWithStore;
const isSenderAllowed = (params) => {
  const { allow, senderId } = params;
  return (0, _allowFrom.isSenderIdAllowed)(allow, senderId, true);
};exports.H = isSenderAllowed;
...
exports.G = resolveSenderAllowMatch;function buildSenderName(msg) {
...
function buildSenderLabel(msg, senderId) {
  ...
  let label = name;
  if (name && username) label = `${name} (${username})`;else
  if (!name && username) label = username;
  const fallbackId = (senderId != null ? (0, _textRuntime.normalizeOptionalString)(String(senderId)) : void 0) ?? (msg.from?.id != null ? String(msg.from.id) : void 0);
...
function hasBotMention(msg, botUsername) {
  ...
  const mention = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(`@${botUsername}`);
  if (hasStandaloneTelegramMention((0, _textRuntime.normalizeLowercaseStringOrEmpty)(text), mention)) return true;
  for (const ent of entities) {
    if (ent.type !== "mention") continue;
    if ((0, _textRuntime.normalizeLowercaseStringOrEmpty)(text.slice(ent.offset, ent.offset + ent.length)) === mention) return true;
  }
  return false;
}
...
function normalizeForwardedUserLabel(user) {
  ...
  const username = (0, _textRuntime.normalizeOptionalString)(user.username);
...
function normalizeForwardedChatLabel(chat, fallbackKind) {
  const title = (0, _textRuntime.normalizeOptionalString)(chat.title);
  const username = (0, _textRuntime.normalizeOptionalString)(chat.username);
...
function buildForwardedContextFromChat(params) {
  ...
  if (!display) return null;
  const signature = (0, _textRuntime.normalizeOptionalString)(params.signature);
  const from = signature ? `${display} (${signature})` : display;
  const chatType = (0, _textRuntime.normalizeOptionalString)(params.chat.type);
  return {
    from,
    date: params.date,
    fromType: params.type,
...
async function resolveTelegramGroupAllowFromContext(params) {
  ...
  const storeAllowFrom = await (params.readChannelAllowFromStore ?? _conversationRuntime.readChannelAllowFromStore)("telegram", process.env, accountId).catch(() => []);
  const { groupConfig, topicConfig } = params.resolveTelegramGroupConfig(params.chatId, threadIdForConfig);
  const groupAllowOverride = (0, _allowFrom.firstDefined)(topicConfig?.allowFrom, groupConfig?.allowFrom);
  return {
    resolvedThreadId,
    dmThreadId,
    storeAllowFrom,
...
function resolveTelegramStreamMode(telegramCfg) {
  return (0, _previewStreamingDHVltX4_.t)(telegramCfg);
}
...
function resolveTelegramDirectPeerId(params) {
  const senderId = params.senderId != null ? (0, _textRuntime.normalizeOptionalString)(String(params.senderId)) ?? "" : "";
...
function resolveTelegramReplyId(raw) {
  return (0, _outboundParamsDPqRaEDE.t)(raw);
}
  ...
  if (!body && replyLike) {
    ...
    if (!body) {
      ...
      if (!body) {
        ...
        if (locationData) body = (0, _channelInbound.formatLocationText)(locationData);
      }
    }
  }
...
function collectTelegramErrorCandidates(err) {
  return (0, _errorRuntime.collectErrorGraphCandidates)(err, (current) => {
    ...
    if ((0, _errorRuntime.readErrorName)(current) === "HttpError") nested.push(current.error);
    return nested;
  });
}
...
function normalizeTelegramNetworkMethod(method) {
  ...
  if (!trimmed) return null;
  return (0, _textRuntime.normalizeLowercaseStringOrEmpty)(trimmed);
}
...
function isRecoverableTelegramNetworkError(err, options = {}) {
  ...
  for (const candidate of collectTelegramErrorCandidates(err)) {
    ...
    const message = (0, _textRuntime.normalizeLowercaseStringOrEmpty)((0, _errorRuntime.formatErrorMessage)(candidate));
...
function buildTelegramLink(link, text) {
  ...
  if ((0, _textRuntime.isAutoLinkedFileRef)(href, text.slice(link.start, link.end))) return null;
...
function renderTelegramHtml(ir) {
  return (0, _textRuntime.renderMarkdownWithMarkers)(ir, {
    styleMarkers: {
      bold: {
        open: "<b>",
        close: "</b>"
...
function markdownToTelegramHtml(markdown, options = {}) {
  const html = renderTelegramHtml((0, _textRuntime.markdownToIR)(markdown ?? "", {
    linkify: true,
    enableSpoilers: true,
    headingStyle: "none",
    blockquotePrefix: "",
...
function getFileReferencePattern() {
  ...
  const fileExtensionsPattern = Array.from(_textRuntime.FILE_REF_EXTENSIONS_WITH_TLD).map(escapeRegex).join("|");
...
function wrapFileReferencesInHtml(html) {
  ...
  const deLinkified = html.replace(AUTO_LINKED_ANCHOR_PATTERN, (_match, label) => {
    if (!(0, _textRuntime.isAutoLinkedFileRef)(`http://${label}`, label)) return _match;
  ...
  while ((match = HTML_TAG_PATTERN.exec(deLinkified)) !== null) {
    ...
    const tagName = (0, _textRuntime.normalizeLowercaseStringOrEmpty)(match[2]);
    ...
    if (tagName === "code") codeDepth = isClosing ? Math.max(0, codeDepth - 1) : codeDepth + 1;else
    if (tagName === "pre") preDepth = isClosing ? Math.max(0, preDepth - 1) : preDepth + 1;else
    if (tagName === "a") anchorDepth = isClosing ? Math.max(0, anchorDepth - 1) : anchorDepth + 1;
...
function splitTelegramHtmlChunks(html, limit) {
  ...
  while ((match = HTML_TAG_PATTERN.exec(html)) !== null) {
    ...
    if (isClosing) popTelegramHtmlTag(openTags, tagName);else
    if (!isSelfClosing) openTags.push({
      name: tagName,
      openTag: rawTag,
      closeTag: `</${tagName}>`
    });
...
function renderTelegramChunksWithinHtmlLimit(ir, limit) {
  return (0, _textRuntime.renderMarkdownIRChunksWithinLimit)({
    ir,
    limit,
...
function markdownToTelegramChunks(markdown, limit, options = {}) {
  return renderTelegramChunksWithinHtmlLimit((0, _textRuntime.markdownToIR)(markdown ?? "", {
    linkify: true,
    enableSpoilers: true,
    headingStyle: "none",
    blockquotePrefix: "",
