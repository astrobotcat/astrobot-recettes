"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = void 0;exports.c = resolveEnableState;exports.i = normalizePluginId;exports.l = resolveMemorySlotDecision;exports.n = createPluginActivationSource;exports.o = resolveEffectiveEnableState;exports.r = isTestDefaultMemorySlotDisabled;exports.s = resolveEffectivePluginActivationState;exports.t = applyTestPluginDefaults;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function getBundledPluginAliasLookup() {
  ...
  for (const plugin of (0, _manifestRegistryBd3A4lqx.t)({ cache: true }).plugins) {
    if (plugin.origin !== "bundled") continue;
    const pluginId = (0, _stringCoerceBUSzWgUA.o)(plugin.id);
    if (pluginId) lookup.set(pluginId, plugin.id);
    for (const providerId of plugin.providers) {
      const normalizedProviderId = (0, _stringCoerceBUSzWgUA.o)(providerId);
      if (normalizedProviderId) lookup.set(normalizedProviderId, plugin.id);
    }
    for (const legacyPluginId of plugin.legacyPluginIds ?? []) {
      const normalizedLegacyPluginId = (0, _stringCoerceBUSzWgUA.o)(legacyPluginId);
...
function normalizePluginId(id) {
  const trimmed = (0, _stringCoerceBUSzWgUA.s)(id) ?? "";
  const normalized = (0, _stringCoerceBUSzWgUA.o)(trimmed) ?? "";
...
const normalizePluginsConfig = (config) => {
  return (0, _manifestRegistryBd3A4lqx.f)(config, normalizePluginId);
};exports.a = normalizePluginsConfig;
...
const hasExplicitPluginConfig = (plugins) => (0, _manifestRegistryBd3A4lqx.u)(plugins);
...
function resolveEnableState(id, origin, config, enabledByDefault) {
  return (0, _manifestRegistryBd3A4lqx.m)({
    id,
    origin,
    config,
    enabledByDefault
...
function isBundledChannelEnabledByChannelConfig(cfg, pluginId) {
  return (0, _manifestRegistryBd3A4lqx.d)(cfg, pluginId);
}
function resolveEffectiveEnableState(params) {
  return (0, _manifestRegistryBd3A4lqx.p)(params, resolveEffectivePluginActivationState);
}
function resolveEffectivePluginActivationState(params) {
  return resolvePluginActivationState(params);
}
function resolveMemorySlotDecision(params) {
  return (0, _manifestRegistryBd3A4lqx.h)(params);
}
//#endregion /* v9-9516d6a80429438e */
