"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = unwrapKnownShellMultiplexerInvocation;exports.C = exports.B = void 0;exports.D = isShellWrapperExecutable;exports.E = hasEnvManipulationBeforeShellWrapper;exports.F = isDispatchWrapperExecutable;exports.H = resolveSafeBinProfiles;exports.I = unwrapDispatchWrappersForResolution;exports.L = unwrapKnownDispatchWrapperInvocation;exports.M = void 0;exports.N = resolveInlineCommandMatch;exports.O = isShellWrapperInvocation;exports.P = extractEnvAssignmentKeysFromDispatchWrappers;exports.R = normalizeExecutableToken;exports.S = void 0;exports.T = extractShellWrapperInlineCommand;exports.V = normalizeSafeBinProfileFixtures;exports._ = resolvePolicyTargetCandidatePath;exports.a = validateSafeBinArgv;exports.b = resolveExecutablePath;exports.c = matchAllowlist;exports.d = resolveApprovalAuditCandidatePath;exports.f = resolveCommandResolution;exports.g = resolvePolicyAllowlistCandidatePath;exports.h = resolveExecutionTargetResolution;exports.i = normalizeTrustedSafeBinDirs;exports.j = void 0;exports.k = resolveShellWrapperTransportArgv;exports.l = parseExecArgvToken;exports.m = resolveExecutionTargetCandidatePath;exports.n = isTrustedSafeBinPath;exports.o = listRiskyConfiguredSafeBins;exports.p = resolveCommandResolutionFromArgv;exports.r = listWritableExplicitTrustedSafeBinDirs;exports.s = normalizeSafeBinName;exports.t = getTrustedSafeBinDirs;exports.u = resolveAllowlistCandidatePath;exports.v = resolvePolicyTargetResolution;exports.w = extractShellWrapperCommand;exports.x = resolveExecWrapperTrustPlan;exports.y = resolveExecutableFromPathEnv;exports.z = void 0;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
const DEFAULT_SAFE_BINS = exports.z = [
"cut",
"uniq",
"head",
"tail",
...
const SAFE_BIN_PROFILES = exports.B = compileSafeBinProfiles({
  jq: {
    maxPositional: 1,
    allowedValueFlags: [
    "--arg",
...
function normalizeSafeBinProfileName(raw) {
  const name = (0, _stringCoerceBUSzWgUA.i)(raw);
...
function normalizeMatchTarget(value) {
  if (process.platform === "win32") return (0, _stringCoerceBUSzWgUA.i)(value.replace(/^\\\\[?.]\\/, "").replace(/\\/g, "/"));
...
function matchesExecAllowlistPattern(pattern, target) {
  ...
  const expanded = trimmed.startsWith("~") ? (0, _homeDirBEqRdfoa.t)(trimmed) : trimmed;
...
function basenameLower(token) {
  ...
  return (0, _stringCoerceBUSzWgUA.i)(win.length < posix.length ? win : posix);
}
...
function scanWrapperInvocation(argv, params) {
  ...
  while (idx < argv.length) {
    ...
    const directive = params.onToken(token, (0, _stringCoerceBUSzWgUA.i)(token));
...
const POSIX_INLINE_COMMAND_FLAGS = exports.j = new Set([
"-lc",
"-c",
"--command"]
);
const POWERSHELL_INLINE_COMMAND_FLAGS = exports.M = new Set([
"-c",
"-command",
"--command",
"-f",
...
const POSIX_SHELL_WRAPPERS = exports.S = new Set(POSIX_SHELL_WRAPPER_NAMES);
new Set(withWindowsExeAliases(WINDOWS_CMD_WRAPPER_NAMES));
const POWERSHELL_WRAPPERS = exports.C = new Set(withWindowsExeAliases(POWERSHELL_WRAPPER_NAMES));
...
function isShellWrapperInvocationInternal(argv, depth) {
  ...
  if (dispatchUnwrap.kind === "unwrapped") return isShellWrapperInvocationInternal(dispatchUnwrap.argv, depth + 1);
  const shellMultiplexerUnwrap = unwrapKnownShellMultiplexerInvocation(argv);
  if (shellMultiplexerUnwrap.kind === "blocked") return false;
  if (shellMultiplexerUnwrap.kind === "unwrapped") return isShellWrapperInvocationInternal(shellMultiplexerUnwrap.argv, depth + 1);
...
function extractCmdInlineCommand(argv) {
  const idx = argv.findIndex((item) => {
    const token = (0, _stringCoerceBUSzWgUA.i)(item);
...
function resolveExecutablePathCandidate(rawExecutable, options) {
  const expanded = rawExecutable.startsWith("~") ? (0, _homeDirBEqRdfoa.t)(rawExecutable, { env: options?.env }) : rawExecutable;
...
function resolveWindowsExecutableExtensions(executable, env) {
  ...
  return ["", ...(env?.PATHEXT ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _stringCoerceBUSzWgUA.i)(ext))];
}
function resolveWindowsExecutableExtSet(env) {
  return new Set((env?.PATHEXT ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _stringCoerceBUSzWgUA.i)(ext)).filter(Boolean));
}
...
function resolveExecutableFromPathEnv(executable, pathEnv, env) {
  const entries = pathEnv.split(_nodePath.default.delimiter).filter(Boolean);
...
function matchArgPattern(argPattern, argv, platform) {
  ...
  try {
    ...
    if ((0, _stringCoerceBUSzWgUA.i)(platform ?? process.platform).startsWith("win")) {
...
function matchAllowlist(entries, resolution, argv, platform) {
  ...
  const useArgPattern = (0, _stringCoerceBUSzWgUA.i)(platform ?? process.platform).startsWith("win");
...
function normalizeSafeBinName(raw) {
  const trimmed = (0, _stringCoerceBUSzWgUA.i)(raw);
  if (!trimmed) return "";
...
function isTrustedSafeBinPath(params) {
  ...
  const resolvedDir = _nodePath.default.dirname(_nodePath.default.resolve(params.resolvedPath));
