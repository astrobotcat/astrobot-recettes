"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = getPreauthHandshakeTimeoutMsFromEnv;exports.i = resolveDeviceAuthConnectErrorDetailCode;exports.n = void 0;exports.o = buildDeviceAuthPayload;exports.r = resolveAuthConnectErrorDetailCode;exports.s = buildDeviceAuthPayloadV3;exports.t = void 0;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _deviceAuthTzV3Kb = require("./device-auth-tzV3Kb-2.js");
var _zodParseEztZP0fF = require("./zod-parse-EztZP0fF.js");
var _deviceIdentityTBOlRcQx = require("./device-identity-TBOlRcQx.js");
var _fingerprintDsC0N2Q = require("./fingerprint-Ds-C0N2Q.js");
var _wsBjBhlcWu = require("./ws-BjBhlcWu.js");
var _messageChannelCBqCPFa_ = require("./message-channel-CBqCPFa_.js");
var _deviceMetadataNormalizationUht6PgV = require("./device-metadata-normalization-uht6PgV2.js");
var _protocolC6T5DFc = require("./protocol-C6T5DFc8.js");
var _nodeFs = _interopRequireDefault(require("node:fs"));
var _nodePath = _interopRequireDefault(require("node:path"));
var _nodeCrypto = require("node:crypto");
var _zod = require("zod");
var _ws = require("ws");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
function loadDeviceAuthTokenFromStore(params) {
  ...
  const role = (0, _deviceAuthTzV3Kb.t)(params.role);
...
function storeDeviceAuthTokenInStore(params) {
  ...
  const entry = {
    token: params.token,
    role,
    scopes: (0, _deviceAuthTzV3Kb.n)(params.scopes),
...
const DeviceAuthStoreSchema = _zod.z.object({
  version: _zod.z.literal(1),
  deviceId: _zod.z.string(),
  tokens: _zod.z.record(_zod.z.string(), _zod.z.unknown())
});
function resolveDeviceAuthPath(env = process.env) {
  return _nodePath.default.join((0, _pathsDvv9VRAc._)(env), "identity", DEVICE_AUTH_FILE);
}
function readStore(filePath) {
  try {
    if (!_nodeFs.default.existsSync(filePath)) return null;
    return (0, _zodParseEztZP0fF.t)(DeviceAuthStoreSchema, _nodeFs.default.readFileSync(filePath, "utf8"));
  } catch {
    return null;
  }
}
...
function buildDeviceAuthPayloadV3(params) {
  const scopes = params.scopes.join(",");
  const token = params.token ?? "";
  const platform = (0, _deviceMetadataNormalizationUht6PgV.t)(params.platform);
  const deviceFamily = (0, _deviceMetadataNormalizationUht6PgV.t)(params.deviceFamily);
  return [
  "v3",
  params.deviceId,
  params.clientId,
...
const ConnectErrorDetailCodes = exports.n = {
...
function readConnectErrorRecoveryAdvice(details) {
  ...
  const normalizedNextStep = (0, _stringCoerceBUSzWgUA.s)(raw.recommendedNextStep) ?? "";
  return {
...
var GatewayClient = class {
  constructor(opts) {
    ...
    this.opts = {
      ...opts,
      deviceIdentity: opts.deviceIdentity === null ? void 0 : opts.deviceIdentity ?? (0, _deviceIdentityTBOlRcQx.n)()
    };
  ...
  start() {
    ...
    if (!(0, _netLBInRHnX.c)(url, { allowPrivateWs })) {
      let displayHost = url;
      try {
    ...
    if (url.startsWith("wss://") && this.opts.tlsFingerprint) {
      ...
      wsOptions.checkServerIdentity = (_host, cert) => {
        ...
        const fingerprint = (0, _fingerprintDsC0N2Q.t)(typeof fingerprintValue === "string" ? fingerprintValue : "");
        const expected = (0, _fingerprintDsC0N2Q.t)(this.opts.tlsFingerprint ?? "");
        if (!expected) return;
    ...
    const ws = new _ws.WebSocket(url, wsOptions);
    this.ws = ws;
    this.socketOpened = false;
    this.connectNonce = null;
    this.connectSent = false;
    ...
    ws.on("message", (data) => this.handleMessage((0, _wsBjBhlcWu.t)(data)));
    ws.on("close", (code, reason) => {
      const reasonText = (0, _wsBjBhlcWu.t)(reason);
      const connectErrorDetailCode = this.pendingConnectErrorDetailCode;
      this.pendingConnectErrorDetailCode = null;
      if (this.ws === ws) this.ws = null;
      this.socketOpened = false;
      this.resolvePendingStop(ws);
      if (code === 1008 && (0, _stringCoerceBUSzWgUA.i)(reasonText).includes("device token mismatch") && !this.opts.token && !this.opts.password && this.opts.deviceIdentity) {
        ...
        try {
          ...
          (0, _loggerBA_TvTc.t)(`cleared stale device-auth token for device ${deviceId}`);
        } catch (err) {
          (0, _loggerBA_TvTc.t)(`failed clearing stale device-auth token for device ${deviceId}: ${String(err)}`);
        }
      }
    ...
    ws.on("error", (err) => {
      (0, _loggerBA_TvTc.t)(`gateway client error: ${String(err)}`);
  ...
  sendConnect() {
    ...
    const nonce = (0, _stringCoerceBUSzWgUA.s)(this.connectNonce) ?? "";
    if (!nonce) {
    ...
    const device = (() => {
      ...
      const payload = buildDeviceAuthPayloadV3({
        ...
        clientId: this.opts.clientName ?? _messageChannelCBqCPFa_.g.GATEWAY_CLIENT,
        clientMode: this.opts.mode ?? _messageChannelCBqCPFa_.h.BACKEND,
        role,
        scopes,
        signedAtMs,
      ...
      const signature = (0, _deviceIdentityTBOlRcQx.a)(this.opts.deviceIdentity.privateKeyPem, payload);
      return {
        id: this.opts.deviceIdentity.deviceId,
        publicKey: (0, _deviceIdentityTBOlRcQx.i)(this.opts.deviceIdentity.publicKeyPem),
        signature,
        signedAt: signedAtMs,
        nonce
      };
    ...
    const params = {
      ...
      client: {
        id: this.opts.clientName ?? _messageChannelCBqCPFa_.g.GATEWAY_CLIENT,
        displayName: this.opts.clientDisplayName,
        version: this.opts.clientVersion ?? _versionBk5OWRN.n,
        platform,
        deviceFamily: this.opts.deviceFamily,
        mode: this.opts.mode ?? _messageChannelCBqCPFa_.h.BACKEND,
    ...
    this.request("connect", params).then((helloOk) => {
      ...
      if (this.shouldRetryWithStoredDeviceToken({
        error: err,
        explicitGatewayToken: (0, _stringCoerceBUSzWgUA.s)(this.opts.token),
        resolvedDeviceToken,
      ...
      if (this.opts.mode === _messageChannelCBqCPFa_.h.PROBE || isGatewayClientStoppedError(err)) (0, _loggerBA_TvTc.t)(msg);else
      (0, _loggerBA_TvTc.n)(msg);
  ...
  isTrustedDeviceRetryEndpoint() {
    ...
    try {
      ...
      if ((0, _netLBInRHnX.a)(parsed.hostname)) return true;
  ...
  selectConnectAuth(role) {
    const explicitGatewayToken = (0, _stringCoerceBUSzWgUA.s)(this.opts.token);
    const explicitBootstrapToken = (0, _stringCoerceBUSzWgUA.s)(this.opts.bootstrapToken);
    const explicitDeviceToken = (0, _stringCoerceBUSzWgUA.s)(this.opts.deviceToken);
    const authPassword = (0, _stringCoerceBUSzWgUA.s)(this.opts.password);
  ...
  handleMessage(raw) {
    try {
      ...
      if ((0, _protocolC6T5DFc.B)(parsed)) {
        this.lastTick = Date.now();
        const evt = parsed;
      ...
      if ((0, _protocolC6T5DFc.gt)(parsed)) {
        this.lastTick = Date.now();
        ...
        if (parsed.ok) pending.resolve(parsed.payload);else
        pending.reject(new GatewayClientRequestError({
      ...
      (0, _loggerBA_TvTc.t)(`gateway client parse error: ${String(err)}`);
    }
  }
  ...
  armConnectChallengeTimeout() {
    ...
    this.connectTimer = setTimeout(() => {
      if (this.connectSent || this.ws?.readyState !== _ws.WebSocket.OPEN) return;
  ...
  validateTlsFingerprint() {
    ...
    const fingerprint = (0, _fingerprintDsC0N2Q.t)(socket.getPeerCertificate()?.fingerprint256 ?? "");
    if (!fingerprint) return /* @__PURE__ */new Error("gateway tls fingerprint unavailable");
    if (fingerprint !== expected) return /* @__PURE__ */new Error("gateway tls fingerprint mismatch");
    return null;
  }
  async request(method, params, opts) {
    if (!this.ws || this.ws.readyState !== _ws.WebSocket.OPEN) throw new Error("gateway not connected");
    ...
    if (!(0, _protocolC6T5DFc.ht)(frame)) throw new Error(`invalid request frame: ${JSON.stringify(_protocolC6T5DFc.ht.errors, null, 2)}`);
...
//#endregion
exports.t = GatewayClient; /* v9-2c513ec1023b763a */
