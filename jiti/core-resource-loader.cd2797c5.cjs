...
class DefaultResourceLoader {
  ...
  async reload() {
    ...
    for (const p of this.additionalExtensionPaths) {
      if ((0, _paths.isLocalPath)(p) && !(0, _nodeFs.existsSync)(p)) {
    ...
    for (const p of this.additionalSkillPaths) {
      if ((0, _paths.isLocalPath)(p) && !(0, _nodeFs.existsSync)(p) && !this.skillDiagnostics.some((d) => d.path === p)) {
    ...
    for (const p of this.additionalPromptTemplatePaths) {
      if ((0, _paths.isLocalPath)(p) && !(0, _nodeFs.existsSync)(p) && !this.promptDiagnostics.some((d) => d.path === p)) {
    ...
    for (const p of this.additionalThemePaths) {
      if (!(0, _nodeFs.existsSync)(p) && !this.themeDiagnostics.some((d) => d.path === p)) {
...
}exports.DefaultResourceLoader = DefaultResourceLoader; /* v9-c3d123f3ee6af8d1 */
