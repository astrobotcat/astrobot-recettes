"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = loadEmbeddedPiMcpConfig;exports.i = getOrCreateSessionMcpRuntime;exports.n = materializeBundleMcpToolsForRun;exports.o = describeStdioMcpServerLaunchConfig;exports.r = disposeSessionMcpRuntime;exports.s = resolveStdioMcpServerLaunchConfig;exports.t = createBundleMcpToolRuntime;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _redactSensitiveUrlDdVF8_IU = require("./redact-sensitive-url-DdVF8_IU.js");
var _mcpConfigBwEBkG_W = require("./mcp-config-BwEBkG_W.js");
...
var _streamableHttp = require("@modelcontextprotocol/sdk/client/streamableHttp.js");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
function loadEmbeddedPiMcpConfig(params) {
  const bundleMcp = (0, _loaderDYW2PvbF.m)({
    workspaceDir: params.workspaceDir,
    cfg: params.cfg
  });
  const configuredMcp = (0, _mcpConfigBwEBkG_W.n)(params.cfg?.mcp?.servers);
  return {
    mcpServers: {
...
function resolveHttpMcpServerLaunchConfig(raw, options) {
  ...
  try {
    ...
    return {
      ok: false,
      reason: `its url is not a valid URL: ${(0, _redactSensitiveUrlDdVF8_IU.a)(url)}`
    };
  }
  ...
  let headers;
  if (raw.headers !== void 0 && raw.headers !== null) if (!isMcpConfigRecord(raw.headers)) options?.onMalformedHeaders?.(raw.headers);else
  headers = toMcpStringRecord(raw.headers, { onDroppedEntry: options?.onDroppedHeader });
  return {
    ok: true,
    config: {
...
function describeHttpMcpServerLaunchConfig(config) {
  return (0, _redactSensitiveUrlDdVF8_IU.i)(config.url);
}
//#endregion
...
function getRequestedTransport(rawServer) {
  ...
  return (0, _stringCoerceBUSzWgUA.i)(rawServer.transport);
}
function resolveHttpTransportConfig(serverName, rawServer, transportType) {
  const launch = resolveHttpMcpServerLaunchConfig(rawServer, {
    transportType,
    onDroppedHeader: (key) => {
      (0, _loggerBA_TvTc.a)(`bundle-mcp: server "${serverName}": header "${key}" has an unsupported value type and was ignored.`);
    },
    onMalformedHeaders: () => {
      (0, _loggerBA_TvTc.a)(`bundle-mcp: server "${serverName}": "headers" must be a JSON object; the value was ignored.`);
    }
  });
...
function resolveMcpTransportConfig(serverName, rawServer) {
  ...
  if (requestedTransport && requestedTransport !== "sse" && requestedTransport !== "streamable-http") {
    (0, _loggerBA_TvTc.a)(`bundle-mcp: skipped server "${serverName}" because transport "${requestedTransport}" is not supported.`);
    return null;
  }
  ...
  (0, _loggerBA_TvTc.a)(`bundle-mcp: skipped server "${serverName}" because ${stdioLaunch.reason} and ${httpReason}.`);
  return null;
}
//#endregion
...
function attachStderrLogging(serverName, transport) {
  ...
  const onData = (chunk) => {
    const message = (0, _stringCoerceBUSzWgUA.s)(typeof chunk === "string" ? chunk : String(chunk)) ?? "";
    if (!message) return;
    for (const line of message.split(/\r?\n/)) {
      const trimmed = line.trim();
      if (trimmed) (0, _loggerBA_TvTc.t)(`bundle-mcp:${serverName}: ${trimmed}`);
    }
  };
  stderr.on("data", onData);
  return () => {
    if (typeof stderr.off === "function") stderr.off("data", onData);else
    if (typeof stderr.removeListener === "function") stderr.removeListener("data", onData);
  };
}
const fetchWithUndici = async (url, init) => await (0, _undiciRuntimeC_MCkNOw.i)().fetch(url, init);
...
function resolveMcpTransport(serverName, rawServer) {
  ...
  if (resolved.transportType === "streamable-http") return {
    transport: new _streamableHttp.StreamableHTTPClientTransport(new URL(resolved.url), { requestInit: resolved.headers ? { headers: resolved.headers } : void 0 }),
    description: resolved.description,
  ...
  return {
    transport: new _sse.SSEClientTransport(new URL(resolved.url), {
...
function sanitizeServerName(raw, usedNames) {
  ...
  let candidate = base;
  let n = 2;
  while (usedNames.has((0, _stringCoerceBUSzWgUA.i)(candidate))) {
    const suffix = `-${n}`;
    candidate = `${base.slice(0, Math.max(1, TOOL_NAME_MAX_PREFIX - suffix.length))}${suffix}`;
    n += 1;
  }
  usedNames.add((0, _stringCoerceBUSzWgUA.i)(candidate));
  return candidate;
}
...
function normalizeReservedToolNames(names) {
  return new Set(Array.from(names ?? [], (name) => (0, _stringCoerceBUSzWgUA.o)(name)).filter((name) => Boolean(name)));
}
function buildSafeToolName(params) {
  ...
  let n = 2;
  while (params.reservedNames.has((0, _stringCoerceBUSzWgUA.i)(candidate))) {
    const suffix = `-${n}`;
...
function redactErrorUrls(error) {
  return (0, _redactSensitiveUrlDdVF8_IU.a)(String(error));
}
...
function createCatalogFingerprint(servers) {
  return _nodeCrypto.default.createHash("sha1").update(JSON.stringify(servers)).digest("hex");
}
function loadSessionMcpConfig(params) {
  ...
  if (params.logDiagnostics !== false) for (const diagnostic of loaded.diagnostics) (0, _loggerBA_TvTc.a)(`bundle-mcp: ${diagnostic.pluginId}: ${diagnostic.message}`);
  return {
    loaded,
    ...
    catalogInFlight = (async () => {
      ...
      try {
        for (const [serverName, rawServer] of Object.entries(loaded.mcpServers)) {
          ...
          if (safeServerName !== serverName) (0, _loggerBA_TvTc.a)(`bundle-mcp: server key "${serverName}" registered as "${safeServerName}" for provider-safe tool names.`);
          ...
          try {
            ...
            for (const tool of listedTools) {
              ...
              tools.push({
                ...
                toolName,
                title: tool.title,
                description: (0, _stringCoerceBUSzWgUA.s)(tool.description),
                inputSchema: tool.inputSchema,
                fallbackDescription: `Provided by bundle MCP server "${serverName}" (${resolved.description}).`
              });
            }
          } catch (error) {
            if (!disposed) (0, _loggerBA_TvTc.a)(`bundle-mcp: failed to start server "${serverName}" (${resolved.description}): ${redactErrorUrls(error)}`);
...
function getSessionMcpRuntimeManager() {
  return (0, _globalSingletonB80lDOJ.n)(SESSION_MCP_RUNTIME_MANAGER_KEY, createSessionMcpRuntimeManager);
}
...
async function materializeBundleMcpToolsForRun(params) {
  ...
  for (const tool of sortedCatalogTools) {
    ...
    if (safeToolName !== `${tool.safeServerName}__${originalName}`) (0, _loggerBA_TvTc.a)(`bundle-mcp: tool "${tool.toolName}" from server "${tool.serverName}" registered as "${safeToolName}" to keep the tool name provider-safe.`);
    reservedNames.add((0, _stringCoerceBUSzWgUA.i)(safeToolName));
    tools.push({
      name: safeToolName,
...
async function createBundleMcpToolRuntime(params) {
  const runtime = createSessionMcpRuntime({
    sessionId: `bundle-mcp:${_nodeCrypto.default.randomUUID()}`,
    workspaceDir: params.workspaceDir,
    cfg: params.cfg
  });
