...
function collectRuntimeConfigAssignments(params) {
  ...
  (0, _channelSecretBasicRuntime.collectConditionalChannelFieldAssignments)({
    ...
    topLevelInheritedAccountActive: ({ account, enabled }) => {
      ...
      return !(0, _channelSecretBasicRuntime.hasConfiguredSecretInputValue)(account.botToken, params.defaults) && accountTokenFile(account).length === 0;
    },
  ...
  const accountWebhookUrl = (account) => (0, _channelSecretBasicRuntime.hasOwnProperty)(account, "webhookUrl") ? normalizeOptionalString(account.webhookUrl) ?? "" : baseWebhookUrl;
  (0, _channelSecretBasicRuntime.collectConditionalChannelFieldAssignments)({
    channelKey: "telegram",
    field: "webhookSecret",
    channel: telegram,
