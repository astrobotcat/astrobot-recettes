"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = getProcessSupervisor;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _execBAdwyfxI = require("./exec-BAdwyfxI.js");
var _killTreeC8M_XC4G = require("./kill-tree-C8M_XC4G.js");
...
async function createChildAdapter(params) {
  ...
  const child = (await (0, _execBAdwyfxI.s)({
    argv: resolvedArgv,
    options,
  ...
  const kill = (signal) => {
    ...
    if (signal === void 0 || signal === "SIGKILL") {
      if (pid) (0, _killTreeC8M_XC4G.t)(pid);
      try {
        child.kill("SIGKILL");
      } catch {}
...
async function createPtyAdapter(params) {
  const module = await Promise.resolve().then(() => jitiImport("@lydell/node-pty").then((m) => _interopRequireWildcard(m)));
  ...
  const kill = (signal = "SIGKILL") => {
    try {
      if (signal === "SIGKILL" && typeof pty.pid === "number" && pty.pid > 0) (0, _killTreeC8M_XC4G.t)(pty.pid);else
      if (process.platform === "win32") pty.kill();else
      pty.kill(signal);
    } catch {}
...
function createProcessSupervisor() {
  ...
  const spawn = async (input) => {
    const runId = (0, _stringCoerceBUSzWgUA.s)(input.runId) ?? _nodeCrypto.default.randomUUID();
    const scopeKey = (0, _stringCoerceBUSzWgUA.s)(input.scopeKey);
    ...
    try {
      ...
      const { warnProcessSupervisorSpawnFailure } = await Promise.resolve().then(() => jitiImport("./supervisor-log.runtime-Clm6RSB3.js").then((m) => _interopRequireWildcard(m)));
