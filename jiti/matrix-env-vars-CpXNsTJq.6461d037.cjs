"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = listMatrixEnvAccountIds;exports.r = resolveMatrixEnvAccountToken;exports.t = getMatrixScopedEnvVarNames;var _accountId = require("openclaw/plugin-sdk/account-id");
...
function resolveMatrixEnvAccountToken(accountId) {
  return Array.from((0, _accountId.normalizeAccountId)(accountId)).map((char) => /[a-z0-9]/.test(char) ? char.toUpperCase() : `_X${char.codePointAt(0)?.toString(16).toUpperCase() ?? "00"}_`).join("");
}
...
function decodeMatrixEnvAccountToken(token) {
  ...
  const normalized = (0, _accountId.normalizeOptionalAccountId)(decoded);
...
function listMatrixEnvAccountIds(env = process.env) {
  ...
  for (const key of MATRIX_GLOBAL_ENV_KEYS) if (typeof env[key] === "string" && env[key]?.trim()) {
    ids.add((0, _accountId.normalizeAccountId)("default"));
    break;
  }
