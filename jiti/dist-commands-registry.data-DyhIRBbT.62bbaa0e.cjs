"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = getNativeCommandSurfaces;exports.t = getChatCommands;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function normalizeArgValue(value) {
  ...
  let text;
  if (typeof value === "string") text = (0, _stringCoerceBUSzWgUA.s)(value) ?? "";else
  if (typeof value === "number" || typeof value === "boolean" || typeof value === "bigint") text = (0, _stringCoerceBUSzWgUA.s)(String(value)) ?? "";else
  if (typeof value === "symbol") text = (0, _stringCoerceBUSzWgUA.s)(value.toString()) ?? "";else
  if (typeof value === "function") text = (0, _stringCoerceBUSzWgUA.s)(value.toString()) ?? "";else
  text = JSON.stringify(value);
  return text ? text : void 0;
}
function formatActionArgs(values, params) {
  const action = (0, _stringCoerceBUSzWgUA.o)(normalizeArgValue(values.action));
...
function registerAlias(commands, key, ...aliases) {
  ...
  const existing = new Set(command.textAliases.map((alias) => (0, _stringCoerceBUSzWgUA.o)(alias)).filter((alias) => Boolean(alias)));
...
function assertCommandRegistry(commands) {
  ...
  for (const command of commands) {
    ...
    } else if (!nativeName) throw new Error(`Native command missing native name: ${command.key}`);else
    {
      const nativeKey = (0, _stringCoerceBUSzWgUA.o)(nativeName) ?? "";
    ...
    for (const alias of command.textAliases) {
      ...
      const aliasKey = (0, _stringCoerceBUSzWgUA.o)(alias) ?? "";
...
function buildBuiltinChatCommands() {
  ...
  defineChatCommand({
    ...
    args: [{
      ...
      type: "string",
      choices: ({ provider, model }) => (0, _thinkingCtJu90cp.r)(provider, model)
    }],
    argsMenu: "auto"
  }),
  defineChatCommand({
...
function buildChatCommands() {
  const commands = [...buildBuiltinChatCommands(), ...(0, _registryLoadedC109837J.n)().filter(supportsNativeCommands).map((plugin) => defineDockCommand(plugin))];
  assertCommandRegistry(commands);
  return commands;
}
...
function getNativeCommandSurfaces() {
  ...
  cachedNativeCommandSurfaces = new Set((0, _registryLoadedC109837J.n)().filter(supportsNativeCommands).map((plugin) => plugin.id));
  cachedNativeRegistryVersion = registryVersion;
  return cachedNativeCommandSurfaces;
}
//#endregion /* v9-549781cb47b411e4 */
