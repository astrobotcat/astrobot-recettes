"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = finishFlow;exports.C = setTaskRunDeliveryStatusByRunId;exports.D = deleteTaskFlowRecordById;exports.E = createTaskFlowForTask;exports.F = requestFlowCancel;exports.I = resolveTaskFlowForLookupToken;exports.L = resumeFlow;exports.M = getTaskFlowRegistryRestoreFailure;exports.N = listTaskFlowRecords;exports.O = failFlow;exports.P = listTaskFlowsForOwnerKey;exports.R = setFlowWaiting;exports.S = setTaskCleanupAfterById;exports.T = createManagedTaskFlow;exports._ = markTaskTerminalById;exports.a = findTaskByRunId;exports.b = recordTaskProgressByRunId;exports.c = linkTaskToFlowById;exports.d = listTasksForFlowId;exports.f = listTasksForOwnerKey;exports.g = markTaskRunningByRunId;exports.h = markTaskLostById;exports.i = ensureTaskRegistryReady;exports.j = getTaskFlowById;exports.k = findLatestTaskFlowForOwnerKey;exports.l = listTaskRecords;exports.m = listTasksForSessionKey;exports.n = createTaskRecord;exports.o = getTaskById;exports.p = listTasksForRelatedSessionKey;exports.r = deleteTaskRecordById;exports.s = isParentFlowLinkError;exports.t = cancelTaskById;exports.u = listTasksForAgentId;exports.v = markTaskTerminalByRunId;exports.w = updateTaskNotifyPolicyById;exports.x = resolveTaskForLookupToken;exports.y = maybeDeliverTaskTerminalUpdate;exports.z = updateFlowRecordByIdExpectedRevision;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
...
var _nodeSqliteVqjevi9y = require("./node-sqlite-Vqjevi9y.js");
var _agentEventsBLJ1mmc = require("./agent-events-BLJ1mmc3.js");
var _heartbeatWakeA3GAt85y = require("./heartbeat-wake-a3GAt85y.js");
var _systemEventsDq_M0n = require("./system-events-Dq_M0n12.js");
var _taskStatusDdxMiKkn = require("./task-status-DdxMiKkn.js");
var _nodeModule = require("node:module");
var _nodeFs = require("node:fs");
var _nodePath = _interopRequireDefault(require("node:path"));
var _nodeOs = _interopRequireDefault(require("node:os"));
var _nodeCrypto = _interopRequireDefault(require("node:crypto"));
var _nodeWorker_threads = require("node:worker_threads");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
function resolveTaskDisplayTitle(task) {
  return (0, _taskStatusDdxMiKkn.i)(task.label?.trim() || (task.runtime === "acp" ? "ACP background task" : task.runtime === "subagent" ? "Subagent task" : task.task.trim() || "Background task"));
}
...
function formatTaskTerminalMessage(task) {
  ...
  const summary = (0, _taskStatusDdxMiKkn.a)(task.terminalSummary, { errorContext: task.status !== "succeeded" || task.terminalOutcome === "blocked" });
  ...
  if (task.status === "lost") {
    const error = (0, _taskStatusDdxMiKkn.a)(task.error, { errorContext: true });
    const fallbackSummary = (0, _taskStatusDdxMiKkn.a)(task.terminalSummary, { errorContext: true });
...
function formatTaskBlockedFollowupMessage(task) {
  ...
  return `Task needs follow-up: ${resolveTaskDisplayTitle(task)}${resolveTaskRunLabel(task)}. ${(0, _taskStatusDdxMiKkn.a)(task.terminalSummary, { errorContext: true }) || "Task is blocked and needs follow-up."}`;
}
function formatTaskStateChangeMessage(task, event) {
  const title = resolveTaskDisplayTitle(task);
  if (event.kind === "running") return `Background task started: ${title}.`;
  if (event.kind === "progress") {
    const summary = (0, _taskStatusDdxMiKkn.a)(event.summary);
...
function resolveTaskStateDir(env = process.env) {
  if (env.OPENCLAW_STATE_DIR?.trim()) return (0, _pathsDvv9VRAc._)(env);
  if (env.VITEST || env.NODE_ENV === "test") {
    const workerIdRaw = env.VITEST_WORKER_ID ?? env.VITEST_POOL_ID ?? "";
    const workerId = Number.parseInt(workerIdRaw, 10);
    const shardSuffix = Number.isFinite(workerId) ? `${process.pid}-${workerId}` : _nodeWorker_threads.isMainThread ? String(process.pid) : `${process.pid}-${_nodeWorker_threads.threadId}`;
    return _nodePath.default.join(_nodeOs.default.tmpdir(), "openclaw-test-state", shardSuffix);
  }
  return (0, _pathsDvv9VRAc._)(env);
}
function resolveTaskRegistryDir(env = process.env) {
  return _nodePath.default.join(resolveTaskStateDir(env), "tasks");
}
function resolveTaskRegistrySqlitePath(env = process.env) {
  return _nodePath.default.join(resolveTaskRegistryDir(env), "runs.sqlite");
}
//#endregion
//#region src/tasks/task-flow-registry.paths.ts
function resolveTaskFlowRegistryDir(env = process.env) {
  return _nodePath.default.join(resolveTaskStateDir(env), "flows");
}
function resolveTaskFlowRegistrySqlitePath(env = process.env) {
  return _nodePath.default.join(resolveTaskFlowRegistryDir(env), "registry.sqlite");
}
//#endregion
...
function ensureFlowRegistryPermissions(pathname) {
  ...
  (0, _nodeFs.chmodSync)(dir, FLOW_REGISTRY_DIR_MODE);
  for (const suffix of FLOW_REGISTRY_SIDECAR_SUFFIXES) {
    const candidate = `${pathname}${suffix}`;
    if (!(0, _nodeFs.existsSync)(candidate)) continue;
    (0, _nodeFs.chmodSync)(candidate, FLOW_REGISTRY_FILE_MODE);
  }
}
function openFlowRegistryDatabase() {
  ...
  const { DatabaseSync } = (0, _nodeSqliteVqjevi9y.t)();
...
const log$1 = (0, _subsystemCgmckbux.t)("tasks/task-flow-registry");
...
function normalizeRestoredFlowRecord(record) {
  ...
  const controllerId = syncMode === "managed" ? (0, _stringCoerceBUSzWgUA.s)(record.controllerId) ?? "core/legacy-restored" : void 0;
  return {
    ...record,
    syncMode,
    ...
    currentStep: (0, _stringCoerceBUSzWgUA.s)(record.currentStep),
    blockedTaskId: (0, _stringCoerceBUSzWgUA.s)(record.blockedTaskId),
    blockedSummary: (0, _stringCoerceBUSzWgUA.s)(record.blockedSummary),
    ...(record.stateJson !== void 0 ? { stateJson: cloneStructuredValue(record.stateJson) } : {}),
    ...(record.waitJson !== void 0 ? { waitJson: cloneStructuredValue(record.waitJson) } : {}),
...
function assertFlowOwnerKey(ownerKey) {
  const normalized = (0, _stringCoerceBUSzWgUA.s)(ownerKey);
  if (!normalized) throw new Error("Flow ownerKey is required.");
  return normalized;
}
function assertControllerId(controllerId) {
  const normalized = (0, _stringCoerceBUSzWgUA.s)(controllerId);
...
function resolveFlowBlockedSummary(task) {
  ...
  return (0, _stringCoerceBUSzWgUA.s)(task.terminalSummary) ?? (0, _stringCoerceBUSzWgUA.s)(task.progressSummary);
}
...
function ensureFlowRegistryReady() {
  ...
  try {
    ...
    flows.clear();
    restoreFailureMessage = (0, _errorsD8p6rxH.i)(error);
...
function buildFlowRecord(params) {
  ...
  return {
    flowId: _nodeCrypto.default.randomUUID(),
    syncMode,
    ...
    goal: params.goal,
    currentStep: (0, _stringCoerceBUSzWgUA.s)(params.currentStep),
    blockedTaskId: (0, _stringCoerceBUSzWgUA.s)(params.blockedTaskId),
    blockedSummary: (0, _stringCoerceBUSzWgUA.s)(params.blockedSummary),
...
function applyFlowPatch(current, patch) {
  const controllerId = patch.controllerId === void 0 ? current.controllerId : (0, _stringCoerceBUSzWgUA.s)(patch.controllerId);
  ...
  return {
    ...
    controllerId,
    currentStep: patch.currentStep === void 0 ? current.currentStep : (0, _stringCoerceBUSzWgUA.s)(patch.currentStep),
    blockedTaskId: patch.blockedTaskId === void 0 ? current.blockedTaskId : (0, _stringCoerceBUSzWgUA.s)(patch.blockedTaskId),
    blockedSummary: patch.blockedSummary === void 0 ? current.blockedSummary : (0, _stringCoerceBUSzWgUA.s)(patch.blockedSummary),
...
function createTaskFlowForTask(params) {
  ...
  return createFlowRecord({
    ...
    goal: (0, _stringCoerceBUSzWgUA.s)(params.task.label) ?? (params.task.task.trim() || "Background task"),
    blockedTaskId: terminalFlowStatus === "blocked" ? (0, _stringCoerceBUSzWgUA.s)(params.task.taskId) : void 0,
...
function setFlowWaiting(params) {
  return updateFlowRecordByIdExpectedRevision({
    ...
    patch: {
      status: (0, _stringCoerceBUSzWgUA.s)(params.blockedTaskId) || (0, _stringCoerceBUSzWgUA.s)(params.blockedSummary) ? "blocked" : "waiting",
...
function syncFlowFromTask(task) {
  ...
  return updateFlowRecordByIdUnchecked(flowId, {
    ...
    goal: (0, _stringCoerceBUSzWgUA.s)(task.label) ?? (task.task.trim() || "Background task"),
...
function ensureTaskRegistryPermissions(pathname) {
  ...
  (0, _nodeFs.chmodSync)(dir, TASK_REGISTRY_DIR_MODE);
  for (const suffix of TASK_REGISTRY_SIDECAR_SUFFIXES) {
    const candidate = `${pathname}${suffix}`;
    if (!(0, _nodeFs.existsSync)(candidate)) continue;
    (0, _nodeFs.chmodSync)(candidate, TASK_REGISTRY_FILE_MODE);
  }
}
...
function upsertTaskWithDeliveryStateToSqlite(params) {
  withWriteTransaction((statements) => {
    ...
    if (params.deliveryState) statements.replaceDeliveryState.run(bindTaskDeliveryState(params.deliveryState));else
...
const log = (0, _subsystemCgmckbux.t)("tasks/registry");
...
const _require = (0, _nodeModule.createRequire)("file:///home/linuxbrew/.linuxbrew/lib/node_modules/openclaw/dist/task-registry-BO2EMqj7.js");
...
function assertParentFlowLinkAllowed(params) {
  ...
  if ((0, _stringCoerceBUSzWgUA.s)(flow.ownerKey) !== (0, _stringCoerceBUSzWgUA.s)(params.ownerKey)) throw new ParentFlowLinkError("owner_key_mismatch", "Task ownerKey must match parent flow ownerKey.", { flowId });
...
function loadTaskRegistryDeliveryRuntime() {
  ...
  deliveryRuntimePromise ??= Promise.resolve().then(() => jitiImport("./task-registry-delivery-runtime-B4A5JbUe.js").then((m) => _interopRequireWildcard(m)));
...
function getTaskRelatedSessionIndexKeys(task) {
  return [...new Set([(0, _stringCoerceBUSzWgUA.s)(task.ownerKey), (0, _stringCoerceBUSzWgUA.s)(task.childSessionKey)].filter(Boolean))];
}
function addOwnerKeyIndex(taskId, task) {
  const key = (0, _stringCoerceBUSzWgUA.s)(task.ownerKey);
  if (!key) return;
...
function taskRunScopeKey(task) {
  ...
  task.runtime,
  task.scopeKind,
  (0, _stringCoerceBUSzWgUA.s)(task.ownerKey) ?? "",
  (0, _stringCoerceBUSzWgUA.s)(task.childSessionKey) ?? ""].
  join("\0");
}
function getTasksByRunScope(params) {
  ...
  if (sessionKey) {
    const childMatches = matches.filter((task) => (0, _stringCoerceBUSzWgUA.s)(task.childSessionKey) === sessionKey);
    if (childMatches.length > 0) return childMatches;
    return matches.filter((task) => task.scopeKind === "session" && (0, _stringCoerceBUSzWgUA.s)(task.ownerKey) === sessionKey);
  }
...
function getPeerTasksForDelivery(task) {
  ...
  return getTasksByRunId(task.runId).filter((candidate) => candidate.runtime === task.runtime && candidate.scopeKind === task.scopeKind && ((0, _stringCoerceBUSzWgUA.s)(candidate.ownerKey) ?? "") === ((0, _stringCoerceBUSzWgUA.s)(task.ownerKey) ?? "") && ((0, _stringCoerceBUSzWgUA.s)(candidate.childSessionKey) ?? "") === ((0, _stringCoerceBUSzWgUA.s)(task.childSessionKey) ?? ""));
}
...
function findExistingTaskForCreate(params) {
  ...
  const runScopeMatches = runId ? getTasksByRunId(runId).filter((task) => task.runtime === params.runtime && task.scopeKind === params.scopeKind && ((0, _stringCoerceBUSzWgUA.s)(task.ownerKey) ?? "") === ((0, _stringCoerceBUSzWgUA.s)(params.ownerKey) ?? "") && ((0, _stringCoerceBUSzWgUA.s)(task.childSessionKey) ?? "") === ((0, _stringCoerceBUSzWgUA.s)(params.childSessionKey) ?? "") && ((0, _stringCoerceBUSzWgUA.s)(task.parentFlowId) ?? "") === ((0, _stringCoerceBUSzWgUA.s)(params.parentFlowId) ?? "")) : [];
  const exact = runId ? runScopeMatches.find((task) => ((0, _stringCoerceBUSzWgUA.s)(task.label) ?? "") === ((0, _stringCoerceBUSzWgUA.s)(params.label) ?? "") && ((0, _stringCoerceBUSzWgUA.s)(task.task) ?? "") === ((0, _stringCoerceBUSzWgUA.s)(params.task) ?? "")) : void 0;
...
function mergeExistingTaskForCreate(existing, params) {
  ...
  if (params.preferMetadata) {
    if (nextLabel && ((0, _stringCoerceBUSzWgUA.s)(existing.label) ?? "") !== nextLabel) patch.label = nextLabel;
    const nextTask = params.task.trim();
    if (nextTask && ((0, _stringCoerceBUSzWgUA.s)(existing.task) ?? "") !== nextTask) patch.task = nextTask;
...
function getLinkedFlowForDelivery(task) {
  ...
  const flow = getTaskFlowById(flowId);
  if (!flow) return;
  if ((0, _stringCoerceBUSzWgUA.s)(flow.ownerKey) !== (0, _stringCoerceBUSzWgUA.s)(task.ownerKey)) return;
  return flow;
}
function resolveTaskDeliveryOwner(task) {
  ...
  if (flow) return {
    ...
    requesterOrigin: (0, _deliveryContextSharedEClQPjt.i)(flow.requesterOrigin ?? taskDeliveryStates.get(task.taskId)?.requesterOrigin),
    flowId: flow.flowId
  };
  if (task.scopeKind !== "session") return {};
  return {
    sessionKey: task.ownerKey.trim(),
    requesterOrigin: (0, _deliveryContextSharedEClQPjt.i)(taskDeliveryStates.get(task.taskId)?.requesterOrigin)
  };
}
...
function updateTask(taskId, patch) {
  ...
  const sessionIndexChanged = (0, _stringCoerceBUSzWgUA.s)(current.ownerKey) !== (0, _stringCoerceBUSzWgUA.s)(next.ownerKey) || (0, _stringCoerceBUSzWgUA.s)(current.childSessionKey) !== (0, _stringCoerceBUSzWgUA.s)(next.childSessionKey);
...
function upsertTaskDeliveryState(state) {
  ...
  const next = {
    taskId: state.taskId,
    ...(state.requesterOrigin ? { requesterOrigin: (0, _deliveryContextSharedEClQPjt.i)(state.requesterOrigin) } : {}),
...
function canDeliverTaskToRequesterOrigin(task) {
  ...
  return Boolean(channel && to && (0, _messageChannelCBqCPFa_.s)(channel));
}
...
function queueBlockedTaskFollowup(task) {
  ...
  const ownerKey = owner.sessionKey?.trim();
  if (!ownerKey) return false;
  (0, _systemEventsDq_M0n.i)(followupText, {
    sessionKey: ownerKey,
...
async function maybeDeliverTaskTerminalUpdate(taskId) {
  ...
  try {
    ...
    try {
      ...
      const requesterAgentId = (0, _sessionKeyBh1lMwK.x)(ownerSessionKey)?.agentId;
  ...
  (0, _agentEventsBLJ1mmc.l)((evt) => {
    ...
    for (const current of scopedTasks) {
      ...
      if (evt.stream === "lifecycle") {
        ...
        if (phase === "start") patch.status = "running";else
        if (phase === "end") {
...
function createTaskRecord(params) {
  ...
  const record = {
    taskId,
    runtime: params.runtime,
    taskKind: (0, _stringCoerceBUSzWgUA.s)(params.taskKind),
    sourceId: (0, _stringCoerceBUSzWgUA.s)(params.sourceId),
    requesterSessionKey,
    ownerKey,
    scopeKind,
    childSessionKey: params.childSessionKey,
    parentFlowId: (0, _stringCoerceBUSzWgUA.s)(params.parentFlowId),
    parentTaskId: (0, _stringCoerceBUSzWgUA.s)(params.parentTaskId),
    agentId: (0, _stringCoerceBUSzWgUA.s)(params.agentId),
    runId: (0, _stringCoerceBUSzWgUA.s)(params.runId),
    label: (0, _stringCoerceBUSzWgUA.s)(params.label),
    task: params.task,
    status,
    deliveryStatus,
    notifyPolicy,
  ...
  upsertTaskDeliveryState({
    taskId,
    requesterOrigin: (0, _deliveryContextSharedEClQPjt.i)(params.requesterOrigin)
  });
...
async function cancelTaskById(params) {
  ...
  try {
    ...
    return {
      found: true,
      cancelled: false,
      reason: (0, _errorsD8p6rxH.i)(error),
      task: cloneTaskRecord(task)
    };
  }
}
...
function listTasksForSessionKey(sessionKey) {
  ensureTaskRegistryReady();
  const key = (0, _stringCoerceBUSzWgUA.s)(sessionKey);
  if (!key) return [];
...
function listTasksForOwnerKey(ownerKey) {
  ensureTaskRegistryReady();
  const key = (0, _stringCoerceBUSzWgUA.s)(ownerKey);
  if (!key) return [];
