...
function isWSL(env = process.env) {
  ...
  try {
    const release = (0, _fs.readFileSync)("/proc/version", "utf-8");
...
function readClipboardImageViaPowerShell() {
  const tmpFile = (0, _path.join)((0, _os.tmpdir)(), `pi-wsl-clip-${(0, _crypto.randomUUID)()}.png`);
  try {
    ...
    const bytes = (0, _fs.readFileSync)(tmpFile);
    if (bytes.length === 0) {
      return null;
    }
