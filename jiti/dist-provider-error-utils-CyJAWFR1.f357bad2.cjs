"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = parseTtsDirectives;exports.i = truncateErrorDetail;exports.n = asObject;exports.r = readResponseTextLimited;exports.t = asBoolean;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function resolveDirectiveProviders(options) {
  ...
  return (0, _providerRegistryCasPS0mm.r)(options?.cfg).toSorted(buildProviderOrder);
}
...
function parseTtsDirectives(text, policy, options) {
  ...
  cleanedText = cleanedText.replace(/\[\[tts:([^\]]+)\]\]/gi, (_match, body) => {
    ...
    if (policy.allowProvider) for (const token of tokens) {
      ...
      if (!rawKey || (0, _stringCoerceBUSzWgUA.i)(rawKey) !== "provider") continue;
      const rawValue = token.slice(eqIndex + 1).trim();
      if (!rawValue) continue;
      const providerId = (0, _stringCoerceBUSzWgUA.i)(rawValue);
      if (!providerId) {
    ...
    const orderedProviders = prioritizeProvider(providers, declaredProviderId ?? (0, _stringCoerceBUSzWgUA.i)(options?.preferredProviderId));
    for (const token of tokens) {
      ...
      const key = (0, _stringCoerceBUSzWgUA.i)(rawKey);
