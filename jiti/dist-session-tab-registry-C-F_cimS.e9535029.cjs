"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = fetchBrowserJson;exports.a = browserCreateProfile;exports.c = browserOpenTab;exports.d = browserSnapshot;exports.f = browserStart;exports.g = browserTabs;exports.h = browserTabAction;exports.i = browserCloseTab;exports.l = browserProfiles;exports.m = browserStop;exports.n = trackSessionBrowserTab;exports.o = browserDeleteProfile;exports.p = browserStatus;exports.r = untrackSessionBrowserTab;exports.s = browserFocusTab;exports.t = closeTrackedBrowserTabsForSessions;exports.u = browserResetProfile;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
require("./ssrf-runtime-CmuKDV7X.js");
var _rateLimitMessage1yTP_ns_ = require("./rate-limit-message-1yTP_ns_.js");
var _bridgeAuthRegistryB8T79NOk = require("./bridge-auth-registry-B8T79NOk.js");
var _controlAuthHABiqRrk = require("./control-auth-hABiqRrk.js");
require("./command-format-7Lwhq0tT.js");function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
function isLoopbackHttpUrl(url) {
  try {
    return (0, _netLBInRHnX.a)(new URL(url).hostname);
  } catch {
    return false;
  }
}
function withLoopbackBrowserAuthImpl(url, init, deps) {
  ...
  try {
    ...
    if (bridgeAuth?.token) headers.set("Authorization", `Bearer ${bridgeAuth.token}`);else
    if (bridgeAuth?.password) headers.set("x-openclaw-password", bridgeAuth.password);
  } catch {}
  return {
    ...init,
    headers
...
function withLoopbackBrowserAuth(url, init) {
  return withLoopbackBrowserAuthImpl(url, init, {
    ...
    resolveBrowserControlAuth: _controlAuthHABiqRrk.n,
    getBridgeAuthForPort: _bridgeAuthRegistryB8T79NOk.n
  });
}
...
function resolveBrowserFetchOperatorHint(url) {
  return !isAbsoluteHttp(url) ? `Restart the OpenClaw gateway (OpenClaw.app menubar, or \`${(0, _commandFormatDd3uP.t)("openclaw gateway")}\`).` : "If this is a sandboxed session, ensure the sandbox browser is running.";
}
function normalizeErrorMessage(err) {
  const message = err instanceof Error ? (0, _stringCoerceBUSzWgUA.s)(err.message) : void 0;
...
function enhanceBrowserFetchError(url, err, timeoutMs) {
  ...
  const msgLower = (0, _stringCoerceBUSzWgUA.i)(msg);
...
async function fetchHttpJson(url, init) {
  ...
  if (upstreamSignal) if (upstreamSignal.aborted) ctrl.abort(upstreamSignal.reason);else
  {
  ...
  let release;
  try {
    const guarded = await (0, _fetchGuardB3p4gGaY.n)({
      url,
      init,
      signal: ctrl.signal,
    ...
    if (!res.ok) {
      if (isRateLimitStatus(res.status)) {
        ...
        throw new BrowserServiceError(`${(0, _rateLimitMessage1yTP_ns_.t)(url)} ${BROWSER_TOOL_MODEL_HINT}`);
      }
...
async function fetchBrowserJson(url, init) {
  ...
  try {
    ...
    const { dispatchBrowserControlRequest } = await Promise.resolve().then(() => jitiImport("./local-dispatch.runtime-DFCUCw4c.js").then((m) => _interopRequireWildcard(m)));
    ...
    const upstreamSignal = init?.signal;
    let upstreamAbortListener;
    if (upstreamSignal) if (upstreamSignal.aborted) abortCtrl.abort(upstreamSignal.reason);else
    {
    ...
    if (result.status >= 400) {
      if (isRateLimitStatus(result.status)) throw new BrowserServiceError(`${(0, _rateLimitMessage1yTP_ns_.t)(url)} ${BROWSER_TOOL_MODEL_HINT}`);
...
function normalizeSessionKey(raw) {
  return (0, _stringCoerceBUSzWgUA.o)(raw) ?? "";
}
...
function isIgnorableCloseError(err) {
  const message = (0, _stringCoerceBUSzWgUA.i)(String(err));
