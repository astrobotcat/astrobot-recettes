...
function normalizeApiKeyInput(raw) {
  const trimmed = (0, _stringCoerceBUSzWgUA.u)(raw) ?? "";
  if (!trimmed) return "";
...
function normalizeTokenProviderInput(tokenProvider) {
  return (0, _stringCoerceBUSzWgUA.o)(tokenProvider);
}
function normalizeSecretInputModeInput(secretInputMode) {
  const normalized = (0, _stringCoerceBUSzWgUA.o)(secretInputMode);
...
async function ensureApiKeyFromEnvOrPrompt(params) {
  const selectedMode = await (0, _providerAuthModeB3mgHAFL.t)({
    prompter: params.prompter,
    explicitMode: params.secretInputMode
  });
  const env = params.env ?? process.env;
  const envKey = (0, _modelAuthEnvB45Q6PX.t)(params.provider, env);
  if (selectedMode === "ref") {
    if (typeof params.prompter.select !== "function") {
      const fallback = (0, _providerAuthRefTaJV7nIq.r)({
        config: params.config,
        provider: params.provider,
        preferredEnvVar: envKey?.source ? (0, _providerAuthRefTaJV7nIq.t)(envKey.source) : void 0,
        env
      });
      await params.setCredential(fallback.ref, selectedMode);
      return fallback.resolvedValue;
    }
    const resolved = await (0, _providerAuthRefTaJV7nIq.n)({
      provider: params.provider,
      config: params.config,
      prompter: params.prompter,
      preferredEnvVar: envKey?.source ? (0, _providerAuthRefTaJV7nIq.t)(envKey.source) : void 0,
