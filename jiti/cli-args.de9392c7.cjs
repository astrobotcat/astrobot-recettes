      ...
      for (const name of toolNames) {
        ...
        } else
        {
          result.diagnostics.push({
            type: "warning",
            message: `Unknown tool "${name}". Valid tools: ${Object.keys(_index.allTools).join(", ")}`
          });
        }
      }
...
function printHelp(extensionFlags) {
  ...
  `\n${_chalk.default.bold("Extension CLI Flags:")}\n${extensionFlags.
  map((flag) => {
...
${_chalk.default.bold("Commands:")}
  ...
  ${_config.APP_NAME} uninstall <source> [-l]   Alias for remove
  ...
  ${_config.APP_NAME} <command> --help          Show help for install/remove/uninstall/update/list
...
${_chalk.default.bold("Examples:")}
  ...
  ${_config.APP_NAME} --model openai/gpt-4o "Help me refactor this code"

  # Use model with thinking level shorthand
  ${_config.APP_NAME} --model sonnet:high "Solve this complex problem"
