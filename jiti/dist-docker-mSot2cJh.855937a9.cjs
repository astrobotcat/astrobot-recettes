"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.C = removeRegistryEntry;exports.E = computeSandboxBrowserConfigHash;exports.S = removeBrowserRegistryEntry;exports.T = updateRegistry;exports._ = resolveSandboxScopeKey;exports.a = execDockerRaw;exports.b = readBrowserRegistry;exports.c = readDockerNetworkDriver;exports.d = appendWorkspaceMountArgs;exports.f = getBlockedBindReason;exports.g = resolveSandboxAgentId;exports.h = splitSandboxBindSpec;exports.i = execDocker;exports.l = readDockerNetworkGateway;exports.m = resolveSandboxHostPathViaExistingAncestor;exports.n = dockerContainerState;exports.o = readDockerContainerEnvVar;exports.p = validateNetworkMode;exports.r = ensureSandboxContainer;exports.s = readDockerContainerLabel;exports.t = buildSandboxCreateArgs;exports.u = readDockerPort;exports.v = resolveSandboxWorkspaceDir;exports.w = updateBrowserRegistry;exports.x = readRegistry;exports.y = slugifySessionKey;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
require("./agent-scope-KFH9bkHi.js");
var _networkModeMIyLMFNb = require("./network-mode-mIyLMFNb.js");
var _zodParseEztZP0fF = require("./zod-parse-EztZP0fF.js");
...
var _windowsSpawnG7jYADOM = require("./windows-spawn-G7jYADOM.js");
var _nodePath = _interopRequireWildcard(require("node:path"));
var _nodeChild_process = require("node:child_process");
var _nodeOs = _interopRequireDefault(require("node:os"));
var _promises = _interopRequireDefault(require("node:fs/promises"));
var _nodeCrypto = _interopRequireDefault(require("node:crypto"));
var _zod = require("zod");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
const RegistryEntrySchema = _zod.z.object({ containerName: _zod.z.string() }).passthrough();
const RegistryFileSchema = _zod.z.object({ entries: _zod.z.array(RegistryEntrySchema) });
...
async function withRegistryLock(registryPath, fn) {
  const lock = await (0, _sessionWriteLockCcI4KSH.t)({
...
async function readRegistryFromFile(registryPath, mode) {
  try {
    const parsed = (0, _zodParseEztZP0fF.t)(RegistryFileSchema, await _promises.default.readFile(registryPath, "utf-8"));
...
async function writeRegistryFile(registryPath, registry) {
  await (0, _jsonFilesL0zR3LSb.r)(registryPath, registry, { trailingNewline: true });
}
async function readRegistry() {
  return { entries: (await readRegistryFromFile(_configCWqq9_ZP.p, "fallback")).entries.map((entry) => normalizeSandboxRegistryEntry(entry)) };
}
...
async function updateRegistry(entry) {
  await withRegistryMutation(_configCWqq9_ZP.p, (entries) => upsertEntry(entries, entry));
}
async function removeRegistryEntry(containerName) {
  await withRegistryMutation(_configCWqq9_ZP.p, (entries) => {
...
async function readBrowserRegistry() {
  return await readRegistryFromFile(_configCWqq9_ZP.d, "fallback");
}
async function updateBrowserRegistry(entry) {
  await withRegistryMutation(_configCWqq9_ZP.d, (entries) => upsertEntry(entries, entry));
}
async function removeBrowserRegistryEntry(containerName) {
  await withRegistryMutation(_configCWqq9_ZP.d, (entries) => {
    const next = removeEntry(entries, containerName);
    if (next.length === entries.length) return null;
    return next;
  });
...
function slugifySessionKey(value) {
  ...
  return `${(0, _stringCoerceBUSzWgUA.i)(trimmed).replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 32) || "session"}-${hash}`;
}
function resolveSandboxWorkspaceDir(root, sessionKey) {
  const resolvedRoot = (0, _utilsD5DtWkEu.m)(root);
...
function resolveSandboxScopeKey(scope, sessionKey) {
  ...
  return `agent:${(0, _sessionKeyBh1lMwK.u)(trimmed)}`;
}
function resolveSandboxAgentId(scopeKey) {
  const trimmed = scopeKey.trim();
  if (!trimmed || trimmed === "shared") return;
  const parts = trimmed.split(":").filter(Boolean);
  if (parts[0] === "agent" && parts[1]) return (0, _sessionKeyBh1lMwK.c)(parts[1]);
  return (0, _sessionKeyBh1lMwK.u)(trimmed);
}
//#endregion
...
function normalizeSandboxHostPath(raw) {
  ...
  if (!trimmed) return "/";
  return _nodePath.posix.normalize(trimmed.replaceAll("\\", "/")).replace(/\/+$/, "") || "/";
}
/**
...
function resolveSandboxHostPathViaExistingAncestor(sourcePath) {
  ...
  return normalizeSandboxHostPath((0, _fileIdentityEQApOIDl.a)(sourcePath));
}
//#endregion
...
const RESERVED_CONTAINER_TARGET_PATHS = ["/workspace", _configCWqq9_ZP.u];
...
function getBlockedHostPaths() {
  ...
  for (const home of getBlockedHomeRoots()) for (const suffix of BLOCKED_HOME_SUBPATHS) blocked.add(normalizeHostPath(_nodePath.default.posix.join(home, suffix)));
  return [...blocked];
}
function getBlockedHomeRoots() {
  const roots = /* @__PURE__ */new Set();
  for (const candidate of [(0, _homeDirBEqRdfoa.o)(process.env, _nodeOs.default.homedir), (0, _homeDirBEqRdfoa.s)(process.env, _nodeOs.default.homedir)]) {
...
function validateNetworkMode(network, options) {
  const blockedReason = (0, _networkModeMIyLMFNb.t)({
    network,
...
function validateSeccompProfile(profile) {
  if (profile && BLOCKED_SECCOMP_PROFILES.has((0, _stringCoerceBUSzWgUA.o)(profile) ?? "")) throw new Error(`Sandbox security: seccomp profile "${profile}" is blocked. Disabling seccomp removes syscall filtering and weakens sandbox isolation. Use a custom seccomp profile file or omit this setting.`);
}
function validateApparmorProfile(profile) {
  if (profile && BLOCKED_APPARMOR_PROFILES.has((0, _stringCoerceBUSzWgUA.o)(profile) ?? "")) throw new Error(`Sandbox security: apparmor profile "${profile}" is blocked. Disabling AppArmor removes mandatory access controls and weakens sandbox isolation. Use a named AppArmor profile or omit this setting.`);
}
...
function appendWorkspaceMountArgs(params) {
  ...
  if (workspaceAccess !== "none" && workspaceDir !== agentWorkspaceDir) args.push("-v", formatManagedWorkspaceBind({
    ...
    containerPath: _configCWqq9_ZP.u,
...
function resolveDockerSpawnInvocation(args, runtime = DEFAULT_DOCKER_SPAWN_RUNTIME) {
  const resolved = (0, _windowsSpawnG7jYADOM.n)((0, _windowsSpawnG7jYADOM.i)({
    command: "docker",
...
function execDockerRaw(args, opts) {
  return new Promise((resolve, reject) => {
    ...
    const child = (0, _nodeChild_process.spawn)(spawnInvocation.command, spawnInvocation.args, {
      stdio: [
      "pipe",
      "pipe",
      "pipe"],
...
const log = (0, _subsystemCgmckbux.t)("docker");
...
function buildSandboxCreateArgs(params) {
  ...
  const envSanitization = (0, _sanitizeEnvVarsOg3CRoPL.t)(params.cfg.env ?? {}, params.envSanitizationOptions);
  ...
  for (const [key, value] of Object.entries((0, _openclawExecEnv5MQ0wyS.n)(envSanitization.allowed))) args.push("--env", `${key}=${value}`);
...
function formatSandboxRecreateHint(params) {
  if (params.scope === "session") return (0, _commandFormatDd3uP.t)(`openclaw sandbox recreate --session ${params.sessionKey}`);
  if (params.scope === "agent") return (0, _commandFormatDd3uP.t)(`openclaw sandbox recreate --agent ${resolveSandboxAgentId(params.sessionKey) ?? "main"}`);
  return (0, _commandFormatDd3uP.t)("openclaw sandbox recreate --all");
}
  ...
  if (hasContainer) {
    ...
    if (hashMismatch) {
      ...
      if (running && (typeof lastUsedAtMs !== "number" || now - lastUsedAtMs < HOT_CONTAINER_WINDOW_MS)) {
        ...
        _runtimeDx7oeLYq.n.log(`Sandbox config changed for ${containerName} (recently used). Recreate to apply: ${hint}`);
      } else {
        await execDocker([
        "rm",
        "-f",
