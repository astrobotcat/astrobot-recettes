"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveChannelNativeApprovalDeliveryPlan;exports.n = deliverApprovalRequestViaChannelNativePlan;exports.r = createExecApprovalChannelRuntime;exports.t = createChannelNativeApprovalRuntime;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _subsystemCgmckbux = require("./subsystem-Cgmckbux.js");
var _operatorApprovalsClientCGHXKajQ = require("./operator-approvals-client-CGHXKajQ.js");
var _approvalNativeTargetKeyDkH4SJHX = require("./approval-native-target-key-DkH4SJHX.js");
//#region src/infra/approval-native-delivery.ts
function dedupeTargets(targets) {
  const seen = /* @__PURE__ */new Set();
  const deduped = [];
  for (const target of targets) {
    const key = (0, _approvalNativeTargetKeyDkH4SJHX.t)(target.target);
...
async function resolveChannelNativeApprovalDeliveryPlan(params) {
  ...
  if (!originTarget) for (const target of approverDmTargets) plannedTargets.push({
    surface: "approver-dm",
    target,
    reason: "fallback"
  });
...
function didReportDeliverToOrigin(report, originAccountId) {
  ...
  const reportAccountId = (0, _stringCoerceBUSzWgUA.s)(report.accountId);
  if (originAccountId !== void 0 && reportAccountId !== void 0 && reportAccountId !== originAccountId) return false;
  const originKey = (0, _approvalNativeTargetKeyDkH4SJHX.t)(originTarget);
  return report.deliveredTargets.some((plannedTarget) => (0, _approvalNativeTargetKeyDkH4SJHX.t)(plannedTarget.target) === originKey);
}
function resolveApprovalRouteNotice(params) {
  ...
  const originAccountId = (0, _stringCoerceBUSzWgUA.s)(target.accountId);
...
function createExecApprovalChannelRuntime(adapter) {
  const log = (0, _subsystemCgmckbux.t)(adapter.label);
  ...
  return {
    async start() {
      ...
      startPromise = (async () => {
        ...
        const client = await (0, _operatorApprovalsClientCGHXKajQ.t)({
          config: adapter.cfg,
