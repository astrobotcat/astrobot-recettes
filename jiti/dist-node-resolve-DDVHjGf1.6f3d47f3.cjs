"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = parsePairingList;exports.n = resolveNodeIdFromNodeList;exports.r = parseNodeList;exports.t = resolveNodeFromNodeList;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function parsePairingList(value) {
  const obj = (0, _recordCoerceBls3blVy.a)(value);
  return {
...
function normalizeNodeKey(value) {
  return (0, _stringCoerceBUSzWgUA.i)(value).replace(/[^a-z0-9]+/g, "-").replace(/^-+/, "").replace(/-+$/, "");
}
...
function formatNodeCandidateLabel(node) {
  ...
  const clientId = (0, _stringCoerceBUSzWgUA.s)(node.clientId);
  if (clientId) details.push(`client=${clientId}`);
  return `${label} [${details.join(", ")}]`;
}
function isCurrentOpenClawClient(clientId) {
  return ((0, _stringCoerceBUSzWgUA.o)(clientId) ?? "").startsWith("openclaw-");
}
function isLegacyClawdbotClient(clientId) {
  const normalized = (0, _stringCoerceBUSzWgUA.o)(clientId) ?? "";
...
function scoreNodeCandidate(node, matchScore) {
  ...
  if (isCurrentOpenClawClient(node.clientId)) score += 10;else
  if (isLegacyClawdbotClient(node.clientId)) score -= 10;
  return score;
}
function resolveScoredMatches(nodes, query) {
  const trimmed = (0, _stringCoerceBUSzWgUA.s)(query);
  if (!trimmed) return [];
...
function resolveNodeIdFromNodeList(nodes, query, options = {}) {
  const q = (0, _stringCoerceBUSzWgUA.s)(query) ?? "";
  if (!q) {
