...
class TreeList {
  ...
  render(width) {
    ...
    if (this.filteredNodes.length === 0) {
      ...
      lines.push((0, _piTui.truncateToWidth)(_theme.theme.fg("muted", `  (0/0)${this.getStatusLabels()}`), width));
      return lines;
    }
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      const foldMarker = isFolded && !showsFoldInConnector ? _theme.theme.fg("accent", "⊞ ") : "";
      ...
      _theme.theme.fg("muted", `${this.formatLabelTimestamp(flatNode.node.labelTimestamp)} `) :
      "";
      const content = this.getEntryDisplayText(flatNode.node, isSelected);
      let line = cursor + _theme.theme.fg("dim", prefix) + foldMarker + pathMarker + label + labelTimestamp + content;
      if (isSelected) {
        line = _theme.theme.bg("selectedBg", line);
      }
      lines.push((0, _piTui.truncateToWidth)(line, width));
    }
    lines.push((0, _piTui.truncateToWidth)(_theme.theme.fg("muted", `  (${this.selectedIndex + 1}/${this.filteredNodes.length})${this.getStatusLabels()}`), width));
    return lines;
  }
  getEntryDisplayText(node, isSelected) {
    ...
    switch (entry.type) {
      ...
      case "session_info":
        result = entry.name ?
        [_theme.theme.fg("dim", "[title: "), _theme.theme.fg("dim", entry.name), _theme.theme.fg("dim", "]")].join("") :
        [_theme.theme.fg("dim", "[title: "), _theme.theme.italic(_theme.theme.fg("dim", "empty")), _theme.theme.fg("dim", "]")].join("");
        break;
      default:
        result = "";
    }
...
class LabelInput {
  ...
  render(width) {
    ...
    lines.push((0, _piTui.truncateToWidth)(`${indent}${(0, _keybindingHints.keyHint)("tui.select.confirm", "save")}  ${(0, _keybindingHints.keyHint)("tui.select.cancel", "cancel")}`, width));
    return lines;
  }
  handleInput(keyData) {
    const kb = (0, _piTui.getKeybindings)();
...
class TreeSelectorComponent extends _piTui.Container {
  ...
  constructor(tree, currentLeafId, terminalHeight, onSelect, onCancel, onLabelChange, initialSelectedId, initialFilterMode) {
    ...
    this.addChild(new _piTui.TruncatedText(_theme.theme.fg("muted", `  ↑/↓: move. ←/→: page. ^←/^→ or Alt+←/Alt+→: fold/branch. ${(0, _keybindingHints.keyText)("app.tree.editLabel")}: label. ^D/^T/^U/^L/^A: filters (^O/⇧^O cycle). ${(0, _keybindingHints.keyText)("app.tree.toggleLabelTimestamp")}: label time`), 0, 0));
...
}exports.TreeSelectorComponent = TreeSelectorComponent; /* v9-07d78e48f6b3f30f */
