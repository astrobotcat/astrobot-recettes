"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = uninstallPlugin;exports.n = resolveUninstallChannelConfigKeys;exports.r = resolveUninstallDirectoryTarget;exports.t = removePluginFromConfig;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
var _slotsMFUVcT3n = require("./slots-MFUVcT3n.js");
var _installCFYMfNJH = require("./install-CFYMfNJH.js");
...
function resolveUninstallDirectoryTarget(params) {
  ...
  let defaultPath;
  try {
    defaultPath = (0, _installCFYMfNJH.a)(params.pluginId, params.extensionsDir);
  } catch {
    return null;
  }
  const configuredPath = params.installRecord?.installPath;
  if (!configuredPath) return defaultPath;
  if (_nodePath.default.resolve(configuredPath) === _nodePath.default.resolve(defaultPath)) return configuredPath;
  return defaultPath;
}
...
function removePluginFromConfig(cfg, pluginId, opts) {
  ...
  if (slots?.memory === pluginId) {
    slots = {
      ...slots,
      memory: (0, _slotsMFUVcT3n.n)("memory")
    };
...
async function uninstallPlugin(params) {
  ...
  if (deleteTarget) {
    const existed = (await _promises.default.access(deleteTarget).then(() => true).catch(() => false)) ?? false;
    try {
      await _promises.default.rm(deleteTarget, {
        recursive: true,
        force: true
      });
      actions.directory = existed;
    } catch (error) {
      warnings.push(`Failed to remove plugin directory ${deleteTarget}: ${(0, _errorsD8p6rxH.i)(error)}`);
    }
  }
  return {
    ok: true,
