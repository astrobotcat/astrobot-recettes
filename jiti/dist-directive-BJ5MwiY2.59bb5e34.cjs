"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = takeDirectiveToken;exports.i = skipDirectiveArgPrefix;exports.n = normalizeQueueDropPolicy;exports.r = normalizeQueueMode;exports.t = extractQueueDirective;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
function normalizeQueueMode(raw) {
  const cleaned = (0, _stringCoerceBUSzWgUA.o)(raw);
  if (!cleaned) return;
...
function parseQueueDebounce(raw) {
  if (!raw) return;
  try {
    const parsed = (0, _parseDurationDHL2gXIv.t)(raw.trim(), { defaultUnit: "ms" });
...
function parseQueueDirectiveArgs(raw) {
  ...
  for (;;) {
    ...
    if (!token) break;
    const lowered = (0, _stringCoerceBUSzWgUA.o)(token);
    if (!lowered) break;
