...
function isThreadSessionKey(sessionKey) {
  return Boolean((0, _sessionThreadInfoLoadedBxSiO1jo.t)(sessionKey).threadId);
}
function resolveSessionResetType(params) {
  ...
  const normalized = (0, _stringCoerceBUSzWgUA.i)(params.sessionKey);
...
function resolveChannelResetConfig(params) {
  ...
  const normalized = (0, _messageChannelCBqCPFa_.u)(params.channel);
  const fallback = (0, _stringCoerceBUSzWgUA.o)(params.channel);
  const key = normalized ?? fallback;
  if (!key) return;
  return resetByChannel[key];
}
//#endregion /* v9-ffbcdf70a0a18e1b */
