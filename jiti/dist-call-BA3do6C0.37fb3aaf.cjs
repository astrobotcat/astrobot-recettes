"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = callGatewayLeastPrivilege;exports.c = randomIdempotencyKey;exports.i = callGatewayCli;exports.l = resolveExplicitGatewayAuth;exports.n = buildGatewayConnectionDetails;exports.o = callGatewayScoped;exports.r = callGateway;exports.s = ensureExplicitGatewayAuth;exports.t = void 0;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _clientDkWAat_P = require("./client-DkWAat_P.js");
...
require("./protocol-C6T5DFc8.js");
var _methodScopesD3xbsVVt = require("./method-scopes-D3xbsVVt.js");
var _gatewayBRe0gTJ = require("./gateway-BRe0gTJ4.js");
var _connectionDetailsDVKcA1hM = require("./connection-details-DVKcA1hM.js");
var _credentialsSecretInputsCW0my3bl = require("./credentials-secret-inputs-CW0my3bl.js");
var _explicitConnectionPolicyDz2zLNQA = require("./explicit-connection-policy-Dz2zLNQA.js");
var _nodeCrypto = require("node:crypto");
//#region src/gateway/call.ts
const defaultCreateGatewayClient = (opts) => new _clientDkWAat_P.t(opts);
const defaultGatewayCallDeps = {
  createGatewayClient: defaultCreateGatewayClient,
  loadConfig: _io5pxHCi7V.a,
  loadOrCreateDeviceIdentity: _deviceIdentityTBOlRcQx.n,
  resolveGatewayPort: _pathsDvv9VRAc.u,
  resolveConfigPath: _pathsDvv9VRAc.o,
  resolveStateDir: _pathsDvv9VRAc._,
  loadGatewayTlsRuntime: _gatewayBRe0gTJ.t
};
...
function resolveGatewayClientDisplayName(opts) {
  ...
  const clientName = opts.clientName ?? _messageChannelCBqCPFa_.g.CLI;
  if ((opts.mode ?? _messageChannelCBqCPFa_.h.CLI) !== _messageChannelCBqCPFa_.h.BACKEND && clientName !== _messageChannelCBqCPFa_.g.GATEWAY_CLIENT) return;
  const method = opts.method.trim();
  return method ? `gateway:${method}` : "gateway:request";
}
function loadGatewayConfig() {
  return (typeof gatewayCallDeps.loadConfig === "function" ? gatewayCallDeps.loadConfig : typeof defaultGatewayCallDeps.loadConfig === "function" ? defaultGatewayCallDeps.loadConfig : _io5pxHCi7V.a)();
}
function resolveGatewayStateDir(env) {
  return (typeof gatewayCallDeps.resolveStateDir === "function" ? gatewayCallDeps.resolveStateDir : _pathsDvv9VRAc._)(env);
}
function resolveGatewayConfigPath(env) {
  return (typeof gatewayCallDeps.resolveConfigPath === "function" ? gatewayCallDeps.resolveConfigPath : _pathsDvv9VRAc.o)(env, resolveGatewayStateDir(env));
}
function resolveGatewayPortValue(config, env) {
  return (typeof gatewayCallDeps.resolveGatewayPort === "function" ? gatewayCallDeps.resolveGatewayPort : _pathsDvv9VRAc.u)(config, env);
}
function buildGatewayConnectionDetails(options = {}) {
  return (0, _connectionDetailsDVKcA1hM.t)(options, {
...
function resolveGatewayCallContext(opts) {
  const cliUrlOverride = (0, _credentialPlannerCxtmVh4K.a)(opts.url);
  const explicitAuth = resolveExplicitGatewayAuth({
    token: opts.token,
    password: opts.password
  });
  const envUrlOverride = cliUrlOverride ? void 0 : (0, _credentialPlannerCxtmVh4K.a)(process.env.OPENCLAW_GATEWAY_URL);
  const urlOverride = cliUrlOverride ?? envUrlOverride;
  const urlOverrideSource = cliUrlOverride ? "cli" : envUrlOverride ? "env" : void 0;
  const canSkipConfigLoad = (0, _explicitConnectionPolicyDz2zLNQA.t)({
    config: opts.config,
    urlOverride,
    explicitAuth
  });
  ...
  return {
    ...
    urlOverride,
    urlOverrideSource,
    remoteUrl: (0, _credentialPlannerCxtmVh4K.a)(remote?.url),
    explicitAuth
  };
}
...
async function resolveGatewayCredentialsWithEnv(context, env) {
  ...
  return (0, _credentialsSecretInputsCW0my3bl.t)({
...
async function resolveGatewayTlsFingerprint(params) {
  ...
  const overrideTlsFingerprint = (0, _credentialPlannerCxtmVh4K.a)(opts.tlsFingerprint);
  const remoteTlsFingerprint = context.isRemoteMode && context.urlOverrideSource !== "cli" ? (0, _credentialPlannerCxtmVh4K.a)(context.remote?.tlsFingerprint) : void 0;
...
function formatGatewayCloseError(code, reason, connectionDetails) {
  const reasonText = (0, _stringCoerceBUSzWgUA.s)(reason) || "no close reason";
...
async function executeGatewayRequestWithScopes(params) {
  ...
  return await new Promise((resolve, reject) => {
    ...
    const client = gatewayCallDeps.createGatewayClient({
      ...
      clientName: opts.clientName ?? _messageChannelCBqCPFa_.g.CLI,
      clientDisplayName: resolveGatewayClientDisplayName(opts),
      clientVersion: opts.clientVersion ?? _versionBk5OWRN.n,
      platform: opts.platform,
      mode: opts.mode ?? _messageChannelCBqCPFa_.h.CLI,
      role: "operator",
      scopes,
...
async function callGatewayCli(opts) {
  return await callGatewayWithScopes(opts, Array.isArray(opts.scopes) ? opts.scopes : _methodScopesD3xbsVVt.t);
}
async function callGatewayLeastPrivilege(opts) {
  return await callGatewayWithScopes(opts, (0, _methodScopesD3xbsVVt.a)(opts.method));
}
async function callGateway(opts) {
  if (Array.isArray(opts.scopes)) return await callGatewayWithScopes(opts, opts.scopes);
  const callerMode = opts.mode ?? _messageChannelCBqCPFa_.h.BACKEND;
  const callerName = opts.clientName ?? _messageChannelCBqCPFa_.g.GATEWAY_CLIENT;
  if (callerMode === _messageChannelCBqCPFa_.h.CLI || callerName === _messageChannelCBqCPFa_.g.CLI) return await callGatewayCli(opts);
