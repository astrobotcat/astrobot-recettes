"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = registerAcpRuntimeBackend;exports.c = parseRuntimeTimeoutSecondsInput;exports.d = validateRuntimeModeInput;exports.f = validateRuntimeModelInput;exports.i = getAcpRuntimeBackend;exports.l = validateRuntimeConfigOptionInput;exports.m = resolveAcpSessionResolutionError;exports.n = getAcpSessionManager;exports.o = requireAcpRuntimeBackend;exports.p = validateRuntimePermissionProfileInput;exports.r = void 0;exports.s = unregisterAcpRuntimeBackend;exports.t = void 0;exports.u = validateRuntimeCwdInput;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
...
var _keyedAsyncQueueCx3Ado5F = require("./keyed-async-queue-Cx3Ado5F.js");
var _errorsDrSVrMCJ = require("./errors-DrSVrMCJ.js");
var _sessionIdentityDHNnkP = require("./session-identity-DHNnkP76.js");
var _sessionMetaDNDObzR = require("./session-meta-D-nDObzR.js");
var _nodePath = require("node:path");
//#region src/acp/control-plane/manager.utils.ts
function resolveAcpAgentFromSessionKey(sessionKey, fallback = "main") {
  return (0, _sessionKeyBh1lMwK.c)((0, _sessionKeyBh1lMwK.x)(sessionKey)?.agentId ?? fallback);
}
function resolveMissingMetaError(sessionKey) {
  return new _errorsDrSVrMCJ.n("ACP_SESSION_INIT_FAILED", `ACP metadata is missing for ${sessionKey}. Recreate this ACP session with /acp spawn and rebind the thread.`);
}
function resolveAcpSessionResolutionError(resolution) {
  ...
  return new _errorsDrSVrMCJ.n("ACP_SESSION_INIT_FAILED", `Session is not ACP-enabled: ${resolution.sessionKey}`);
}
...
function canonicalizeAcpSessionKey(params) {
  ...
  const lowered = (0, _stringCoerceBUSzWgUA.i)(normalized);
  if (lowered === "global" || lowered === "unknown") return lowered;
  const parsed = (0, _sessionKeyBh1lMwK.x)(lowered);
  if (parsed) return (0, _mainSessionDtefsIzj.t)({
    cfg: params.cfg,
    agentId: parsed.agentId,
    sessionKey: lowered
  });
  const mainKey = (0, _sessionKeyBh1lMwK.l)(params.cfg.session?.mainKey);
  if (lowered === "main" || lowered === mainKey) return (0, _mainSessionDtefsIzj.i)(params.cfg);
  return lowered;
}
...
function normalizeAcpErrorCode(code) {
  ...
  for (const allowed of _errorsDrSVrMCJ.t) if (allowed === normalized) return allowed;
  return "ACP_TURN_FAILED";
}
function createUnsupportedControlError(params) {
  return new _errorsDrSVrMCJ.n("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${params.backend}" does not support ${params.control}.`);
}
...
async function reconcileManagerRuntimeSessionIdentifiers(params) {
  ...
  if (!runtimeStatus && params.runtime.getStatus) try {
    runtimeStatus = await (0, _errorsDrSVrMCJ.a)({
    ...
    (0, _globalsDe6QTwLG.r)(`acp-manager: failed to refresh ACP runtime status for ${params.sessionKey}: ${String(error)}`);
    return {
  ...
  const now = Date.now();
  const currentIdentity = (0, _sessionIdentityDHNnkP.u)(params.meta);
  const identityAfterEvent = (0, _sessionIdentityDHNnkP.s)({
    current: currentIdentity,
    incoming: (0, _sessionIdentityDHNnkP.n)({
      handle: params.handle,
      now
    }),
    now
  }) ?? currentIdentity;
  const nextIdentity = (0, _sessionIdentityDHNnkP.s)({
    current: identityAfterEvent,
    incoming: (0, _sessionIdentityDHNnkP.r)({
      status: runtimeStatus,
      now
    }),
    now
  }) ?? identityAfterEvent;
  const handleIdentifiers = (0, _sessionIdentityDHNnkP.c)(nextIdentity);
  ...
  if (!(!(0, _sessionIdentityDHNnkP.i)(currentIdentity, nextIdentity) || hasLegacyAcpIdentityProjection(params.meta))) return {
    handle: nextHandle,
    meta: params.meta,
    runtimeStatus
  };
  ...
  if (!(0, _sessionIdentityDHNnkP.i)(currentIdentity, nextIdentity)) {
    ...
    (0, _globalsDe6QTwLG.r)(`acp-manager: session identity updated for ${params.sessionKey} (agentSessionId ${currentAgentSessionId} -> ${nextAgentSessionId}, acpxSessionId ${currentAcpxSessionId} -> ${nextAcpxSessionId}, acpxRecordId ${currentAcpxRecordId} -> ${nextAcpxRecordId})`);
  }
...
function failInvalidOption(message) {
  throw new _errorsDrSVrMCJ.n("ACP_INVALID_RUNTIME_OPTION", message);
}
...
function validateBoundedText(params) {
  const normalized = (0, _stringCoerceBUSzWgUA.s)(params.value);
...
function validateRuntimeCwdInput(rawCwd) {
  ...
  if (!(0, _nodePath.isAbsolute)(cwd)) failInvalidOption(`Working directory must be an absolute path. Received "${cwd}".`);
  return cwd;
}
...
function parseRuntimeTimeoutSecondsInput(rawTimeout) {
  const normalized = (0, _stringCoerceBUSzWgUA.s)(rawTimeout);
...
function validateRuntimeOptionPatch(patch) {
  ...
  const next = {};
  if (Object.hasOwn(rawPatch, "runtimeMode")) if (rawPatch.runtimeMode === void 0) next.runtimeMode = void 0;else
  next.runtimeMode = validateRuntimeModeInput(rawPatch.runtimeMode);
  if (Object.hasOwn(rawPatch, "model")) if (rawPatch.model === void 0) next.model = void 0;else
  next.model = validateRuntimeModelInput(rawPatch.model);
  if (Object.hasOwn(rawPatch, "cwd")) if (rawPatch.cwd === void 0) next.cwd = void 0;else
  next.cwd = validateRuntimeCwdInput(rawPatch.cwd);
  if (Object.hasOwn(rawPatch, "permissionProfile")) if (rawPatch.permissionProfile === void 0) next.permissionProfile = void 0;else
  next.permissionProfile = validateRuntimePermissionProfileInput(rawPatch.permissionProfile);
  if (Object.hasOwn(rawPatch, "timeoutSeconds")) if (rawPatch.timeoutSeconds === void 0) next.timeoutSeconds = void 0;else
  ...
  if (Object.hasOwn(rawPatch, "backendExtras")) {
    ...
    if (rawExtras === void 0) next.backendExtras = void 0;else
    if (!rawExtras || typeof rawExtras !== "object" || Array.isArray(rawExtras)) failInvalidOption("Backend extras must be a key/value object.");else
    {
...
function normalizeRuntimeOptions(options) {
  const runtimeMode = (0, _stringCoerceBUSzWgUA.s)(options?.runtimeMode);
  const model = (0, _stringCoerceBUSzWgUA.s)(options?.model);
  const cwd = (0, _stringCoerceBUSzWgUA.s)(options?.cwd);
  const permissionProfile = (0, _stringCoerceBUSzWgUA.s)(options?.permissionProfile);
  let timeoutSeconds;
  ...
  const backendExtrasEntries = Object.entries(options?.backendExtras ?? {}).map(([key, value]) => [(0, _stringCoerceBUSzWgUA.s)(key), (0, _stringCoerceBUSzWgUA.s)(value)]).filter(([key, value]) => Boolean(key && value));
...
function inferRuntimeOptionPatchFromConfigOption(key, value) {
  ...
  const normalizedKey = (0, _stringCoerceBUSzWgUA.i)(validated.key);
...
async function resolveManagerRuntimeCapabilities(params) {
  let reported;
  if (params.runtime.getCapabilities) reported = await (0, _errorsDrSVrMCJ.a)({
  ...
  const normalizedKeys = (reported?.configOptionKeys ?? []).map((entry) => (0, _stringCoerceBUSzWgUA.s)(entry)).filter(Boolean);
  return {
...
async function applyManagerRuntimeControls(params) {
  ...
  const advertisedKeys = new Set((capabilities.configOptionKeys ?? []).map((entry) => (0, _stringCoerceBUSzWgUA.s)(entry)).filter(Boolean));
  await (0, _errorsDrSVrMCJ.a)({
    run: async () => {
      if (runtimeMode) {
      ...
      if (configOptions.length > 0) {
        ...
        for (const [key, value] of configOptions) {
          if (advertisedKeys.size > 0 && !advertisedKeys.has(key)) throw new _errorsDrSVrMCJ.n("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${backend}" does not accept config key "${key}".`);
...
function resolveAcpRuntimeRegistryGlobalState() {
  return (0, _globalSingletonB80lDOJ.n)(ACP_RUNTIME_REGISTRY_STATE_KEY, () => ({ backendsById: /* @__PURE__ */new Map() }));
}
...
function registerAcpRuntimeBackend(backend) {
  const id = (0, _stringCoerceBUSzWgUA.o)(backend.id) || "";
...
function requireAcpRuntimeBackend(id) {
  ...
  if (!backend) throw new _errorsDrSVrMCJ.n("ACP_BACKEND_MISSING", "ACP runtime backend is not configured. Install and enable the acpx runtime plugin.");
  if (!isBackendHealthy(backend)) throw new _errorsDrSVrMCJ.n("ACP_BACKEND_UNAVAILABLE", "ACP runtime backend is currently unavailable. Try again in a moment.");
  if (normalized && backend.id !== normalized) throw new _errorsDrSVrMCJ.n("ACP_BACKEND_MISSING", `ACP runtime backend "${normalized}" is not registered.`);
  return backend;
}
const __testing$1 = exports.r = {
...
const DEFAULT_DEPS = {
  listAcpSessions: _sessionMetaDNDObzR.t,
  readSessionEntry: _sessionMetaDNDObzR.n,
  upsertSessionMeta: _sessionMetaDNDObzR.i,
...
var SessionActorQueue = class {
  constructor() {
    this.queue = new _keyedAsyncQueueCx3Ado5F.t();
  ...
  async run(actorKey, op) {
    return this.queue.enqueue(actorKey, op, {
      ...
      onSettle: () => {
        ...
        if (pending <= 0) this.pendingBySession.delete(actorKey);else
...
function summarizeBackgroundTaskText(text) {
  const normalized = (0, _stringCoerceBUSzWgUA.s)(text) ?? "ACP background task";
...
function appendBackgroundTaskProgressSummary(current, chunk) {
  const normalizedChunk = (0, _stringCoerceBUSzWgUA.s)(chunk)?.replace(/\s+/g, " ");
...
function resolveBackgroundTaskTerminalResult(progressSummary) {
  const normalized = (0, _stringCoerceBUSzWgUA.s)(progressSummary)?.replace(/\s+/g, " ").trim();
  if (!normalized) return {};
  const permissionDeniedMatch = normalized.match(/\b(?:write failed:\s*)?permission denied(?: for (?<path>\S+))?\.?/i);
  if (permissionDeniedMatch) {
    const path = (0, _stringCoerceBUSzWgUA.s)(permissionDeniedMatch.groups?.path)?.replace(/[.,;:!?]+$/, "");
    return {
...
var AcpSessionManager = class {
  ...
  resolveSession(params) {
    ...
    if ((0, _sessionKeyBh1lMwK._)(sessionKey)) return {
      kind: "stale",
      sessionKey,
  ...
  async reconcilePendingSessionIdentities(params) {
    ...
    try {
      ...
      (0, _globalsDe6QTwLG.r)(`acp-manager: startup identity scan failed: ${String(error)}`);
      return {
        checked,
        resolved,
        failed: failed + 1
    ...
    for (const session of acpSessions) {
      ...
      const currentIdentity = (0, _sessionIdentityDHNnkP.u)(session.acp);
      if (!(0, _sessionIdentityDHNnkP.o)(currentIdentity) || !(0, _sessionIdentityDHNnkP.a)(currentIdentity)) continue;
      checked += 1;
      try {
        if (await this.withSessionActor(session.sessionKey, async () => {
          ...
          return !(0, _sessionIdentityDHNnkP.o)((0, _sessionIdentityDHNnkP.u)((await this.reconcileRuntimeSessionIdentifiers({
            cfg: params.cfg,
            sessionKey: session.sessionKey,
            runtime,
            handle,
        ...
        failed += 1;
        (0, _globalsDe6QTwLG.r)(`acp-manager: startup identity reconcile failed for ${session.sessionKey}: ${String(error)}`);
      }
    }
    return {
      checked,
  ...
  async initializeSession(input) {
    ...
    if (!sessionKey) throw new _errorsDrSVrMCJ.n("ACP_SESSION_INIT_FAILED", "ACP session key is required.");
    const agent = (0, _sessionKeyBh1lMwK.c)(input.agent);
    ...
    return await this.withSessionActor(sessionKey, async () => {
      ...
      const handle = await (0, _errorsDrSVrMCJ.a)({
      ...
      const effectiveCwd = (0, _stringCoerceBUSzWgUA.s)(handle.cwd) ?? requestedCwd;
      const effectiveRuntimeOptions = normalizeRuntimeOptions({
        ...initialRuntimeOptions,
        ...(effectiveCwd ? { cwd: effectiveCwd } : {})
      });
      const identityNow = Date.now();
      const initializedIdentity = (0, _sessionIdentityDHNnkP.s)({
        current: void 0,
        incoming: (0, _sessionIdentityDHNnkP.t)({
          handle,
          now: identityNow
        }),
        now: identityNow
      ...
      try {
        ...
        await runtime.close({
          ...
          (0, _globalsDe6QTwLG.r)(`acp-manager: cleanup close failed after metadata write error for ${sessionKey}: ${String(closeError)}`);
        });
        throw error;
      }
      if (!persisted?.acp) {
        ...
        throw new _errorsDrSVrMCJ.n("ACP_SESSION_INIT_FAILED", `Could not persist ACP metadata for ${sessionKey}.`);
      }
  ...
  async getSessionStatus(params) {
    ...
    return await this.withSessionActor(sessionKey, async () => {
      ...
      let runtimeStatus;
      if (runtime.getStatus) runtimeStatus = await (0, _errorsDrSVrMCJ.a)({
        run: async () => {
          this.throwIfAborted(params.signal);
      ...
      const identity = (0, _sessionIdentityDHNnkP.u)(meta);
      return {
        sessionKey,
  ...
  async setSessionConfigOption(params) {
    ...
    return await this.withSessionActor(sessionKey, async () => {
      ...
      const advertisedKeys = new Set((capabilities.configOptionKeys ?? []).map((entry) => (0, _stringCoerceBUSzWgUA.s)(entry)).filter(Boolean));
      if (advertisedKeys.size > 0 && !advertisedKeys.has(key)) throw new _errorsDrSVrMCJ.n("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${handle.backend || meta.backend}" does not accept config key "${key}".`);
      await (0, _errorsDrSVrMCJ.a)({
    ...
    await this.withSessionActor(sessionKey, async () => {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
          ...
          if (input.signal?.aborted) internalAbortController.abort();else
          ...
          const turnPromise = (async () => {
            for await (const event of runtime.runTurn({
              ...
              if (event.type === "error") streamError = new _errorsDrSVrMCJ.n(normalizeAcpErrorCode(event.code), (0, _stringCoerceBUSzWgUA.s)(event.message) || "ACP turn failed before completion.");else
          ...
          const acpError = (0, _errorsDrSVrMCJ.i)({
            error,
          ...
          if (!retryFreshHandle && !skipPostTurnCleanup && runtime && handle && meta && meta.mode === "oneshot") try {
            ...
            (0, _globalsDe6QTwLG.r)(`acp-manager: ACP oneshot close failed for ${sessionKey}: ${String(error)}`);
          } finally {
            this.clearCachedRuntimeState(sessionKey);
          }
        }
    ...
    try {
      ...
      if (outcome === timeoutToken) {
        observedTurnPromise.then((lateOutcome) => {
          if (lateOutcome.kind === "error") (0, _globalsDe6QTwLG.r)(`acp-manager: detached late turn error after timeout for ${params.sessionKey}: ${String(lateOutcome.error)}`);
        });
        await params.onTimeout();
        throw new _errorsDrSVrMCJ.n("ACP_TURN_FAILED", `ACP turn timed out after ${Math.max(1, Math.round(params.timeoutLabelMs / 1e3))}s.`);
      }
      if (outcome.kind === "error") throw outcome.error;
      return outcome.value;
    } finally {
    ...
    try {
      ...
      if (outcome === timeoutToken) {
        observedCleanupPromise.then((lateOutcome) => {
          if (lateOutcome.kind === "error") (0, _globalsDe6QTwLG.r)(`acp-manager: detached timed-out turn ${params.label} cleanup failed for ${params.sessionKey}: ${String(lateOutcome.error)}`);
        });
        (0, _globalsDe6QTwLG.r)(`acp-manager: timed-out turn ${params.label} cleanup exceeded ${ACP_TURN_TIMEOUT_CLEANUP_GRACE_MS}ms for ${params.sessionKey}`);
        return false;
      }
      if (outcome.kind === "error") (0, _globalsDe6QTwLG.r)(`acp-manager: timed-out turn ${params.label} cleanup failed for ${params.sessionKey}: ${String(outcome.error)}`);
      return true;
    } finally {
      if (timer) clearTimeout(timer);
    }
  ...
  async closeSession(input) {
    ...
    return await this.withSessionActor(sessionKey, async () => {
      ...
      const currentIdentity = (0, _sessionIdentityDHNnkP.u)(meta);
      const shouldSkipRuntimeClose = input.discardPersistentState && currentIdentity != null && !(0, _sessionIdentityDHNnkP.a)(currentIdentity);
      ...
      if (shouldSkipRuntimeClose) {
        if (input.discardPersistentState) {
          ...
          try {
            ...
            (0, _globalsDe6QTwLG.r)(`acp close fast-reset: unable to prepare fresh session for ${sessionKey}: ${error instanceof Error ? error.message : String(error)}`);
          }
        }
        this.clearCachedRuntimeState(sessionKey);
      } else try {
        ...
        if (input.allowBackendUnavailable && (acpError.code === "ACP_BACKEND_MISSING" || acpError.code === "ACP_BACKEND_UNAVAILABLE" || input.discardPersistentState && acpError.code === "ACP_SESSION_INIT_FAILED" || this.isRecoverableAcpxExitError(acpError.message))) {
          if (input.discardPersistentState) {
            ...
            try {
              ...
              (0, _globalsDe6QTwLG.r)(`acp close recovery: unable to prepare fresh session for ${sessionKey}: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`);
            }
          }
          this.clearCachedRuntimeState(sessionKey);
  ...
  async ensureRuntimeHandle(params) {
    const agent = (0, _stringCoerceBUSzWgUA.s)(params.meta.agent) || resolveAcpAgentFromSessionKey(params.sessionKey, "main");
    const mode = params.meta.mode;
    const runtimeOptions = resolveRuntimeOptionsFromMeta(params.meta);
    const cwd = runtimeOptions.cwd ?? (0, _stringCoerceBUSzWgUA.s)(params.meta.cwd);
    ...
    const previousIdentity = (0, _sessionIdentityDHNnkP.u)(previousMeta);
    let identityForEnsure = previousIdentity;
    const persistedResumeSessionId = mode === "persistent" ? (0, _sessionIdentityDHNnkP.l)(previousIdentity) : void 0;
    const shouldPrepareFreshPersistentSession = mode === "persistent" && previousIdentity != null && !(0, _sessionIdentityDHNnkP.a)(previousIdentity);
    const ensureSession = async (resumeSessionId) => await (0, _errorsDrSVrMCJ.a)({
      run: async () => await runtime.ensureSession({
        sessionKey: params.sessionKey,
        agent,
        mode,
    ...
    if (persistedResumeSessionId) try {
      ...
      (0, _globalsDe6QTwLG.r)(`acp-manager: resume init failed for ${params.sessionKey}; retrying without persisted ACP session id: ${acpError.message}`);
    ...
    ensured = await ensureSession();
    const now = Date.now();
    const effectiveCwd = (0, _stringCoerceBUSzWgUA.s)(ensured.cwd) ?? cwd;
    ...
    const nextHandleIdentifiers = (0, _sessionIdentityDHNnkP.c)(nextIdentity);
    const nextHandle = {
      ...ensured,
  ...
  async isCachedRuntimeHandleReusable(params) {
    ...
    try {
      ...
      if (this.isRuntimeStatusUnavailable(status)) {
        this.clearCachedRuntimeState(params.sessionKey);
        (0, _globalsDe6QTwLG.r)(`acp-manager: evicting cached runtime handle for ${params.sessionKey} after unhealthy status probe: ${status.summary ?? "status unavailable"}`);
        return false;
      }
      return true;
    } catch (error) {
      this.clearCachedRuntimeState(params.sessionKey);
      (0, _globalsDe6QTwLG.r)(`acp-manager: evicting cached runtime handle for ${params.sessionKey} after status probe failed: ${String(error)}`);
      return false;
    }
  }
  isRuntimeStatusUnavailable(status) {
    if (!status) return false;
    const detailsStatus = (0, _stringCoerceBUSzWgUA.i)(status.details?.status);
    if (detailsStatus === "dead" || detailsStatus === "no-session") return true;
    const summaryMatch = status.summary?.match(/\bstatus=([^\s]+)/i);
    const summaryStatus = (0, _stringCoerceBUSzWgUA.i)(summaryMatch?.[1]);
  ...
  enforceConcurrentSessionLimit(params) {
    ...
    if (activeCount >= limit) throw new _errorsDrSVrMCJ.n("ACP_SESSION_INIT_FAILED", `ACP max concurrent sessions reached (${activeCount}/${limit}).`);
  }
  ...
  async prepareFreshHandleRetry(params) {
    ...
    if (this.isRecoverableAcpxExitError(params.error.message)) {
      this.clearCachedRuntimeState(params.sessionKey);
      (0, _globalsDe6QTwLG.r)(`acp-manager: retrying ${params.sessionKey} with a fresh runtime handle after early turn failure: ${params.error.message}`);
      return true;
    }
    ...
    if (params.runtime.prepareFreshSession) try {
      ...
      (0, _globalsDe6QTwLG.r)(`acp-manager: failed preparing a fresh persistent session for ${params.sessionKey}: ${(0, _errorsD8p6rxH.i)(error)}`);
      return false;
    }
    this.clearCachedRuntimeState(params.sessionKey);
    (0, _globalsDe6QTwLG.r)(`acp-manager: retrying ${params.sessionKey} with a fresh persistent session after missing backend resume target: ${params.error.message}`);
    return true;
  }
  ...
  async clearPersistedRuntimeResumeState(params) {
    ...
    if (!(await this.writeSessionMeta({
      ...
      mutate: (current, entry) => {
        ...
        const base = current ?? entry.acp;
        if (!base) return null;
        const currentIdentity = (0, _sessionIdentityDHNnkP.u)(base);
      ...
      (0, _globalsDe6QTwLG.r)(`acp-manager: unable to clear persisted runtime resume state for ${params.sessionKey}`);
      return false;
    }
    return true;
  }
  ...
  async evictIdleRuntimeHandles(params) {
    ...
    for (const candidate of candidates) await this.actorQueue.run(candidate.actorKey, async () => {
      ...
      try {
        ...
        (0, _globalsDe6QTwLG.r)(`acp-manager: idle eviction close failed for ${candidate.state.handle.sessionKey}: ${String(error)}`);
      }
    });
  }
  ...
  async setSessionState(params) {
    await this.writeSessionMeta({
      ...
      mutate: (current, entry) => {
        ...
        const lastError = (0, _stringCoerceBUSzWgUA.s)(params.lastError);
        if (lastError) next.lastError = lastError;else
        if (params.clearLastError) delete next.lastError;
        return next;
      }
    });
  }
  ...
  async writeSessionMeta(params) {
    try {
      ...
      (0, _globalsDe6QTwLG.r)(`acp-manager: failed persisting ACP metadata for ${params.sessionKey}: ${String(error)}`);
      return null;
    }
  }
  ...
  throwIfAborted(signal) {
    ...
    throw new _errorsDrSVrMCJ.n("ACP_TURN_FAILED", "ACP operation aborted.");
  }
  ...
  runtimeHandleMatchesMeta(params) {
    const identity = (0, _sessionIdentityDHNnkP.u)(params.meta);
    const expectedHandleIds = (0, _sessionIdentityDHNnkP.c)(identity);
    ...
    return ((0, _stringCoerceBUSzWgUA.s)(params.handle.acpxRecordId) ?? "") === expectedAcpxRecordId;
  }
  resolveBackgroundTaskContext(params) {
    ...
    const requesterSessionKey = (0, _stringCoerceBUSzWgUA.s)(childEntry?.spawnedBy) ?? (0, _stringCoerceBUSzWgUA.s)(childEntry?.parentSessionKey);
    ...
    return {
      ...
      label: (0, _stringCoerceBUSzWgUA.s)(childEntry?.label),
  ...
  createBackgroundTaskRecord(context, startedAt) {
    try {
      ...
      (0, _globalsDe6QTwLG.r)(`acp-manager: failed creating background task for ${context.runId}: ${String(error)}`);
    }
  }
  markBackgroundTaskRunning(runId, params) {
    try {
      (0, _taskExecutorZyzAPLM.d)({
        runId,
        runtime: "acp",
        sessionKey: params.sessionKey,
      ...
      (0, _globalsDe6QTwLG.r)(`acp-manager: failed updating background task for ${runId}: ${String(error)}`);
    }
  }
