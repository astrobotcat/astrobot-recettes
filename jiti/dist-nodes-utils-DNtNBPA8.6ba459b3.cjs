"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = selectDefaultNodeFromList;exports.i = resolveNodeIdFromList;exports.n = resolveNode;exports.r = resolveNodeId;exports.t = listNodes;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _gatewayB1XKgomf = require("./gateway-B1XKgomf.js");
var _nodeResolveDDVHjGf = require("./node-resolve-DDVHjGf1.js");
...
function shouldFallbackToPairList(error) {
  const message = (0, _stringCoerceBUSzWgUA.o)(messageFromError(error)) ?? "";
...
async function loadNodes(opts) {
  try {
    return (0, _nodeResolveDDVHjGf.r)(await (0, _gatewayB1XKgomf.t)("node.list", opts, {}));
  } catch (error) {
    if (!shouldFallbackToPairList(error)) throw error;
    const { paired } = (0, _nodeResolveDDVHjGf.i)(await (0, _gatewayB1XKgomf.t)("node.pair.list", opts, {}));
    return paired.map((n) => ({
      nodeId: n.nodeId,
...
function isLocalMacNode(node) {
  return (0, _stringCoerceBUSzWgUA.o)(node.platform)?.startsWith("mac") === true && typeof node.nodeId === "string" && node.nodeId.startsWith("mac-");
}
...
function resolveNodeIdFromList(nodes, query, allowDefault = false) {
  return (0, _nodeResolveDDVHjGf.n)(nodes, query, {
    allowDefault,
    pickDefaultNode
  });
}
...
async function resolveNode(opts, query, allowDefault = false) {
  return (0, _nodeResolveDDVHjGf.t)(await loadNodes(opts), query, {
    allowDefault,
    pickDefaultNode
  });
}
//#endregion /* v9-56e10a9aa4bd6eee */
