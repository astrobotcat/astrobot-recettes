"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = normalizeToolParameters;exports.t = normalizeToolParameterSchema;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
var _providerModelCompatDsxuyzi = require("./provider-model-compat-Dsxuyzi4.js");
var _channelToolsDdZyHuyK = require("./channel-tools-DdZyHuyK.js");
var _providerToolsZ_yR2St = require("./provider-tools-Z_yR2St8.js");
...
function normalizeToolParameterSchema(schema, options) {
  ...
  const normalizedProvider = (0, _stringCoerceBUSzWgUA.i)(options?.modelProvider);
  const isGeminiProvider = normalizedProvider.includes("google") || normalizedProvider.includes("gemini");
  const isAnthropicProvider = normalizedProvider.includes("anthropic");
  const unsupportedToolSchemaKeywords = (0, _providerModelCompatDsxuyzi.o)(options?.modelCompat);
  function applyProviderCleaning(s) {
    if (isGeminiProvider && !isAnthropicProvider) return (0, _providerToolsZ_yR2St.g)(s);
    if (unsupportedToolSchemaKeywords.size > 0) return (0, _providerToolsZ_yR2St.p)(s, unsupportedToolSchemaKeywords);
    return s;
  }
...
function normalizeToolParameters(tool, options) {
  function preserveToolMeta(target) {
    (0, _channelToolsDdZyHuyK.l)(tool, target);
    (0, _channelToolsDdZyHuyK.t)(tool, target);
    return target;
  }
