"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveInboundLastRouteSessionKey;exports.i = resolveAgentRoute;exports.n = deriveLastRoutePolicy;exports.r = pickFirstExistingAgentId;exports.t = buildAgentSessionKey;var _stringCoerceBUSzWgUA = require("./string-coerce-BUSzWgUA.js");
...
var _bindingsUP5iyfKc = require("./bindings-uP5iyfKc.js");
...
function normalizeToken(value) {
  return (0, _stringCoerceBUSzWgUA.i)(value);
}
...
function buildAgentSessionKey(params) {
  ...
  return (0, _sessionKeyBh1lMwK.i)({
    agentId: params.agentId,
    mainKey: _sessionKeyBh1lMwK.n,
    channel,
...
function resolveAgentLookupCache(cfg) {
  ...
  for (const agent of listAgents(cfg)) {
    ...
    if (!rawId) continue;
    byNormalizedId.set((0, _sessionKeyBh1lMwK.c)(rawId), (0, _sessionKeyBh1lMwK.f)(rawId));
  }
  const next = {
    agentsRef,
    byNormalizedId,
    fallbackDefaultAgentId: (0, _sessionKeyBh1lMwK.f)((0, _agentScopeKFH9bkHi.x)(cfg))
  };
...
function pickFirstExistingAgentId(cfg, agentId) {
  ...
  const normalized = (0, _sessionKeyBh1lMwK.c)(trimmed);
  if (lookup.byNormalizedId.size === 0) return (0, _sessionKeyBh1lMwK.f)(trimmed);
...
function resolveAccountPatternKey(accountPattern) {
  if (!accountPattern.trim()) return _accountIdJ7GeQlaZ.t;
  return (0, _accountIdJ7GeQlaZ.n)(accountPattern);
}
function buildEvaluatedBindingsByChannel(cfg) {
  const byChannel = /* @__PURE__ */new Map();
  let order = 0;
  for (const binding of (0, _bindingsUP5iyfKc.n)(cfg)) {
...
function normalizePeerConstraint(peer) {
  ...
  const kind = (0, _chatTypeDFnPOWna.t)(peer.kind);
...
function resolveAgentRoute(input) {
  ...
  const accountId = (0, _accountIdJ7GeQlaZ.n)(input.accountId);
  const peer = input.peer ? {
    kind: (0, _chatTypeDFnPOWna.t)(input.peer.kind) ?? input.peer.kind,
  ...
  const shouldLogDebug = (0, _globalsDe6QTwLG.a)();
  const parentPeer = input.parentPeer ? {
    kind: (0, _chatTypeDFnPOWna.t)(input.parentPeer.kind) ?? input.parentPeer.kind,
  ...
  const choose = (agentId, matchedBy) => {
    ...
    const sessionKey = (0, _stringCoerceBUSzWgUA.i)(buildAgentSessionKey({
    ...
    const mainSessionKey = (0, _stringCoerceBUSzWgUA.i)((0, _sessionKeyBh1lMwK.r)({
      agentId: resolvedAgentId,
      mainKey: _sessionKeyBh1lMwK.n
    }));
    const route = {
  ...
  if (shouldLogDebug) {
    (0, _loggerBA_TvTc.t)(`[routing] resolveAgentRoute: channel=${channel} accountId=${accountId} peer=${formatPeer(peer)} guildId=${guildId || "none"} teamId=${teamId || "none"} bindings=${bindings.length}`);
    for (const entry of bindings) (0, _loggerBA_TvTc.t)(`[routing] binding: agentId=${entry.binding.agentId} accountPattern=${entry.match.accountPattern || "default"} peer=${formatNormalizedPeer(entry.match.peer)} guildId=${entry.match.guildId ?? "none"} teamId=${entry.match.teamId ?? "none"} roles=${entry.match.roles?.length ?? 0}`);
  }
  const baseScope = {
    guildId,
    teamId,
  ...
  for (const tier of tiers) {
    ...
    if (matched) {
      if (shouldLogDebug) (0, _loggerBA_TvTc.t)(`[routing] match: matchedBy=${tier.matchedBy} agentId=${matched.binding.agentId}`);
      return choose(matched.binding.agentId, tier.matchedBy);
    }
  }
  return choose((0, _agentScopeKFH9bkHi.x)(input.cfg), "default");
}
//#endregion /* v9-f360f4302b37a944 */
