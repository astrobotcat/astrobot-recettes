...
class ModelSelectorComponent extends _piTui.Container {
  ...
  filterModels(query) {
    ...
    (0, _piTui.fuzzyFilter)(this.activeModels, query, ({ id, provider }) => `${id} ${provider} ${provider}/${id} ${provider} ${id}`) :
    this.activeModels;
    this.selectedIndex = Math.min(this.selectedIndex, Math.max(0, this.filteredModels.length - 1));
    this.updateList();
  }
...
}exports.ModelSelectorComponent = ModelSelectorComponent; /* v9-074cef65b5ad3db7 */
