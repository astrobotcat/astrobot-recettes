"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveSecretRefValue;exports.i = resolveSecretRefString;exports.n = void 0;exports.o = resolveSecretRefValues;exports.r = isProviderScopedSecretResolutionError;exports.t = void 0;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
var _utilsD5DtWkEu = require("./utils-D5DtWkEu.js");
var _refContractB0QmVSlT = require("./ref-contract-B0QmVSlT.js");
var _auditFsCma03vGs = require("./audit-fs-Cma03vGs.js");
var _scanPathsDTbstpEd = require("./scan-paths-DTbstpEd.js");
var _runWithConcurrencyDiVVaJMi = require("./run-with-concurrency-DiVVaJMi.js");
var _jsonPointerLsoZMCI = require("./json-pointer-LsoZMCI5.js");
...
function throwUnknownProviderResolutionError(params) {
  ...
  throw providerResolutionError({
    ...
    message: (0, _errorsD8p6rxH.i)(params.err),
    cause: params.err
  });
}
async function readFileStatOrThrow(pathname, label) {
  const stat = await (0, _auditFsCma03vGs.i)(pathname);
...
function resolveResolutionLimits(config) {
  ...
  return {
    maxProviderConcurrency: (0, _sharedDLDubI1E.r)(resolution?.maxProviderConcurrency, DEFAULT_PROVIDER_CONCURRENCY),
    maxRefsPerProvider: (0, _sharedDLDubI1E.r)(resolution?.maxRefsPerProvider, DEFAULT_MAX_REFS_PER_PROVIDER),
    maxBatchBytes: (0, _sharedDLDubI1E.r)(resolution?.maxBatchBytes, DEFAULT_MAX_BATCH_BYTES)
  };
}
...
function resolveConfiguredProvider(ref, config) {
  ...
  if (!providerConfig) {
    if (ref.source === "env" && ref.provider === (0, _refContractB0QmVSlT.l)(config, "env")) return { source: "env" };
    throw providerResolutionError({
      source: ref.source,
...
async function assertSecurePath(params) {
  ...
  if (params.trustedDirs && params.trustedDirs.length > 0) {
    if (!params.trustedDirs.map((entry) => (0, _utilsD5DtWkEu.m)(entry)).some((dir) => (0, _scanPathsDTbstpEd.n)(dir, effectivePath))) throw new Error(`${params.label} is outside trustedDirs: ${effectivePath}`);
  }
  if (params.allowInsecurePath) return effectivePath;
  const perms = await (0, _auditFsCma03vGs.r)(effectivePath);
...
async function readFileProviderPayload(params) {
  ...
  const filePath = (0, _utilsD5DtWkEu.m)(params.providerConfig.path);
  const readPromise = (async () => {
    const secureFilePath = await assertSecurePath({
      targetPath: filePath,
      label: `secrets.providers.${params.providerName}.path`
    });
    const timeoutMs = (0, _sharedDLDubI1E.r)(params.providerConfig.timeoutMs, DEFAULT_FILE_TIMEOUT_MS);
    const maxBytes = (0, _sharedDLDubI1E.r)(params.providerConfig.maxBytes, DEFAULT_FILE_MAX_BYTES);
    ...
    try {
      ...
      if (!(0, _utilsD5DtWkEu.l)(parsed)) throw new Error(`File provider "${params.providerName}" payload is not a JSON object.`);
      return parsed;
    } catch (error) {
...
async function resolveEnvRefs(params) {
  ...
  for (const ref of params.refs) {
    ...
    if (!(0, _sharedDLDubI1E.n)(envValue)) throw refResolutionError({
      source: "env",
      provider: params.providerName,
      refId: ref.id,
...
async function resolveFileRefs(params) {
  ...
  if (mode === "singleValue") {
    for (const ref of params.refs) {
      if (ref.id !== "value") throw refResolutionError({
        ...
        provider: params.providerName,
        refId: ref.id,
        message: `singleValue file provider "${params.providerName}" expects ref id "${_refContractB0QmVSlT.i}".`
      });
  ...
  for (const ref of params.refs) try {
    resolved.set(ref.id, (0, _jsonPointerLsoZMCI.n)(payload, ref.id, { onMissing: "throw" }));
  } catch (err) {
    throw refResolutionError({
      source: "file",
      provider: params.providerName,
      refId: ref.id,
      message: (0, _errorsD8p6rxH.i)(err),
      cause: err
    });
  }
  return resolved;
...
function parseExecValues(params) {
  ...
  if (!(0, _utilsD5DtWkEu.l)(responseValues)) throw providerResolutionError({
    source: "exec",
    provider: params.providerName,
    message: `Exec provider "${params.providerName}" response missing "values".`
  });
  const responseErrors = (0, _utilsD5DtWkEu.l)(parsed.errors) ? parsed.errors : null;
  const out = {};
  for (const id of params.ids) {
    if (responseErrors && id in responseErrors) {
      const entry = responseErrors[id];
      if ((0, _utilsD5DtWkEu.l)(entry) && typeof entry.message === "string" && entry.message.trim()) throw refResolutionError({
        source: "exec",
        provider: params.providerName,
        refId: id,
...
async function resolveExecRefs(params) {
  ...
  const commandPath = (0, _utilsD5DtWkEu.m)(params.providerConfig.command);
  ...
  const timeoutMs = (0, _sharedDLDubI1E.r)(params.providerConfig.timeoutMs, DEFAULT_EXEC_TIMEOUT_MS);
  const noOutputTimeoutMs = (0, _sharedDLDubI1E.r)(params.providerConfig.noOutputTimeoutMs, timeoutMs);
  const maxOutputBytes = (0, _sharedDLDubI1E.r)(params.providerConfig.maxOutputBytes, DEFAULT_EXEC_MAX_OUTPUT_BYTES);
...
async function resolveSecretRefValues(refs, options) {
  ...
  for (const ref of refs) {
    ...
    if (ref.source === "exec" && !(0, _refContractB0QmVSlT.o)(id)) throw new Error(`${(0, _refContractB0QmVSlT.a)()} (ref: ${ref.source}:${ref.provider}:${id}).`);
    uniqueRefs.set((0, _refContractB0QmVSlT.u)(ref), {
      ...ref,
      id
    });
  }
  ...
  const taskResults = await (0, _runWithConcurrencyDiVVaJMi.t)({
  ...
  for (const result of taskResults.results) for (const ref of result.group.refs) {
    ...
    resolved.set((0, _refContractB0QmVSlT.u)(ref), result.values.get(ref.id));
  }
  return resolved;
}
async function resolveSecretRefValue(ref, options) {
  const cache = options.cache;
  const key = (0, _refContractB0QmVSlT.u)(ref);
...
async function resolveSecretRefString(ref, options) {
  ...
  if (!(0, _sharedDLDubI1E.n)(resolved)) throw new Error(`Secret reference "${ref.source}:${ref.provider}:${ref.id}" resolved to a non-string or empty value.`);
  return resolved;
}
//#endregion /* v9-8579a81df04c01ac */
