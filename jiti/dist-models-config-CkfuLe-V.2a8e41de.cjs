"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = ensureOpenClawModelsJson;var _errorsD8p6rxH = require("./errors-D8p6rxH8.js");
...
require("./public-surface-runtime-nilbjlrn.js");
require("./bundled-plugin-metadata-Da-MNAHv.js");
...
var _modelsConfigProvidersSecretsClAqHxSG = require("./models-config.providers.secrets-ClAqHxSG.js");
require("./facade-loader-CGu7k8Om.js");
require("node:url");
...
function mergeProviders(params) {
  ...
  for (const [key, explicit] of Object.entries(params.explicit ?? {})) {
    const providerKey = (0, _stringCoerceBUSzWgUA.s)(key) ?? "";
...
function resolveProviderApi(entry) {
  return (0, _stringCoerceBUSzWgUA.s)(entry?.api);
}
function resolveModelApiSurface(entry) {
  ...
  const apis = entry.models.flatMap((model) => {
    if (!model || typeof model !== "object") return [];
    const api = model.api;
    const normalized = (0, _stringCoerceBUSzWgUA.s)(api);
...
function shouldPreserveExistingApiKey(params) {
  ...
  if (nextApiKey && (0, _modelAuthMarkersVeOgG6R.u)(nextApiKey)) return false;
  return !secretRefManagedProviders.has(providerKey) && typeof existing.apiKey === "string" && existing.apiKey.length > 0 && !(0, _modelAuthMarkersVeOgG6R.u)(existing.apiKey, { includeEnvVarName: false });
}
...
_nodePath.default.resolve("/home/linuxbrew/.linuxbrew/lib/node_modules/openclaw/dist", "../..");
//#endregion
...
function loadProviderRuntime() {
  providerRuntimePromise ??= Promise.resolve().then(() => jitiImport("./plugins/provider-discovery.runtime.js").then((m) => _interopRequireWildcard(m)));
...
function normalizePluginDiscoveryResult(params) {
  ...
  if ("provider" in result) {
    ...
    {
      const normalizedKey = (0, _providerIdKaStHhRz.r)(providerId);
  ...
  for (const [key, value] of Object.entries(result.providers)) {
    const normalizedKey = (0, _providerIdKaStHhRz.r)(key);
...
const log = (0, _subsystemCgmckbux.t)("agents/model-providers");
...
function resolveProviderDiscoveryFilter(params) {
  ...
  for (const id of ids) {
    const owners = (0, _pluginAutoEnableBbVfCcz.u)({
      provider: id,
      config,
      workspaceDir,
      env
...
function resolveConfiguredImplicitProvider(params) {
  for (const providerId of params.providerIds) {
    const configured = (0, _providerIdKaStHhRz.n)(params.configuredProviders ?? void 0, providerId);
...
async function resolvePluginImplicitProviders(ctx, providers, order) {
  ...
  for (const provider of byOrder[order]) {
    const resolveCatalogProviderApiKey = (providerId) => {
      ...
      return {
        apiKey: (0, _modelAuthMarkersVeOgG6R.u)(syntheticApiKey) ? syntheticApiKey : (0, _modelAuthMarkersVeOgG6R.m)("file"),
...
async function resolveImplicitProviders(params) {
  ...
  let authStore;
  const getAuthStore = () => authStore ??= (0, _storeC1I9Mkh.n)(params.agentDir, { allowKeychainPrompt: false });
  const context = {
    ...params,
    get authStore() {
      return getAuthStore();
    ...
    env,
    resolveProviderApiKey: (0, _modelsConfigProvidersSecretsClAqHxSG.t)(env, getAuthStore, params.config),
    resolveProviderAuth: (0, _modelsConfigProvidersSecretsClAqHxSG.n)(env, getAuthStore, params.config)
  };
...
function resolveProviderPluginLookupKey(providerKey, provider) {
  const api = (0, _stringCoerceBUSzWgUA.s)(provider?.api) ?? "";
  if (providerKey === "google-antigravity" || providerKey === "google-vertex" || api === "google-generative-ai") return "google";
  if (provider?.models?.some((model) => (0, _stringCoerceBUSzWgUA.s)(model.api) === "google-generative-ai")) return "google";
  if (api && _zodSchemaCoreCYrn8zgQ.R.includes(api) && !GENERIC_PROVIDER_APIS.has(api)) return api;
  return providerKey;
}
//#endregion
...
function applyProviderNativeStreamingUsagePolicy(providerKey, provider) {
  return (0, _providerRuntimeKhVgWetm.r)({
...
function normalizeProviderConfigPolicy(providerKey, provider) {
  return (0, _providerRuntimeKhVgWetm.h)({
    provider: resolveProviderPluginLookupKey(providerKey, provider),
    context: {
      provider: providerKey,
      providerConfig: provider
...
function resolveProviderConfigApiKeyPolicy(providerKey, provider) {
  ...
  return (env) => (0, _providerRuntimeKhVgWetm.O)({
...
function normalizeSourceProviderLookup(providers) {
  ...
  for (const [key, provider] of Object.entries(providers)) {
    ...
    if (!normalizedKey || !(0, _utilsD5DtWkEu.l)(provider)) continue;
    out[normalizedKey] = provider;
  }
  return out;
}
function resolveSourceManagedApiKeyMarker(params) {
  const sourceApiKeyRef = (0, _typesSecretsCeL3gSMO.d)({
    value: params.sourceProvider?.apiKey,
    defaults: params.sourceSecretDefaults
  }).ref;
  if (!sourceApiKeyRef || !sourceApiKeyRef.id.trim()) return;
  return sourceApiKeyRef.source === "env" ? sourceApiKeyRef.id.trim() : (0, _modelAuthMarkersVeOgG6R.m)(sourceApiKeyRef.source);
}
function resolveSourceManagedHeaderMarkers(params) {
  const sourceHeaders = (0, _utilsD5DtWkEu.l)(params.sourceProvider?.headers) ? params.sourceProvider.headers : void 0;
  if (!sourceHeaders) return {};
  const markers = {};
  for (const [headerName, headerValue] of Object.entries(sourceHeaders)) {
    const sourceHeaderRef = (0, _typesSecretsCeL3gSMO.d)({
      value: headerValue,
      defaults: params.sourceSecretDefaults
    }).ref;
    if (!sourceHeaderRef || !sourceHeaderRef.id.trim()) continue;
    markers[headerName] = sourceHeaderRef.source === "env" ? (0, _modelAuthMarkersVeOgG6R.p)(sourceHeaderRef.id) : (0, _modelAuthMarkersVeOgG6R.h)(sourceHeaderRef.source);
  }
  return markers;
}
function enforceSourceManagedProviderSecrets(params) {
  ...
  for (const [providerKey, provider] of Object.entries(providers)) {
    if (!(0, _utilsD5DtWkEu.l)(provider)) continue;
    ...
    if (Object.keys(sourceHeaderMarkers).length > 0) {
      const currentHeaders = (0, _utilsD5DtWkEu.l)(nextProvider.headers) ? nextProvider.headers : void 0;
...
function normalizeProviders(params) {
  ...
  const resolveProfileApiKey = (providerKey) => {
    authStore ??= (0, _storeC1I9Mkh.n)(params.agentDir, { allowKeychainPrompt: false });
    return (0, _modelsConfigProvidersSecretsClAqHxSG.s)({
      provider: providerKey,
      store: authStore,
      env
    });
  ...
  for (const [key, provider] of Object.entries(providers)) {
    ...
    const normalizedHeaders = (0, _modelsConfigProvidersSecretsClAqHxSG.a)({
    ...
    const providerWithConfiguredApiKey = (0, _modelsConfigProvidersSecretsClAqHxSG.i)({
    ...
    const providerWithResolvedEnvApiKey = (0, _modelsConfigProvidersSecretsClAqHxSG.o)({
      providerKey: normalizedKey,
      provider: normalizedProvider,
      env,
      secretRefManagedProviders: params.secretRefManagedProviders
    ...
    const providerWithApiKey = (0, _modelsConfigProvidersSecretsClAqHxSG.c)({
      providerKey: normalizedKey,
      provider: normalizedProvider,
      env,
      profileApiKey,
...
function resolveProvidersForMode(params) {
  ...
  if (!(0, _utilsD5DtWkEu.l)(existing) || !(0, _utilsD5DtWkEu.l)(existing.providers)) return params.providers;
...
async function readFileMtimeMs(pathname) {
  try {
    const stat = await _promises.default.stat(pathname);
...
async function buildModelsJsonFingerprint(params) {
  const authProfilesMtimeMs = await readFileMtimeMs(_nodePath.default.join(params.agentDir, "auth-profiles.json"));
  const modelsFileMtimeMs = await readFileMtimeMs(_nodePath.default.join(params.agentDir, "models.json"));
  const envShape = (0, _stateDirDotenvCh7ZMbTt.i)(params.config, {});
...
async function ensureModelsFileModeForModelsJson(pathname) {
  await _promises.default.chmod(pathname, 384).catch(() => {});
}
async function writeModelsFileAtomicForModelsJson(targetPath, contents) {
  const tempPath = `${targetPath}.${process.pid}.${Date.now()}.tmp`;
  await _promises.default.writeFile(tempPath, contents, { mode: 384 });
  await _promises.default.rename(tempPath, targetPath);
}
function resolveModelsConfigInput(config) {
  const runtimeSource = (0, _runtimeSnapshotBwqEmc6G.a)();
  if (!config) {
    const loaded = (0, _io5pxHCi7V.a)();
    return {
  ...
  const projected = (0, _io5pxHCi7V.s)(config);
  return {
    config: projected,
...
async function ensureOpenClawModelsJson(config, agentDirOverride) {
  ...
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : (0, _agentPathsJWlHCT.t)();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    const env = (0, _stateDirDotenvCh7ZMbTt.i)(cfg);
    ...
    await _promises.default.mkdir(agentDir, {
      recursive: true,
      mode: 448
    });
